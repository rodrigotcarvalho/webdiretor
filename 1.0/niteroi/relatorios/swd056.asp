...
for i=0 to ubound(conjunto_dados)	
	...
	While Not RSA.EOF
		...
		nome_aluno=replace_latin_char(nome_aluno,"url")	
	...
	if curso=0 then
		...
		if gera_pdf="sim" then	
			...
			mensagem_cabecalho=no_curso&texto_turma
	...
	'CABEÇALHO==========================================================================================		
			...
			Set param_table1 = Pdf.CreateParam("width="&area_utilizavel&"; height=50; rows=4; cols=3; border=0; cellborder=0; cellspacing=0;")
			...
			Table(2, 1).AddText mensagem_cabecalho, "size=8;html=true", Font
			Table(2, 3).AddText "<div align=""right""><b>Ano Letivo:</b> "&ano_letivo&"</div>", "size=8;html=true", Font 				
			Table(3, 1).AddText "<div align=""right""><b>Legenda:</b> Freq(%) - Frequ&ecirc;ncia que o aluno obteve no ano   /     RF - Resultado Final    /     APR - Aprovado   /    APP - Aprovado Por Progress&atilde;o Parcial  /   REP - Reprovado   /<BR>  TRA  - Matr&iacute;cula Trancada   /   TRF  - Transferido   /  DES  -  Desistente</div>", "size=8;html=true", Font	
...
'response.end()
			...
			if (ubound(co_materia_exibe)+1)>20 then
				colunas_de_notas=ubound(co_materia_exibe)+1
			else
				colunas_de_notas=20		
			end if
			total_de_colunas=colunas_de_notas+2								
			altura_medias=25
			y_segunda_tabela=y_primeira_tabela-55	
			...
			Notas_Tit(1, (total_de_colunas-1)).AddText "<div align=""center""><b>FRQ.<br>(%)</b></div>", "size=8;alignment=center; indenty=0;html=true", Font
			Notas_Tit(1, total_de_colunas).AddText "<div align=""center""><b>RF.</b></div>", "size=8;alignment=center; indenty=6;html=true", Font 
			...
			for a=0 to ubound(alunos_encontrados)	
				...
				for r=0 to ubound(vetor_materia_resultado)
					m3=Calc_Ter_Media (unidade, curso, co_etapa, turma, dados_alunos(0), vetor_materia_resultado(r), caminho_nota, tb_nota, prd_ter_media, "sem_calculo", outro)	
					res=split(m3,"#!#")
					if r=0 then
						notas_apuradas=res(0)
						resultados_apurados=res(1)
					else
						notas_apuradas=notas_apuradas&"#!#"&res(0)					
				...
				SQL_s ="SELECT CO_Situacao FROM TB_Matriculas where CO_Matricula ="& dados_alunos(0)&" AND NU_Ano="&ano_letivo
				...
				if situac<>"C" THEN
					...
					if RSt.eof then			
						...
						'resultado_final_aluno=replace_latin_char(RSt("TX_Descricao_Situacao"),"html")
						resultado_final_aluno=situac
					end if	
				end if	
				...
				resultados_alunos = split(notas_apuradas, "#!#" )	
				...
				nome_aluno = replace(dados_alunos(2),"%2E",".")	
				nome_aluno = replace(nome_aluno,"%28","(")	
				nome_aluno = replace(nome_aluno,"%29",")")										
				Notas_Tit(linha, 2).AddText nome_aluno, param_materias
				
				coluna=2
				param_materias.Add "indentx=0"
				
				calcular_frequencia="s"
				
				for n=0 to ubound(co_materia_verifica)	
				
		
					Set RSm = Server.CreateObject("ADODB.Recordset")
					SQLm = "SELECT * FROM TB_Materia where CO_Materia ='"& co_materia_verifica(n) &"'"
					RSm.Open SQLm, CON0				
...
'response.Write(resultados_materia(0)&"<<BR>")
			
					if (resultados_materia(0)="&nbsp;" or resultados_materia(0)="" or isnull(resultados_materia(0))) and disc_obrigatoria=TRUE then
						calcular_frequencia="n"				
					end if	
'response.Write(calcular_frequencia&"-"&disc_obrigatoria&"<<BR>")					
...
'					else
					if isnumeric(media) then
						conceito=formatnumber(media,parametros_gerais(unidade,curso,etapa,turma,disciplina,"decimais_md",outro))	
					else
						conceito=media
					end if				
'					end if						
				...
				next		
'			response.Write(		calcular_frequencia)
'			response.End()
				if calcular_frequencia="s" then
					
					Set RS3 = Server.CreateObject("ADODB.Recordset")
					SQL3 = "SELECT SUM(NU_Aulas) as HorasAula FROM TB_Programa_Aula where CO_Etapa ='"& co_etapa &"' AND CO_Curso ='"& curso &"'"
					RS3.Open SQL3, CON0		
					...
					if curso=1 then
						...
						horas_aula=RS3("HorasAula")		
					end if						
					
			
								
					Set RSd = Server.CreateObject("ADODB.Recordset")
					SQLd = "SELECT * FROM TB_Etapa where CO_Etapa ='"& co_etapa &"' AND CO_Curso ='"& curso &"'"
					RSd.Open SQLd, CON0
					...
					if tipo_freq="D" then									
						...
						SQL_N = "Select * from TB_Frequencia_Periodo WHERE CO_Matricula = "& dados_alunos(0)
						...
						SQL1 = "SELECT SUM(NU_Faltas) as Total_Faltas FROM "&tb_nota&" where CO_Matricula ="& dados_alunos(0)
						RS1.Open SQL1, CON_N	
						
						total_faltas=RS1("Total_Faltas")									
					end if	
					total_faltas=total_faltas*1
					if total_faltas="" or isnull(total_faltas) then
						total_faltas=0
					end if					

					frequencia=((horas_aula-total_faltas)/horas_aula)*100	
					'response.Write(	frequencia&"=(("&horas_aula&"-"&total_faltas&")/"&horas_aula&")*100<BR>")
				...
				Notas_Tit(linha, (total_de_colunas-1)).AddText "<div align=""center"">"&frequencia&"</DIV>", param_materias	
				if dados_alunos(3)="T" then
					resultado_final_aluno="TRF"
				elseif dados_alunos(3)="D" then
					resultado_final_aluno="DES"
				elseif dados_alunos(3)="R" then
					resultado_final_aluno="TRA"															
				end if
				Notas_Tit(linha, total_de_colunas).AddText "<div align=""center"">"&resultado_final_aluno&"</DIV>", param_materias															
			next
'response.End()
			limite=0
			Do While True
