<%
'Verificar se a nota de recuperação do 4 periodo foi laçançada
Function Recuperacao4PeriodoLancada (unidade, curso, co_etapa, turma, tp_modelo, tp_freq, cod_cons, co_materia, conexao, tb_nota, tp_materia)

var_bd_rec=var_bd_periodo(tp_modelo,tp_freq,tb_nota,4,"BDR")		
			
		if tp_materia="T_F_F_N" or tp_materia="F_T_F_N"	 or tp_materia="F_F_T_N" then
			
			codigo_materia_pr=busca_materia_mae(co_materia)
			va_media=Calcula_Media_T_F_F_N(unidade, curso, co_etapa, turma, tp_modelo, tp_freq, cod_cons, codigo_materia_pr, co_materia, conexao, tb_nota, 4, var_bd_rec, outro)
		...
		elseif tp_materia="T_T_F_N" then
			...
			va_media=Calcula_Media_T_T_F_N(unidade, curso, co_etapa, turma, tp_modelo, tp_freq, cod_cons, co_materia, vetor_filhas_T_T_F_N, conexao, tb_nota, 4, var_bd_rec, outro)		
		...
		elseif tp_materia="T_F_T_N" then
			...
			va_media=Calcula_Media_T_F_T_N(unidade, curso, co_etapa, turma, tp_modelo, tp_freq, cod_cons, co_materia, vetor_filhas_T_F_T_N, conexao, tb_nota, 4, var_bd_rec, outro)	
 ...
 if isnumeric(valor) then
 	Recuperacao4PeriodoLancada= "S"
 else
 	Recuperacao4PeriodoLancada= "N"
 end if
...
divisor=0		
	...
	if anulou="s" then
		...
		if nome_nota=campo_falta then
			...
			response.Write(va_m3&"-"&acumula&"/"&divisor&"-<BR>")		
...
'response.Write(cod_cons&"_"&co_materia&"_"&tp_materia&"<BR>")
	...
	for prd_soma=1 to maximo_periodo
		...
		var_bd=var_bd_periodo(tp_modelo,tp_freq,tb_nota,prd_soma,"BDM3")
		var_bd_rec=var_bd_periodo(tp_modelo,tp_freq,tb_nota,periodo_rec,"BDR")		
		...
		elseif tp_materia="T_F_T_N" then
			...
			va_media=Calcula_Media_T_F_T_N(unidade, curso, co_etapa, turma, tp_modelo, tp_freq, cod_cons, co_materia, vetor_filhas_T_F_T_N, conexao, tb_nota, prd_soma, var_bd, outro)	
			if periodo_rec>=prd_soma then			
				va_ast=Calcula_Asterisco(tp_modelo, tp_freq, unidade, curso, co_etapa, turma, tp_modelo, tp_freq, cod_cons, co_materia, conexao, tp_materia, tb_nota, prd_soma)	
			else
				va_ast="&nbsp;"				
			end if	
...
'		response.Write("A+"&acumula&"<BR>")				
	next	
'response.Write(acumula&"<BR>")			
'response.End()
	tipo_arred = parametros_gerais(unidade, curso, co_etapa, turma, codigo_materia_pr,"arred_md",0)	
	val_arred = parametros_gerais(unidade, curso, co_etapa, turma, codigo_materia_pr,"decimais_md",0)	
	if valor="nulo" then
		Calcula_Soma="&nbsp;"
	else

		Calcula_Soma=arredonda(acumula,tipo_arred,val_arred,0)
