	...
	'================================================================================================================			
			...
			for ae=0 to ubound(alunos_encontrados)	
				...
				for cmv=0 to ubound(co_materia_verifica)
					...
					if resultados_apurados(1)<>"&nbsp;" then							
						modifica_result = Verifica_Conselho_Classe(dados_alunos(0), co_materia_verifica(cmv), "MA", outro)									
						if modifica_result = "N" then
							modifica_result = Verifica_Conselho_Classe(dados_alunos(0), co_materia_verifica(cmv), "RF", outro)
							if modifica_result = "N" then
								modifica_result = Verifica_Conselho_Classe(dados_alunos(0), co_materia_verifica(cmv), "MF", outro)						
							end if										
						end if
						if modifica_result <> "N" then
							res_aluno = modifica_result
						else
							res_aluno = resultados_apurados(1)	
						end if								
					else	
						res_aluno = resultados_apurados(1)						
					end if						
					...
					if cmv=0 then
						vetor_resultados_apurados=res_aluno
						vetor_resultados_completos=resultados_apurados(0)&"#!#"&res_aluno							
					else
						vetor_resultados_apurados=vetor_resultados_apurados&"#!#"res_aluno	
						vetor_resultados_completos=vetor_resultados_completos&"#$#"&resultados_apurados(0)&"#!#"&res_aluno											
					end if	
				...
				for cmvr=0 to ubound(co_materia_verifica)
					...
					ELSE	
						if resultado_final_aluno="REP" or resultado_final_aluno="Rep" or resultado_final_aluno="APR" or resultado_final_aluno="Apr" or resultado_final_aluno="APC" then
							if isnumeric(media)	then
...
'					next
				...
				for r = 0 to ubound (vetor_resultados)
					...
					if vetor_resultados(r) ="APR" or vetor_resultados(r) = "REP" then						
