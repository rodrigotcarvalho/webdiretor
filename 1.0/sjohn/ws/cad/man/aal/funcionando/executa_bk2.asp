...
if opt="v" then
cod_assoc= request.form("vinc_pub")
vinculado= request.form("aluno_pub")
...
WEND
		...
		SQL = "SELECT * FROM TB_Alunos WHERE CO_Matricula ="& cod_assoc
		RS.Open SQL, CON1
			...
			SQL_aux = "SELECT * FROM TBI_Alunos WHERE CO_Matricula ="& cod_assoc
			...
			if RS_aux.EOF Then
				...
				RSALUNO_aux_bd("CO_Matricula")=cod_assoc
				...
				sql_atualiza_al= "UPDATE TBI_Alunos SET IN_Pai_Falecido="&pai_fal&", IN_Mae_Falecida="&mae_fal&", TP_Resp_Fin ='"& resp_fin &"', TP_Resp_Ped ='"& resp_ped &"' WHERE CO_Matricula = "& cod_assoc
		...
		SQLA = "SELECT * FROM TB_Contatos WHERE TP_Contato ='ALUNO' And CO_Matricula ="& cod_assoc
...
numero_com = RSCONTA("NU_Logradouro_Com")
...
'Grava dados de aluno Vinculado
		...
		SQLAC_delete= "DELETE * FROM TBI_Contatos WHERE CO_Matricula ="&cod_assoc
		RSCONTATO_aux_delete.Open SQLAC_delete, CONCONT_aux
				...
				RSCONTATO_aux_bd1("CO_Matricula")=cod_assoc
...
familiares = Split(ordem_familiares, "##")
	for i=1 to ubound(familiares)
		...
		SQLAA= "SELECT * FROM TB_Contatos WHERE TP_Contato='"&cod_familiar&"' and CO_Matricula ="&cod_assoc
		...
		if RSCONTATO.EOF then
			...
			SQLAA_aux= "SELECT * FROM TBI_Contatos WHERE TP_Contato='"&cod_familiar&"' and CO_Matricula ="&cod_assoc
			...
			if RSCONTATO_aux.EOF then
				...
				RSCONTATO_aux_bd("CO_Matricula")=cod_assoc
		...
		Set RSCONTATO_aux_delete = Server.CreateObject("ADODB.Recordset")
		SQLAC_delete= "DELETE * FROM TBI_Contatos WHERE TP_Contato = '"&cod_familiar&"' and CO_Matricula ="&vinculado
		RSCONTATO_aux_delete.Open SQLAC_delete, CONCONT_aux
				...
				RSCONTATO_aux_bd_vincula("CO_Matricula")=vinculado
				RSCONTATO_aux_bd_vincula("TP_Contato")=cod_familiar
				if cod_familiar<>vinculado then
					RSCONTATO_aux_bd_vincula("CO_Matricula_Vinc")=cod_assoc
				...
				sql_atualiza=sql_atualiza&"CO_CEP_Com='"& cep_com_familiar_aux &"', NU_Telefones_Com ='"& tel_com_familiar_aux&"', "&sql_vinc&", TP_Contato_Vinc ='"& tp_vinc_familiar_aux&"' WHERE CO_Matricula = "& cod_assoc &" AND TP_Contato = '"& cod_familiar &"'"
				Set RSCONTATO_aux_bd2 = CONCONT_aux.Execute(sql_atualiza)

				Set RSCONTATO_aux_bd2_vincula = server.createobject("adodb.recordset")
				if cod_familiar<>vinculado then
					sql_atualiza_vincula= "UPDATE TBI_Contatos SET CO_Matricula_Vinc ="& cod_assoc &", TP_Contato_Vinc ='"& cod_familiar&"' WHERE CO_Matricula = "& vinculado &" AND TP_Contato = '"& cod_familiar &"'"
				else
					sql_atualiza_vincula= "UPDATE TBI_Contatos SET CO_Matricula_Vinc =NULL, TP_Contato_Vinc ='' WHERE CO_Matricula = "& vinculado &" AND TP_Contato = '"& cod_familiar &"'"
				end if
...
<table width="1000" border="0" align="right" cellspacing="0" class="tb_corpo">
  <td class="tb_tit"
>Dados copiados do Aluno</td>
          </tr>
          <tr> 
            <td> 
<% 
vinculado=vinculado*1
cod_assoc=cod_assoc*1
if vinculado=cod_assoc then
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
                        <tr bgcolor="#FFFFFF" background="../../../../img/fundo_interno.gif"> 
                          ...
                          <td width="46%" height="10"><font size="2" face="Arial, Helvetica, sans-serif">  <!--  'O valor de codigo.value vem do arquivo altera.asp-->
                            <input name="Button" type="button" class="borda_bot" id="Submit" value="Vincular" onClick="recuperarCurso(vinculado_novo.value,codigo.value)">
                            </font> </td>
                        </tr>
                      </table>
<%else%>			
			<table width="100%" border="0" cellspacing="0" cellpadding="0">
              ...
              <%response.Write(Server.URLEncode(cod_assoc))%><input name="vinculado" type="hidden" class="borda" id="vinculado" value="<%response.Write(cod_assoc)%>" size="12"><input name="origem" type="hidden" class="borda" id="vinculado" value="<%response.Write(vinculado)%>" size="12">
              </font></div>
                    </td>
          ...
          <td width="37%" height="10"> <div align="left"><font class="form_dado_texto"> 
              <%response.Write(Server.URLEncode(nome_prof))%>
                  ...
                  <td width="46%" height="10"><font size="2" face="Arial, Helvetica, sans-serif">
                    <input type="button" name="Button" value="Desvincular"  class="borda_bot3" onClick="recuperarCurso(origem.value,origem.value)">
                    </font> </td>
                </tr>
              </table>
<%end if%>			  
...
><font class="form_corpo"> 
            <input name="rua_res" type="text" class="textInput" id="rua4" value="<%response.Write(Server.URLEncode(rua_res))%>" size="30" maxlength="30">
            </font></td>
...
><font class="form_corpo">
            <input name="num_res" type="text" class="textInput" id="numero4" value="<%response.Write(Server.URLEncode(num_res))%>" size="12" maxlength="10">          
			</font></td>
          <td width="15%" class="tb_corpo"
...
><div align="left"><font class="form_corpo"> </font> <font class="form_corpo"> 
              <input name="comp_res" type="text" class="textInput" id="comp_res" value="<%response.Write(Server.URLEncode(comp_res))%>" size="20" maxlength="15">
              </font></div></td>
        </tr>
        <tr> 
...
><div align="center">:</div></td>
          <td height="10" class="tb_corpo"
><font class="form_corpo"> 
            <input name="cep" type="text" class="textInput" id="cep" value="<%response.Write(Server.URLEncode(cep))%>" size="11" maxlength="9" onKeyup="formatar(this, '#####-###')">
			</font></td>
          <td height="10" class="tb_corpo"
>&nbsp;</td>
          <td class="tb_corpo"
...
><div align="center">:</div></td>
          <td height="10" class="tb_corpo"
><font class="form_corpo"> 
            <input name="tel_res" type="text" class="textInput" id="tel_res" value="<%response.Write(Server.URLEncode(tel_res))%>" size="30" maxlength="30">          
			</font></td>
          <td height="10" class="tb_corpo"
...
><div align="center">:</div></td>
          <td height="10" class="tb_corpo"
><font class="form_corpo"> 
            <input name="rua_com" type="text" class="textInput" id="rua_com" value="<%response.Write(Server.URLEncode(rua_com))%>" size="30" maxlength="30">		
			</font></td>
          <td height="10" class="tb_corpo"
> <div align="left"><font class="form_dado_texto"> N&uacute;mero</font></div></td>
          <td class="tb_corpo"
><div align="center">:</div></td>
          <td class="tb_corpo"
><font class="form_corpo"> 
            <input name="num_com" type="text" class="textInput" id="num_com" value="<%response.Write(Server.URLEncode(num_com))%>" size="12" maxlength="10">
			</font></td>
          <td class="tb_corpo"
><font class="form_dado_texto">Complemento</font></td>
          <td class="tb_corpo"
...
><div align="left"><font class="form_corpo"> 
              <input name="comp_com" type="text" class="textInput" id="comp_com" value="<%response.Write(Server.URLEncode(comp_com))%>" size="20" maxlength="15">
			  </font></div></td>
        </tr>
        <tr class="tb_corpo"
> 
          <td height="26"><font class="form_dado_texto">Estado</font></td>
...
%>
          ...
          <td><font class="form_dado_texto">Bairro</font></td>
          <td><div align="center">:</div></td>
          <td height="26"><font class="form_corpo"> 
                            <div id="bairro_com"><font class="form_corpo"> 
...
> 
          ...
          <td><div align="center">:</div></td>
          <td height="26"><font class="form_corpo"> 
            <input name="cepcom" type="text" class="textInput" id="cepcom" value="<%response.Write(Server.URLEncode(cep2))%>" size="11" maxlength="9" onKeyup="formatar(this, '#####-###')">
		    </font></td>
          <td height="26">&nbsp;</td>
          <td>&nbsp;</td>
          <td>&nbsp;</td>
        ...
        <tr class="tb_corpo"
> 
          <td height="28"> <div align="left"><font class="form_dado_texto">Telefones 
              deste endere&ccedil;o:</font></div></td>
          <td><div align="center">:</div></td>
          <td width="22%" height="28"><font class="form_corpo"> 
            <input name="tel_com" type="text" class="textInput" id="tel_com" value="<%response.Write(Server.URLEncode(tel_com))%>" size="30" maxlength="30">
			</font></td>
          <td height="28"> <div align="left">
		  <marquee id="mqLooper1" loop="1"  onStart="<%response.Write("recuperarFamiliares('"&Server.URLEncode(ordem_familiares)&"','"&total_tp_familiares&"','"&foco&"','"&vinculado&"')")%>"></marquee>
		  </div></td>
          <td><div align="center"></div></td>
          <td>&nbsp;</td>
          <td>&nbsp;</td>
    ...
    <td><table width="100%" border="0" cellspacing="0">
        <tr> 
          ...
          <td width="22%" height="26"><font class="form_corpo"> 
            <input name="pai" type="text" class="borda" onBlur="recuperarPai(this.value,'p')" value="<%response.Write(pai)%>" size="30">
            </font></td>
...
<input name="mae" type="text" class="borda" onBlur="recuperarMae(this.value,'m')" value="<%response.Write(mae)%>" size="30">
            </font></td>
...
<input name="num_res_fam" type="text" class="borda" id="num_res"  value="<%response.write(num_res)%>" size="12" maxlength="10">
            </font></td>
...
<%response.write(Server.URLEncode(comp_res))%>
              <input name="comp_res_fam" type="text" class="borda" id="comp_res" value="<%response.write(Server.URLEncode(comp_res))%>" size="12" maxlength="10">
...
END IF
%>
              <input name="estadores_fam" type="text" class="borda" id="estadores_fam" value="<%response.write(Server.URLEncode(NO_UF))%>" size="12" maxlength="10">
...
END IF
%>
              <input name="cidres_fam" type="text" class="borda" id="cidres_fam" value="<%response.write(Server.URLEncode(NO_UF))%>" size="12" maxlength="10">
...
END IF
%>
              <input name="bairrores_fam" type="text" class="borda" id="bairrores_fam" value="<%response.write(Server.URLEncode(NO_UF))%>" size="12" maxlength="10">
...
><font class="form_dado_texto"> <%response.write(cep)%>
            <input name="cep_fam" type="text" class="borda" id="cep_fam" value="<%response.write(cep)%>" size="11" maxlength="9">
...
><font class="form_corpo"><%response.write(tel_res)%> 
            <input name="tel_res_fam" type="text" class="borda" id="tel_res_fam" value="<%response.write(tel_res)%>" size="50" maxlength="50">
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
                                    <tr> 
                                      <td width="9%"><input type="radio" name="mes_end" value="s"  onClick="recuperarEnd('<%response.Write(cod)%>','<%response.Write(foco)%>')" checked></td>
                                      <td width="25%"><font class="form_corpo">Sim</font></td>
                                      <td width="5%"><input name="mes_end" type="radio"  onClick="recuperarOrigemEnd('<%response.Write(cod)%>','<%response.Write(foco)%>')" value="n"></td>
                                      <td width="61%"><font class="form_corpo">N&atilde;o 
                                        <input name="id_res_familiar_aux" type="text" value="s">
                                        </font></td>
                                    </tr>
                                  </table>
</td>
...
> <div align="left"><font class="form_dado_texto">:</font></div></td>
          <td width="217" height="10" class="tb_corpo"
><font class="form_corpo"> 
            <input name="rua_res_fam" type="text" class="borda" id="rua_res" size="30">
            </font></td>
          <td width="140" height="10" class="tb_corpo"
> <div align="left"><font class="form_dado_texto"> N&uacute;mero</font></div></td>
          <td width="19" class="tb_corpo"
> <div align="center"><font class="form_dado_texto">:</font></div></td>
          <td width="206" class="tb_corpo"
><font class="form_corpo"> 
            <input name="num_res_fam" type="text" class="borda" id="num_res" size="12" maxlength="10">
            </font></td>
          <td width="90" class="tb_corpo"
><font class="form_dado_texto">Complemento</font></td>
          <td width="15" class="tb_corpo"
> <div align="center"><font class="form_dado_texto">:</font></div></td>
          <td width="139" height="10" class="tb_corpo"
> <div align="left"><font class="form_corpo"> </font> <font class="form_corpo"> 
              <input name="comp_res_fam" type="text" class="borda" id="comp_res" size="12" maxlength="10">
              </font></div></td>
        </tr>
        <tr> 
          <td width="145" height="21" class="tb_corpo"
...
><font class="form_dado_texto"> 
            <input name="cep_fam" type="text" class="borda" id="cep" onKeyup="formatar(this, '#####-###')" size="11" maxlength="9">
            </font></td>
          <td width="140" height="10" class="tb_corpo"
>&nbsp;</td>
          <td width="19" class="tb_corpo"
...
> <div align="left"><font class="form_dado_texto">:</font></div></td>
          <td height="10" colspan="2" class="tb_corpo"
><font class="form_corpo"> 
            <input name="tel_res_fam" type="text" class="borda" id="tel_res" size="50" maxlength="50">
            </font> </td>
          <td width="19" class="tb_corpo"
> <div align="center"></div></td>
          <td width="206" class="tb_corpo"
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
                                    <tr> 
                                      <td width="9%"><input type="radio" name="mes_end" value="s"  onClick="recuperarEnd('<%response.Write(cod)%>','<%response.Write(foco)%>')"></td>
                                      <td width="25%"><font class="form_corpo">Sim</font></td>
                                      <td width="5%"><input name="mes_end" type="radio"  onClick="recuperarOrigemEnd('<%response.Write(cod)%>','<%response.Write(foco)%>')" value="n" checked></td>
                                      <td width="61%"><font class="form_corpo">N&atilde;o 
                                        <input name="id_res_familiar_aux" type="text" value="n">
                                        </font></td>
                                    </tr>
                                  </table>
										</td>
