...
'		end if	
	...
	if modalidade="cc" then
		...
		dados_buscados ="CO_Item#!#NO_Item#!#Sum_QT_Solicitado#!#Expr1" 	
	
		if isnull(turma) or turma = "" or turma = "999990" then
			if isnull(etapa) or etapa = "" or etapa = "999990" then
				query = "SELECT TB_Mov_Estoque.CO_Etapa, TB_Mov_Estoque.CO_Turma, TB_Mov_Estoque_Item.CO_Item, SUM(TB_Mov_Estoque_Item.QT_Solicitado) AS Sum_QT_Solicitado,  Sum(([QT_Solicitado]*[VA_Medio_Refer])) AS Expr1 FROM (TB_Mov_Estoque INNER JOIN TB_Mov_Estoque_Item ON TB_Mov_Estoque.NU_Pedido = TB_Mov_Estoque_Item.NU_Pedido) INNER JOIN TB_Item ON TB_Mov_Estoque_Item.CO_Item = TB_Item.CO_Item Where (DA_Pedido BETWEEN #"&data_de&"# AND #"&data_ate&"#) AND TB_Mov_Estoque.NU_Unidade = 1 AND TB_Mov_Estoque.CO_Curso = '0' GROUP BY TB_Mov_Estoque.CO_Etapa, TB_Mov_Estoque.CO_Turma, TB_Mov_Estoque_Item.CO_Item ORDER BY TB_Mov_Estoque.CO_Etapa, TB_Mov_Estoque.CO_Turma, Sum(([QT_Solicitado]*[VA_Medio_Refer])) DESC"															
			else
				query = "SELECT TB_Mov_Estoque.CO_Etapa, TB_Mov_Estoque.CO_Turma, TB_Mov_Estoque_Item.CO_Item, SUM(TB_Mov_Estoque_Item.QT_Solicitado) AS Sum_QT_Solicitado, Sum(([QT_Solicitado]*[VA_Medio_Refer])) AS Expr1 FROM (TB_Mov_Estoque INNER JOIN TB_Mov_Estoque_Item ON TB_Mov_Estoque.NU_Pedido = TB_Mov_Estoque_Item.NU_Pedido) INNER JOIN TB_Item ON TB_Mov_Estoque_Item.CO_Item = TB_Item.CO_Item Where (DA_Pedido BETWEEN #"&data_de&"# AND #"&data_ate&"#) AND TB_Mov_Estoque.NU_Unidade = 1 AND TB_Mov_Estoque.CO_Curso = '0' AND TB_Mov_Estoque.CO_Etapa = '"&etapa&"'GROUP BY TB_Mov_Estoque.CO_Etapa, TB_Mov_Estoque.CO_Turma, TB_Mov_Estoque_Item.CO_Item ORDER BY TB_Mov_Estoque.CO_Etapa, TB_Mov_Estoque.CO_Turma, Sum(([QT_Solicitado]*[VA_Medio_Refer])) DESC"														
			end if
		else
	...
	elseif modalidade="it" then
		...
		query = "SELECT TB_Mov_Estoque.CO_Etapa, TB_Mov_Estoque.CO_Turma, TB_Mov_Estoque_Item.CO_Item,TB_Mov_Estoque_Item.QT_Solicitado, TB_Mov_Estoque.NU_Pedido FROM (TB_Mov_Estoque INNER JOIN TB_Mov_Estoque_Item ON TB_Mov_Estoque.NU_Pedido = TB_Mov_Estoque_Item.NU_Pedido) LEFT JOIN TB_Item ON TB_Mov_Estoque_Item.CO_Item = TB_Item.CO_Item Where (DA_Pedido BETWEEN #"&data_de&"# AND #"&data_ate&"#) ORDER BY TB_Item.CO_Item,  TB_Mov_Estoque.CO_Etapa, TB_Mov_Estoque.CO_Turma DESC"															
	end if		
end if
