...
>
  <tr> 
    <td width="203" height="15" bgcolor="#FFFFFF">
<% if unidade = 1 then
...
<div align="center"><img src="../../../../img/<%response.Write(logo)%>"> </div></td>
    <td width="741" bgcolor="#FFFFFF"><table width="100%" border="0" align="right" cellspacing="0">
        <tr> 
          <td width="50%" rowspan="2"><font size="3" face="Verdana, Arial, Helvetica, sans-serif"><strong>UNIDADE 
            <%
			no_unidade= ucase(no_unidade)
			response.Write(" "&no_unidade)
			%>
            </strong></font></td>
          <td width="50%" height="8" class="linhaBaixo">&nbsp; </td>
        </tr>
        <tr> 
          <td height="5" ></td>
        </tr>
...
nu_chamada_check = 1
	...
	CONEXAOA = "Select * from TB_Alunos INNER JOIN TB_Matriculas ON TB_Alunos.CO_Matricula=TB_Matriculas.CO_Matricula where TB_Matriculas.NU_Unidade = "& unidade &" AND TB_Matriculas.CO_Curso = '"& curso &"' AND TB_Matriculas.CO_Etapa = '"& co_etapa &"' AND TB_Matriculas.CO_Turma = '"& turma &"' and TB_Matriculas.NU_Ano = "&Session("ano_letivo")&" order by TB_Matriculas.NU_Chamada"
...
nu_chamada_check=nu_chamada_check+1%>
		...
		Set RS4 = Server.CreateObject("ADODB.Recordset")
		SQL4 = "SELECT * FROM TB_Periodo where TP_Modelo='"&tp_modelo&"' AND NU_Periodo="&periodo
		RS4.Open SQL4, CON0
...
NO_Periodo= RS4("SG_Periodo")
  		...
  		Set RS3 = Server.CreateObject("ADODB.Recordset")
		CONEXAO3 = "Select "& campo_check & " from "& nota & " WHERE CO_Materia = '"& nome_nota(k) &"' And CO_Matricula = "& nu_matricula&" And NU_Periodo="&periodo
