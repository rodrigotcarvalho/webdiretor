...
obr=unidade&"$!$"&curso&"$!$"&etapa&"$!$"&turma&"$!$"&co_materia&"$!$"&periodo
...
val_default_lbg = parametros_gerais(unidade,curso,etapa,turma,co_materia,"default_lbg",outro)
...
for i=1 to max
	...
	va_sim=request.form("val_bat_"&i)
	...
	if va_sim="" or isnull(va_sim) then
		...
		if isnumeric(va_sim) then					
			va_sim=va_sim*1
			val_default_lbg = val_default_lbg*1
				if int(va_sim) <> va_sim then
					fail = 1 
					erro = "int"
					url = nu_matricula&"_"&va_sim&"_"&erro
					grava = "no"
				elseif va_sim<0 or va_sim>val_default_lbg then
					fail = 1 
					erro = "out"
					url = nu_matricula&"_"&va_sim&"_"&erro
					grava = "no"					
...
mat_princ=busca_materia_mae(co_materia)	
		Set RS0 = Server.CreateObject("ADODB.Recordset")
		CONEXAO0 = "Select * from "&tb&" WHERE CO_Matricula = "& nu_matricula &" AND CO_Materia_Principal = '"& mat_princ &"' AND CO_Materia = '"& co_materia &"' AND NU_Periodo="&periodo	
		response.Write(CONEXAO0)	
	...
	If RS0.EOF THEN	
		...
		RS.addnew
			...
			RS("CO_Materia_Principal")=mat_princ
		...
		Set RS0 = Server.CreateObject("ADODB.Recordset")
		CONEXAO0 = "delete from "&tb&" WHERE CO_Matricula = "& nu_matricula &" AND CO_Materia_Principal = '"& mat_princ &"' AND CO_Materia = '"& co_materia &"' AND NU_Periodo="&periodo	
		Set RS0 = CON.Execute(CONEXAO0)
