...
'response.Write("MREC "&media_rec&"-"&tipo_calculo&"-"&dados_aluno(0)&"<BR><BR>")						
					if tipo_calculo="anual" then
						...
						tipo_media = "MF"
						modifica_result = Verifica_Conselho_Classe(vetor_aluno, co_materia(c), tipo_media, outro)
						if modifica_result <> "N" then
...
'end if			
							...
							if modifica_result <> "N" then
								resultado = Calc_Med_An_Fin(unidade, curso, co_etapa, turma, vetor_aluno, co_materia(c), caminho_nota, tb_nota, qtd_periodos, periodo_m2, periodo_m3,"anual", outro)
								vetor_resultado = split(resultado, "#!#")
