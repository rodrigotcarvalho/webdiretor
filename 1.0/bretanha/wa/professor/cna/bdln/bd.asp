...
if nota= "TB_NOTA_S" then	
	atualiza_TB = "TB_Da_Aula_Subs"
	atualiza_CP = "CO_Materia"	
else
	atualiza_TB = "TB_Da_Aula"
	atualiza_CP = "CO_Materia_principal"
end if
...
if opt="blq" or opt="dblq" then
	...
	sql_atualiza= "UPDATE "&atualiza_TB&" SET "&situcao_periodo&" = '"&st&"' WHERE CO_Professor="& co_prof &" AND "&atualiza_CP&"='"& co_materia &"' AND NU_Unidade="& unidade &" AND CO_Curso='"& curso &"' AND CO_Etapa='"& etapa &"' AND CO_Turma='"& turma &"' AND TP_Nota='"& nota &"'"
	response.Write(sql_atualiza)
	Set RS2 = CON2.Execute(sql_atualiza)
else
		Set RS2 = Server.CreateObject("ADODB.Recordset")
		SQL2 = "SELECT * FROM "&atualiza_TB&" where NU_Unidade="& unidade &"AND CO_Curso='"& curso &"'AND CO_Etapa='"& etapa &"'AND CO_Turma='"& turma &"'"
		RS2.Open SQL2, CON2
