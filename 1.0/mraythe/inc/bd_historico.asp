...
Function totaisHistorico (codCons,tipoHistorico,tipoInformacao)
	...
	for e=0 to ubound(vetorEtapas)
		...
		Set RSA = Server.CreateObject("ADODB.Recordset")
		SQLA = "SELECT NU_Carga_Horaria_Total, NU_Dias_Letivo, TX_Frequencia_Total FROM TB_Historico_Ano WHERE CO_Matricula ="&codCons&" AND TP_Curso = '"&tipoHistorico&"' and CO_Seg = '"&vetorEtapas(e)&"' order by DA_Ano"	
		RSA.Open SQLA, CON7	
		...
		if RSA.EOF then
			...
			elseif tipoInformacao = "N" then
				wrkInfHistorico = RSA("NU_Dias_Letivo")			
			else
