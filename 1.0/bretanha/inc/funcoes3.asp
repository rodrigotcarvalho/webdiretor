...
qtd_alunos=contalunos(CAMINHO_al,ano_letivo,unidade,curso,etapa,turma,"C")
	...
	if opcao<>"S" then
		...
		if RS.EOF then
			Set RS = Server.CreateObject("ADODB.Recordset")
			CONEXAO = "Select * from TB_Da_Aula_Subs WHERE NU_Unidade = "& unidade &" AND CO_Curso = '"& curso &"' AND CO_Etapa = '"& etapa &"' AND CO_Turma = '"& turma &"' AND  CO_Materia = '"& co_materia &"'"
			Set RS = CONg.Execute(CONEXAO)		
			
			mat_princ=RS("CO_Materia_Principal")
			
			Set RS5a = Server.CreateObject("ADODB.Recordset")
			SQL5a = "SELECT * FROM TB_Programa_Subs where CO_Etapa ='"& etapa &"' AND CO_Curso ='"& curso &"' AND CO_Materia_Principal ='"& mat_princ &"' AND CO_Materia_Filha = '"&co_materia&"'"
			RS5a.Open SQL5a, CON_0
			...
			dados_modelo_S = in_faltas&"#!#"&in_bonus&"#!#"&rec_semestral			
		end if		
...
dados_tabela=verifica_dados_tabela(CAMINHOn,opcao,dados_modelo_S)
...
elseif errou="pt" or errou="pp" or errou="pp1" or errou="pp2" Then
 ...
 <% for i= 0 to ubound(linha_pesos)
		...
		if linha_pesos(i)="PESO" then
			...
			if ano_letivo>2011 then
				if nome_pesos_variaveis(i) = "PE_Teste" then
					valor_peso=1				
				elseif nome_pesos_variaveis(i) = "PE_Prova" then
					valor_peso=2
				end if
...
nu_chamada=nu_chamada*1
	...
	elseif nu_chamada = errante then
				...
				coluna=0	 
				 for n= 0 to ubound(nome_variaveis)
					...
					if RS3.EOF then 
						...
						nu_matricula=nu_matricula*1		
						nu_chamada=nu_chamada*1															
						if valido="n" and nu_chamada = errante then
...
Function bonus_e_simulados (unidade,curso,etapa,turma,co_materia,periodo,areaconhecimento,ano_letivo,co_usr,opcao,subopcao,outro)									
chave = session("chave")
session("chave")=chave
split_chave=split(chave,"-")
sistema_origem=split_chave(0)
...
elseif sistema_origem="WA" then	
	...
	if (UCASE(tb_nota)=UCASE("TB_Nota_F") or UCASE(tb_nota)=UCASE("TB_Nota_M" ) or UCASE(tb_nota)=UCASE("TB_Nota_K" ) )  then
...
if (UCASE(tb_nota)=UCASE("TB_Nota_A") or UCASE(tb_nota)=UCASE("TB_Nota_S" ) )  then
	...
	onchange="""validaDefault('$$$$','!!!!');"""
		...
		<tr class="<%response.Write(classe_anterior)%>" id="<%response.Write("celula"&nu_chamada)%>">
		  ...
		  <% 		valor=""
					Set RS3 = Server.CreateObject("ADODB.Recordset")
					if opcao = "LSM" then
						SQL_N = "Select * from "& tb &" WHERE CO_Matricula = "& nu_matricula & " AND TP_Area = "&areadoconhecimento&" AND NU_Periodo="&periodo				
					elseif opcao = "LBS" or opcao = "LBG" then
			...
			<table width="100%" border="0" cellspacing="0">
			  ...
			  <tr>
				...
				<td width="33%"><div align="center"><font size="3" face="Courier New, Courier, mono">
					...
					<input name="co_materia" type="hidden" id="co_materia" value="<%= co_materia%>">
					<input name="periodo" type="hidden" id="periodo" value="<%= periodo%>">
					<input name="areaconhecimento" type="hidden" id="periodo" value="<%= areaconhecimento%>">
					<input name="co_prof" type="hidden" id="co_prof" value="<% = co_prof%>">
					<input name="max" type="hidden" id="max" value="<% =max%>">
					<input name="co_usr" type="hidden" id="co_usr" value="<% = co_usr%>">
					<input name="ano_letivo" type="hidden" id="ano_letivo" value="<% = ano_letivo%>">
