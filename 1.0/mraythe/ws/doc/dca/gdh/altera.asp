...
for i=0 to ubound(conjunto_dados)	
	...
	caminho_nota=caminho_notas(CONG, tb_nota, outro)	
...
'				end if			
			...
			SQL1 = "SELECT SUM(NU_Faltas) as total_faltas  FROM "&tb_nota&" where CO_Matricula ="& co_matricula &" And NU_Periodo<4"
			RS1.Open SQL1, CON_N
			...
			if RS1.EOF then
				...
				if isnull(total_faltas) or total_faltas="" then				
					total_faltas=0			
				end if	
			end if	
			
			Set RSC = Server.CreateObject("ADODB.Recordset")
			SQLC = "SELECT sum(NU_Aulas) AS carga_materia FROM TB_Programa_Aula where CO_Etapa ='"& co_etapa_t &"' AND CO_Curso ='"& curso_t &"' and IN_MAE=TRUE"
			RSC.Open SQLC, CON0
			
			if RSC.EOF then
				horas_aula=NULL
			else
				horas_aula= RSC("carga_materia")		
			end if				
	
			total_faltas=total_faltas*1
			if horas_aula="" or isnull(horas_aula) then
				frequencia_geral=100
			elseif horas_aula=0 then
				frequencia_geral=100			
			else	
				horas_aula=horas_aula*1
				frequencia_geral=((horas_aula-total_faltas)/horas_aula)*100
			end if	
			
			if frequencia_geral<100 then
				frequencia_geral=arredonda(frequencia_geral,"mat_dez",1,0)	
			end if				
			d.writeLine ano_letivo&";"&co_matricula&";FREQUENCIA;"&frequencia_geral&";;;;"			
		NEXT
	End if	
NEXT
