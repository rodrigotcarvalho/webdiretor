...
elseif sistema_origem="WA" then	
	if funcao_origem="EPN" then
		...
		'Só usado no caso da tabela V que utiliza também a F (2013) e a K(>2013)
		'--------------------------------------------------------------------
...
calc=calc*1
	...
	%>
			<tr>
					...
					<td width="340" class="<%response.Write(classe)%>">Matrícula <%response.Write(nu_matricula)%> cadastrada em TB_Matriculas sem correspondência em TB_Alunos</td>               
