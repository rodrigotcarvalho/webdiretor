<%
Function Banner_novo(nivel,this_file,sistema_local,nome,permissao,ano_letivo)
  ...
  <tr> 
    <td colspan="4"><table width="100%" height="14" border="0" cellspacing="0">
          <tr class="menu_banner"> 
			...
			<select name="ano_letivo" class="select_style" onChange="location.href='<%response.write(parent_folder)%>inc/redireciona.asp?opt=al&ano_letivo=' + this.options[this.selectedIndex].value;">
                <%
		...
		WEND 		
            ...
            <td width="15%" height="10"> <div align="right">Sistemas 
                Autorizados </div></td>
            <td width="15%" height="10">  
			<select name="sistema" id="sistema" class="select_style" onChange="location.href='<%response.write(parent_folder)%>inc/redireciona.asp?opt=sa&sistema=' + this.options[this.selectedIndex].value;">
				<%
				nivel=nivel*1
				if nivel=0 then%>
...
elseif co_sistema="WN" then
	...
	if RS_p.EOF then
		...
		%>
                <option value="<%=info%>" selected><%=sistema%></option>
		<%else%>				
                <option value="<%=info%>"><%=sistema%></option>				
        <%end if				
	RS.MOVENEXT
	end if
else		
