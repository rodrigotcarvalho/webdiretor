...
Param_Logo_Gde("ScaleY") = 0.6
		   Page.Canvas.DrawImage Logo, Param_Logo_Gde
			...
			if curso=2 then
			SQLC = "SELECT * FROM TB_Cabecalhos WHERE CO_Documento=6"
			else
			SQLC = "SELECT * FROM TB_Cabecalhos WHERE CO_Documento=5"
			end if
			RSC.Open SQLC, CON0			
			...
			Text = "<center><i><b><font style=""font-size:17pt;"">"&documento&"</font></b></i></center>"
			...
			Page.Canvas.SetParams "LineCap=0" 
			altura_segundo_separador= Page.Height - altura_logo_gde -margem -40
			With Page.Canvas
			...
			End With
			Table(1, 1).ColSpan = 3			
			Table(1, 1).AddText "Aos 23 de dezembro de "&ano_letivo&" terminou-se o processo de apura&ccedil;&atilde;o de notas finais e nota global dos alunos deste estabelecimento com os seguintes resultados:", "size=8;html=true", Font	
			...
			IF RS5.EOF Then
				...
				while not RS5.EOF
					...
					if co_materia_check=1 then
						vtr_materia=co_mat_fil
						vetor_materia_rel=co_materia						
					else
						vtr_materia=vtr_materia&"#!#"&co_mat_fil
...
'response.Write(vtr_materia)
'response.end()
			
			co_materia_exibe=Split(vetor_materia_rel,"#!#")		
			co_materia_verifica=Split(vtr_materia,"#!#")							
			colunas_de_notas=18
			total_de_colunas=20					
			altura_medias=25
			...
			Notas_Tit(1, 1).AddText "<div align=""center""><b>Id Aluno</b></div>", "size=8;indenty=6; html=true", Font 			
			...
			resultados=Calc_Med_An_Fin(unidade, curso, co_etapa, turma, vetor_matriculas, vtr_materia, caminho_nota, tb_nota, 5, 4, 5, "final", 0)		
...
'				Text = "E, para constar, eu ____________________________________________________________________________ Secret&aacute;rio(a), lavrei a presente relação que vai assinada pelo(a) Diretor(a) do Estabelecimento" 			
'				
'				Do While Len(Text) > 0
'					CharsPrinted = Page.Canvas.DrawText(Text, Param, Font )
'				 
...
'				Loop 
				...
				Relatorio = "SWD058 - Sistema Web Diretor"
