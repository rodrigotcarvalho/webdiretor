...
if controle= "D" then
	...
	consulta_prim = "select * from TB_Usuario where CO_Usuario = " & lg 
	RS1.Open consulta_prim, conexao_ctl
...
case 14
	...
	mail_prim=session("mail_prim")
case 15
	msg="É obrigatório o preenchimento do e-mail!"
	tipo="e"
	lg = session("lg")
	pas= session("senha")
	pas1= session("pas1")
...
<!--
body {
	margin:0px; background-image: url('img/grade-fundo.gif');
...
pas2 = document.login.pas2.value;
 ...
 if( (document.login.email.length==0  && document.login.tp_usuario.value=='R') || ((document.login.email.length > 0) && (document.login.email.value.indexOf('@')==-1 || document.login.email.value.indexOf('.')==-1)))
	{
...
senha = document.login.senha_original.value;
if(pas1 == senha){
...
<table width="1000" border="0" align="center" cellpadding="0" cellspacing="0">
  ...
  <tr> 
    ...
    <td width="379"><input name="log" type="hidden" id="log" value="prim" />
	              <input name="codigo_seguranca" type="hidden" id="codigo_seguranca" value="<%response.Write(codigo_seguranca)%>">
	              <input name="tp_usuario" type="hidden" id="tp_usuario" value="<%response.write(tp_usuario)%>"></td>
  </tr>
  <tr>
    ...
    <td height="100" colspan="3" valign="top"><table width="100%" border="0" cellspacing="0" cellpadding="0">
      ...
      <tr>
        ...
        <td width="63%" height="20"><input name="senha_original" type="password" class="textbox" id="senha_original" value="<%RESPONSE.Write(pas)%>" size="25" onKeyUp="GuardaSenha(this.value)"></td>
        </tr>
      <tr>
  ...
  <tr> 
    <td height="22" align="right" valign="top"><input name="autorizo" type="checkbox" value="ok" checked /></td>
    <td align="right" valign="top"><!--DWLayoutEmptyCell-->&nbsp;</td>
    <td colspan="3" rowspan="2" valign="top"><textarea name="concordo" cols="27" rows="4" readonly="readonly" id="concordo">Autorizo o Colégio Saint John a enviar informações através do Web Família para o e-mail cadastrado.</textarea></td>
    <td></td>
  </tr>
  <tr> 
    <td height="21">&nbsp;</td>
