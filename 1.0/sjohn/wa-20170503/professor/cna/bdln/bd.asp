...
if periodo=1 then
	situacao_periodo="ST_Per_1"
elseif periodo=2 then
	situacao_periodo="ST_Per_2"
elseif periodo =3 then
	situacao_periodo="ST_Per_3"
elseif periodo =4 then
	situacao_periodo="ST_Per_4"
elseif periodo =5 then
	situacao_periodo="ST_Per_5"
elseif periodo =6 then
	situacao_periodo="ST_Per_6"
end if

if opt="blq" or opt="dblq" then
	Set RS_b = Server.CreateObject("ADODB.Recordset")
	sql_atualiza= "UPDATE TB_Da_Aula SET "&situacao_periodo&" = '"&st&"' WHERE CO_Professor="& co_prof &" AND CO_Materia_principal='"& co_materia &"' AND NU_Unidade="& unidade &" AND CO_Curso='"& curso &"' AND CO_Etapa='"& etapa &"' AND CO_Turma='"& turma &"' AND TP_Nota='"& nota &"'"
