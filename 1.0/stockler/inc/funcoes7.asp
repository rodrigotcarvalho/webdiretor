...
'	nome_periodo=split(vetor_nom_periodos,"#!#")
	...
	vetor_alunos=alunos_turma(ano_letivo,unidade,curso,co_etapa,turma,"nome")
'	response.Write(vetor_alunos)
	'response.End()
	n_alunos= split(vetor_alunos,"#$#")			

		for al=0 to ubound(n_alunos)
			aluno= split(n_alunos(al),"#!#")
			cod_cons=aluno(0)
			if cod_cons <> "nulo" then
	'			response.Write(vetor_num_periodo)
	'			response.end()
	'			response.Write(unidade&","&curso&","&co_etapa&","&turma&","&vetor_num_periodo&","&cod_cons&","& vetor_materia&","&caminho_nota&"<BR>")
		...
		if qto_falta="s" then
			...
			for v=0 to ubound(vetor_periodo)
				...
				elseif vetor_periodo(v)="TRI2" then	
					temp_num_periodo=temp_num_periodo&"#!#2#!#2#!#0"
					periodo_exibe=periodo_exibe&"#!#"&vetor_periodo(v)&"#!#MC#!#QF1"
			...
			for v=0 to ubound(vetor_periodo)
				if vetor_periodo(v)="TRI1" then	
					...
					periodo_exibe=vetor_periodo(v)
				elseif vetor_periodo(v)="TRI2" then	
					temp_num_periodo=temp_num_periodo&"#!#2#!#2"
					periodo_exibe=periodo_exibe&"#!#"&vetor_periodo(v)&"#!#MC"
				elseif vetor_periodo(v)="TRI3" then	
...
'	end if
	...
	if (mae=TRUE and fil=FALSE and in_co=FALSE and isnull(peso)) then
		if no_periodo<>"QF1" and no_periodo<>"QF2" and no_periodo<>"MA" and no_periodo<>"MS"and no_periodo<>"RECP" and no_periodo<>"BIM1*"and no_periodo<>"BIM2*" and no_periodo<>"MF" then	
			if no_periodo="MC" then
				tipo_campo = "nota_acumulada"			
			else
				tipo_campo = "nota_periodo"
			end if	
			media=Calcula_Media_T_F_F_N(unidade, curso, co_etapa, turma, cod_aluno, codigo_materia, caminho_nota, tb_nota, periodo,tipo_campo)	
'	response.Write(media&" - codigo_materia="&codigo_materia&"<BR>")	
			...
			acumula_m=0
'			if no_periodo="MC" then	
'				result=Calcula_Media_T_F_F_N(unidade, curso, co_etapa, turma, cod_aluno, codigo_materia, caminho_nota, tb_nota, 2,"nota_periodo")					
'
'				if result="&nbsp;" or result="" or isnull(result) then	
'					result=""
'				else				
'					result=arredonda(result/10,"mat_dez",1,0)
'				end if		
'				media=result												
'			else
			if no_periodo="QF1" then
