...
response.Write(no_curso)%>
	...
	<%
		...
		if RSTB.EOF then
			Set RSTB = Server.CreateObject("ADODB.Recordset")
			CONEXAO = "Select * from TB_Da_Aula_Subs WHERE CO_Professor= "& co_prof &"AND NU_Unidade = "& unidade &" AND CO_Curso = '"& curso &"' AND CO_Etapa = '"& co_etapa &"' AND CO_Turma = '"& turma &"' AND CO_Materia = '"& co_materia &"'"
			Set RSTB = CON2.Execute(CONEXAO)		
			if RSTB.EOF then
