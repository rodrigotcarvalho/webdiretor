...
from=email_financeiro
	...
	SQL4B = "SELECT * FROM TB_Posicao WHERE CO_Matricula_Escola ="& e_cod &" and SQ_Bloqueto = "&P_NOSSO_NUMERO	
	...
	for e = 0 to ubound(dados_form)
		...
		if InStr(end_email,"@")=0 then
			...
			if eh_segunda_via = "N" then
				assunto = Replace(assunto,"2ª Via de Boleto","Boleto")
			end if
			...
			if eh_segunda_via = "S" then
				mensagem=mensagem&"Segue anexo a 2&ordf; Via do Boleto banc&aacute;rio"
			else
				mensagem=mensagem&"Segue anexo o Boleto banc&aacute;rio"
			end if
			
			if nome_lanc = "Mensalidade" then
				mensagem=mensagem&" da parcela referente à mensalidade" 
			else
				mensagem=mensagem&" da parcela referente ao servi&ccedil;o "&nome_lanc
			end if
			
			mensagem=mensagem&" do m&ecirc;s de "&desc_meses&" do ano letivo de "&ano_letivo&Chr(13)&Chr(13)	 		
