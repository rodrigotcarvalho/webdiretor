...
for i=0 to ubound(conjunto_dados)	
	...
	'CABEÇALHO==========================================================================================		
		   ...
		   Page.Canvas.DrawImage Logo, Param_Logo_Gde
			...
			Text = "<center><i><b><font style=""font-size:18pt;"">MAP&Atilde;O DE M&Eacute;DIAS FINAIS</font></b></i></center>"
...
'response.end()
			...
			linha=2
		'	resultados=Calc_Med_An_Fin(unidade, curso, co_etapa, turma, vetor_matriculas, vetor_materia, caminho_nota, tb_nota, 6, 5, 6, "recuperacao", 0)	
...
'response.Write(alunos_encontrados(a)&"<BR>")
'response.Write(resultados_apurados(a)&"<BR>")
				...
				for n=0 to ubound(co_materia_exibe)	
					coluna=coluna+1	
					resultados = Calc_Prim_Media (unidade, curso, co_etapa, turma, dados_alunos(0), co_materia_exibe(n), caminho_nota, tb_nota,prd_prim_media, tipo_calculo, outro)		
					resultados_verifica = split(resultados, "#!#" )
					if	resultados_verifica(1)= "APR" or resultados_verifica(1)= "Apr" or resultados_verifica(1)= "&nbsp;"	then					
						resultados="&nbsp;#!#&nbsp;"
					else	
						resultados_2 = Calc_Seg_Media (unidade, curso, co_etapa, turma, dados_alunos(0), co_materia_exibe(n), caminho_nota, tb_nota, prd_seg_media, tipo_calculo, outro)
					
						resultados_verifica_2 = split(resultados_2, "#!#" )
						if	resultados_verifica_2(1)= "APR" or resultados_verifica_2(1)= "Apr" or resultados_verifica_2(1)= "&nbsp;"	then					
							resultados="&nbsp;#!#&nbsp;"
						else	
							resultados = Calc_Ter_Media (unidade, curso, co_etapa, turma, dados_alunos(0), co_materia_exibe(n), caminho_nota, tb_nota, prd_ter_media, "sem_calculo", outro)	
			...
			limite=limite+1
			   LastRow = Page.Canvas.DrawTable( Notas_Tit, param_table2 )
				...
				if LastRow >= Notas_Tit.Rows.Count Then 
					...
					Relatorio = "SWD105 - Sistema Web Diretor"
