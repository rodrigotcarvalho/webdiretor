...
elseif opt="inc" then
	...
	for q=1 to itens_criados
		...
		if isnull(item_fornecedor_n) or item_fornecedor_n="" then
		    ...
		    if item_fornecedor_n <> "nulo" then
			   ...
			   if isnull(quantidade_n) or quantidade_n="" then
				  quantidade_n = 0
			   end if			
				...
				if RSC.EOF then
				   ...
				   qtd_atual = RSC("QT_Atual")	   
				   val_atual = RSC("VA_Medio_Refer")	 				   
				   if isnull(qtd_atual) or qtd_atual="" then
					  qtd_atual = 0
				   end if
				   
				   if isnull(val_atual) or val_atual="" then
					  val_atual = 0
				   end if				   
		...
		val_total = val_total+(quantidade_n*val_atual)
	next	
	...
	if cadastra="S" then
		...
		RS.addnew
		  ...
		  RS.update
		  If Err.number<>0 then
		    response.Write(nu_pedido&" "&data_inclui&" "&projeto&" "&unidade&" "&curso&" "&etapa&" "&turma&" "&val_total&" "&obs&" "&solicitado)
			Response.Write "ERRO: " & Err.Description & "<BR>"
			response.End()
		  end if		  
...
'RESPONSE.Write(">"&situacao_pedido)
	...
	'response.Write(qtd_estoque &"="& qtd_restabelecido&"'<BR>")			   
