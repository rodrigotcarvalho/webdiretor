...
if mae=TRUE AND fil=false AND in_co=true AND isnull(nu_peso) then
		...
		elseif mae=false AND fil =false AND in_co=True then
			...
			if RSnFIL.EOF then
				...
				if isnull(m1) or m1="" then
					nota_princ1=nota_princ1
					div_princ1 = div_princ1
				else
					nota_princ1=nota_princ1 +(m1*nu_peso) 	
					div_princ1 = div_princ1+nu_peso								
					contanotas=contanotas+1
				end if	
				ACU_Peso1=ACU_Peso1+nu_peso2				
			end if									
	...
	wend
		div_princ1=div_princ1*1
		if div_princ1=0 then
			media1="&nbsp;"
		else
			if contanotas=0 then
				media1="&nbsp;"			
			else
				media1=nota_princ1/div_princ1
				'response.Write("'"&media1&"="&nota_princ1&"/"&div_princ1&"'")
				media1=arredonda(media1,"mat_dez",1,0)
			end if
		end if
