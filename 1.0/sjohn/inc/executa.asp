...
RESPONSE.Write("SELECT DISTINCT CO_Materia_Principal FROM TB_Da_Aula where CO_Professor ="& prof_altera&" and CO_Etapa = '"&session("e_pub") &"' AND NU_Unidade = "&session("u_pub")&" and CO_Curso = '"&session("c_pub") &"' order by CO_Materia_Principal")
...
session("t_pub")=session("t_pub")
		...
		Set RSG = Server.CreateObject("ADODB.Recordset")
		SQLG = "SELECT DISTINCT CO_Materia FROM TB_Programa_Aula where IN_MAE = True And CO_Etapa = '"&session("e_pub") &"' AND CO_Curso = '"&session("c_pub") &"' order by CO_Materia"
		RSG.Open SQLG, CON0
...
ELSE
%>
                      <select name="mat_prin" class="select_style" onChange="GuardaDisciplina(this.value);">
                        <option value="999999" selected></option>
                        <%
while not RSG.EOF
...
<select name="areaconhecimento" class="select_style" id="areaconhecimento">
    ...
    <%
		...
		Set RS3 = Server.CreateObject("ADODB.Recordset")	
		SQL3 = "SELECT Distinct (TB_Area_ConhecimentoxTB_Programa_Aula.TP_Area) as TP_Area, TB_Area_Conhecimento.TX_Descricao as TX_Descricao, TB_Area_Conhecimento.NU_Prioridade_Combo FROM TB_Area_ConhecimentoxTB_Programa_Aula INNER JOIN TB_Area_Conhecimento ON TB_Area_ConhecimentoxTB_Programa_Aula.TP_Area = TB_Area_Conhecimento.TP_Area WHERE (((TB_Area_ConhecimentoxTB_Programa_Aula.CO_Curso)='"&session("c_pub")&"') AND ((TB_Area_ConhecimentoxTB_Programa_Aula.CO_Etapa)='" & session("e_pub") & "')) order by TB_Area_Conhecimento.NU_Prioridade_Combo;"
		RS3.Open SQL3, CON0	
...
%>
<option value="<%=co_ac%>"> 
    <%response.Write(Server.URLEncode(ds_ac))%>
</option>
    <%
RS3.MOVENEXT
WEND
