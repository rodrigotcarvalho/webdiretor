...
<%elseif nv="5" then
unidade_altera5=Request.Form("u_pub5")
if unidade_altera5="" or isnull(unidade_altera5) then
session("u_pub5")=session("u_pub5")
else
session("u_pub5")=unidade_altera5
session("c_pub5")=session("c_pub5")
session("e_pub5")=session("e_pub5")
session("t_pub5")=session("t_pub5")
end if
%>
                      <select name="curso5" class="borda" onchange="recuperarEtapa5(this.value)">                      
<%end if%>					  
...
elseif nv="5" then
		SQL0 = "SELECT * FROM TB_Unidade_Possui_Etapas where NU_Unidade="&session("u_pub5")		
end if		
		RS0.Open SQL0, CON0
...
<%elseif nv="5" then
curso_altera5=Request.Form("c_pub5")
if curso_altera5="" or isnull(curso_altera5) then
session("c_pub5")=session("c_pub5")
else
session("c_pub5")=curso_altera5
session("e_pub5")=session("e_pub5")
session("t_pub5")=session("t_pub5")
end if
%>
                      <select name="etapa5" class="borda" onchange="recuperarTurma5(this.value)">                      
<%end if%>
...
elseif nv="5" then
		SQL0b = "SELECT * FROM TB_Unidade_Possui_Etapas where NU_Unidade="&session("u_pub5")&"AND CO_Curso='"&session("c_pub5")&"'"		
end if			
...
elseif nv="5" then
		SQL0c = "SELECT * FROM TB_Etapa where CO_Curso='"&session("c_pub5")&"' AND CO_Etapa='"&CO_Etapa&"'"		
end if			
...
<%elseif nv="5" then
etapa_altera5=Request.Form("e_pub5")
if etapa_altera5="" or isnull(etapa_altera5) then
session("c_pub5")=session("c_pub5")
else
session("e_pub5")=etapa_altera5
session("t_pub5")=session("t_pub5")
end if
%>
						<select name="turma4" class="borda">                        
...
elseif nv="5" then
		SQL3 = "SELECT * FROM TB_Turma where NU_Unidade="&session("u_pub5")&"AND CO_Curso='"&session("c_pub5")&"' AND CO_Etapa='" & session("e_pub5") & "' order by CO_Turma" 		
end if		
		RS3.Open SQL3, CON0						
