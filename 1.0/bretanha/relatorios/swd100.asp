...
'response.Write(aln&" 3 "&nome_aluno&"<BR>")				
		...
		largura_Foto=formatnumber(Foto.Width*redutor_foto,0)
		altura_Foto=formatnumber(Foto.Height*redutor_foto,0)
		if qtd_fotos=1 then
			y_foto = y_foto-altura_Foto-10	
		end if
		Param_Foto("x") = x_foto
		Param_Foto("y") = y_foto
		Param_Foto("ScaleX") = redutor_foto
		Param_Foto("ScaleY") = redutor_foto
		...
		if qtd_fotos mod 7 = 0 then
			y_foto = y_foto-altura_Foto-margem	
			x_foto = margem		
			linhas = linhas+1					
		else						
			x_foto = x_foto+largura_Foto+espacador_foto
		end if		
		if linhas>5 then
