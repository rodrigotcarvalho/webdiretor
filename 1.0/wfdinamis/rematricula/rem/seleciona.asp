...
if tipo = "contrato" then
	wrk_modelo = vetor_modeloContrato(opt-1)
else
	wrk_modelo = vetor_modeloAdendo(0)
end if	
...
Após sua confirmação a empresa certificadora e a escola farão todo o controle das assinaturas e enviarão a seguir o <%response.write(tipo)%> na integra para o email <%response.Write(emailRespFin)%> para assinatura de <%response.Write(nomeRespFin)%>.<br>&nbsp;<br>

Ao receber o email Clique no link visualizar para assinar, leia o <%response.write(tipo)%> e ao final clique no botão assinar.<br>&nbsp;<br>

<strong>Confirma a assinatura do <%response.write(tipo)%>&nbsp;<%=opt%>?</strong>

</td>
                  </tr> 
				   <tr>
                   	<td height="100" colspan="5" align="center" class="style1">
					<%tipo = UCase(Left(tipo, 1)) &  Mid(tipo, 2)%>
                    <input name="assinar1" type="submit" class="bt_contrato" id="assinar1" value="Assinar <%response.write(tipo)%>" onClick="MM_goToURL('parent','../../relatorios/contratoAdendoPDF.asp?c=<%=cod%>&amp;modelo=<%response.Write(wrk_modelo)%>&amp;t=<%response.write(wrk_mod)%>&amp;dr=R&amp;v=<%response.Write(opt)%>');return document.MM_returnValue">
                    </td>
              </tr>                             
                  ...
                  <td width="100%" valign="top">
                      <table width="100%" border="0" cellpadding="0" cellspacing="0">
                          ...
                          <tr>
                            ...
                            <ul><li><a href="../../relatorios/contratoAdendoPDF.asp?c=<%=cod%>&amp;modelo=<%response.Write(vetor_modeloAdendo(aden))%>&amp;t=A&amp;dr=R"><%response.Write(vetor_modeloAdendo(aden))%></a></li></ul>
                            <%next%>
