	...
	<%for notas=0 to ubound(colunas_notas_periodo)			
		...
		if colunas_notas_calc(notas)= "BDM" or colunas_notas_calc(notas)= "BDR"or colunas_notas_calc(notas)= "RF" or colunas_notas_calc(notas)= "BDF" then
			if tp_materia="T_F_F_N" or tp_materia="F_T_F_N"	 or tp_materia="F_F_T_N" then
				...
				media=Calcula_Media_T_F_F_N(unidade, curso, co_etapa, turma, tp_modelo, tp_freq, wrk_cod_cons, codigo_materia_pr, wrk_co_materia, CONn, tb_nota, colunas_notas_periodo(notas), var_bd, outro)			
				
			elseif tp_materia="T_T_F_N" then
			
				vetor_materia=busca_materias_filhas(wrk_co_materia)
				media=Calcula_Media_T_T_F_N(unidade, curso, co_etapa, turma, tp_modelo, tp_freq, wrk_cod_cons, wrk_co_materia, vetor_materia, CONn, tb_nota, colunas_notas_periodo(notas), var_bd, outro)		
					
			elseif tp_materia="T_F_T_N" then
				vetor_materia=busca_materias_filhas(wrk_co_materia)						
				media=Calcula_Media_T_F_T_N(unidade, curso, co_etapa, turma, tp_modelo, tp_freq, wrk_cod_cons, wrk_co_materia, vetor_materia, CONn, tb_nota, colunas_notas_periodo(notas), var_bd, outro)	
			...
			elseif colunas_notas_calc(notas)= "MA" then
				...
				primeira_media=Calc_Prim_Media (unidade, curso, co_etapa, turma, wrk_cod_cons, wrk_co_materia, caminho_nota, tb_nota, prd_prim_media, tipo_calculo, outro)	
			...
			elseif colunas_notas_calc(notas)= "MF" then
				prd_seg_media=Periodo_Media(tipo_modelo,"REC",outro)				
				segunda_media=Calc_Seg_Media (unidade, curso, co_etapa, turma, wrk_cod_cons, wrk_co_materia, caminho_nota, tb_nota, prd_seg_media, tipo_calculo, outro)
			...
			elseif colunas_notas_calc(notas)= "PF"	then
				prd_ter_media=Periodo_Media(tipo_modelo,"MF",outro)
				terceira_media=Calc_Ter_Media (unidade, curso, co_etapa, turma, wrk_cod_cons,  wrk_co_materia, caminho_nota, tb_nota, prd_ter_media, "sem_calculo", "ficha")
			...
			if teste=false then
				...
				if mostra_nota="s" then
					...
					if colunas_notas_calc(notas)<> "BDF" then
						arred = parametros_gerais(unidade, curso, co_etapa, turma, codigo_materia_pr,"decimais_media",0)					
						media=formatnumber(media,arred)
					end if
					if media>=ntvml then	
						response.Write(media)
					else	
	...
	<%for notas=0 to ubound(colunas_notas_periodo)
		...
		if colunas_notas_calc(notas)= "BDM" or colunas_notas_calc(notas)= "BDR"or colunas_notas_calc(notas)= "RF" or colunas_notas_calc(notas)= "BDF" then
			if tp_materia="T_F_F_N" or tp_materia="F_T_F_N"	 or tp_materia="F_F_T_N" then					
				...
				media=Calcula_Media_T_F_F_N(unidade, curso, co_etapa, turma, tp_modelo, tp_freq, cod_cons, codigo_materia_pr, co_materia(a), CONn, tb_nota, colunas_notas_periodo(notas), var_bd, outro)			
				
			elseif tp_materia="T_T_F_N" then
			
				vetor_materia=busca_materias_filhas(co_materia(a))
				media=Calcula_Media_T_T_F_N(unidade, curso, co_etapa, turma, tp_modelo, tp_freq, cod_cons, co_materia(a), vetor_materia, CONn, tb_nota, colunas_notas_periodo(notas), var_bd, outro)		
					
			elseif tp_materia="T_F_T_N" then
				vetor_materia=busca_materias_filhas(co_materia(a))						
				media=Calcula_Media_T_F_T_N(unidade, curso, co_etapa, turma, tp_modelo, tp_freq, cod_cons, co_materia(a), vetor_materia, CONn, tb_nota, colunas_notas_periodo(notas), var_bd, outro)	
			...
			elseif colunas_notas_calc(notas)= "MA" then
				prd_prim_media=Periodo_Media(tipo_modelo,"MA",outro)
				primeira_media=Calc_Prim_Media (unidade, curso, co_etapa, turma, cod_cons, co_materia(a), caminho_nota, tb_nota, prd_prim_media, tipo_calculo, outro)	
				inf_primeira_media=split(primeira_media,"#!#")
				media=inf_primeira_media(0)
				resultado=inf_primeira_media(1)						
				periodo_autoriza=prd_prim_media
				periodo_res=prd_prim_media				
				
			elseif colunas_notas_calc(notas)= "MF" then
				prd_seg_media=Periodo_Media(tipo_modelo,"REC",outro)
							
				segunda_media=Calc_Seg_Media (unidade, curso, co_etapa, turma, cod_cons, co_materia(a), caminho_nota, tb_nota, prd_seg_media, tipo_calculo, outro)
				inf_segunda_media=split(segunda_media,"#!#")
				media=inf_segunda_media(0)
				resultado=inf_segunda_media(1)
			...
			elseif colunas_notas_calc(notas)= "PF"	then
				prd_ter_media=Periodo_Media(tipo_modelo,"MF",outro)
				terceira_media=Calc_Ter_Media (unidade, curso, co_etapa, turma, cod_cons,  co_materia(a), caminho_nota, tb_nota, prd_ter_media, "sem_calculo", "ficha")
