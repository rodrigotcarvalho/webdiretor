...
if mat_princ="" or isnull(mat_princ) then
	...
	'consulta1 = "select * from Email where CO_Escola=7"
...
'response.write(email_suporte_escola&"; "&mail&"; "&mail_CC)
'response.end()
 indEnviaEmail = enviaEmail("Suporte Web Diretor", email_suporte_escola, "Coordenador", mail, email_suporte_escola, "osmarpio@openlink.com.br#!#webdiretor@gmail.com", pVetorBcc, "Confirmação do Lançamento de Notas através do Sistema Web Diretor", mensagem, pMsgHtml, pAutentica, pEnderecoFisicoAnexo)
'response.write(">>"&indEnviaEmail)
if indEnviaEmail<>"S" then

  Response.Write ("Ocorreu um erro.<BR>")
  Response.Write ("O Erro é " & indEnviaEmail)
  response.end()
'else
'	response.Write("OK")  
end if
