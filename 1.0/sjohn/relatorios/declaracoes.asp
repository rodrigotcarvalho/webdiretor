				...
				if co_declaracao="swd200" or co_declaracao="swd201" or co_declaracao="swd202" or co_declaracao="swd203"  or co_declaracao="swd204"then	
					...
					IF RS5.EOF Then
						...
						while not RS5.EOF
							...
							if conta_co_materia=1 then
								p_vetor_materia=co_mat_fil
								...
								p_vetor_materia=p_vetor_materia&"#!#"&co_mat_fil
							...
							compara_m3 = parametros_gerais(unidade, curso, co_etapa, turma, co_mat_fil,"compara_m3",0)					
							aproxima_m3 = parametros_gerais(unidade, curso, co_etapa, turma, co_mat_fil,"aproxima_m3",0)											
							resultados=Calc_Ter_Media (unidade, curso, co_etapa, turma, tp_modelo, tp_freq, cod_cons, co_mat_fil, CON_N, tb_nota, periodo_m3, "ATA", compara_m3, aproxima_m3, outro)
'							response.Write(resultados&"<BR>")									
'							response.Write(conta_co_materia&"<BR>")							
'							response.Write(unidade&"-"&curso&"-"&co_etapa&"-"&turma&"-"&tp_modelo&"-"&tp_freq&"-"&cod_cons&"-"&co_mat_fil&"-"&CON_N&"-"&tb_nota&"-"&periodo_m3&"-ATA-"&compara_m3&"-"&aproxima_m3&"-"&outro&"<BR>")
							resultados_apurados = split(resultados, "#!#" )	
							if conta_co_materia=1then
							...
							conta_co_materia=conta_co_materia+1										
						RS5.MOVENEXT
						wend						
					end if	
'					response.Write(	cod_cons&"<BR>")
'					response.Write(	p_vetor_materia&"<BR>")						
'					response.Write(	vetor_resultados_completos&"<BR>")								
'response.Write(unidade&","& curso&","& co_etapa&","& turma&","& cod_cons&","& vetor_materia&","& caminho_nota&","& tb_nota&","&"<BR>")
					'resultados=Calc_Med_An_Fin(unidade, curso, co_etapa, turma, cod_cons, vetor_materia, caminho_nota, tb_nota, 6, 5, 6, "final", 0)		
...
'response.Write(resultados&"<BR>")
					...
					if ubound(resultados_apurados)=-1 then
						...
						frequencia=Calcula_Frequencia(unidade, curso, co_etapa, turma, tp_modelo, tp_freq, cod_cons, "&nbsp;", "&nbsp;", CON_N , tb_nota, 0, carga, "aluno", "percent", 0)					
							
						
						resultado_final_aluno=novo2_apura_resultado_aluno(curso, co_etapa, cod_cons, p_vetor_materia, vetor_resultados_completos, frequencia, periodo_m1,  periodo_m2, periodo_m3, "ata", "S", "S", outro)					
...
Param_Logo_Gde("ScaleY") = 0.5
					...
					if co_declaracao="swd200" then
						arquivo="SWD200"
						tx_declaracao=declaramos&"cursou regularmente o "&no_curso&", neste Estabelecimento de Ensino, tendo sido aprovad"&desinencia&"."
					elseif co_declaracao="swd201" then
						arquivo="SWD201"
						tx_declaracao=declaramos&"cursou regularmente o "&no_curso&", neste Estabelecimento de Ensino, tendo sido aprovad"&desinencia&"."&declaramos_ainda
					elseif co_declaracao="swd202" then
						arquivo="SWD202"
						tx_declaracao=declaramos&"cursou regularmente o "&no_curso&", neste Estabelecimento de Ensino, tendo sido reprovad"&desinencia&"."
					elseif co_declaracao="swd203" then
						arquivo="SWD203"
						tx_declaracao=declaramos&"cursou regularmente o "&no_curso&", neste Estabelecimento de Ensino, tendo sido reprovad"&desinencia&"."&declaramos_ainda												
					...
					elseif co_declaracao="swd205" then
						arquivo="SWD205"
						tx_declaracao=declaramos&"frequentou  regularmente o  "&no_curso&", neste Estabelecimento de Ensino, para o ano letivo de "&ano_letivo&"."				
					elseif co_declaracao="swd206" then
						arquivo="SWD206"
						tx_declaracao=declaramos&"est&aacute; matriculad"&desinencia&" no "&no_curso&" para o ano letivo de "&ano_letivo&"."
					elseif co_declaracao="swd207" then
						arquivo="SWD207"
						tx_declaracao=declaramos&"est&aacute; matriculad"&desinencia&" e frequentando regularmente o "&no_curso&" no ano letivo de "&ano_letivo&"."
		
					elseif co_declaracao="swd208" then
						arquivo="SWD208"
						tx_declaracao="&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Declaramos,  para fazer prova junto ao Servi&ccedil;o Militar, que "&desinencia&" alun"&desinencia&" <b>"&nome_aluno&"</b>, filh"&desinencia&" de "&nome_pai&" e de "&nome_mae&", nascid"&desinencia&" no dia "&dt_nascimento&", cursa regularmente a "&no_curso&", neste Estabelecimento de Ensino, no ano letivo de "&ano_letivo&"."
		
					elseif co_declaracao="swd209" then
						arquivo="SWD209"
						tx_declaracao=declaramos&"est&aacute; cursando regularmente a "&no_curso&", neste Estabelecimento de Ensino, podendo concluir em dezembro de "&ano_letivo&"."			
					end if	
