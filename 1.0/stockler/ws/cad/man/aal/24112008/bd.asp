...
dia_n = vetor_nasce2(0)
mes_n = vetor_nasce2(1)
ano_n = vetor_nasce2(2)

dia_a = dia_n
mes_a = mes_n
ano_a = ano_n

emissao_fam = dia_n&"/"&mes_n&"/"&ano_n
end if
'cod_ultimo_familiar="PAI"
...
RSCONTATO_aux_bd("CO_OERG_PFisica")=tipo_id_fam
...
sql_atualiza_vinc="UPDATE TBI_Contatos SET CO_Matricula_Vinc ="& co_vinc_familiar_aux &",TP_Contato_Vinc ='"& tp_vinc_familiar_aux &"' WHERE CO_Matricula = "& cod &" AND TP_Contato = '"& cod_ultimo_familiar &"'"
Set RSCONTATO_aux_vinc = CONCONT_aux.Execute(sql_atualiza_vinc)
end if	

END IF

'response.Write(sql_atualiza_vinc)
'response.End()
...
'response.Write("1>>"&sql_atualiza_al_vinc)
'response.end()
'================================================================================================================================================
...
response.redirect("altera.asp?opt=err1&cod="&cod&"&v="&vinculado)
else	
...
response.redirect("altera.asp?opt=err2&cod="&cod&"&v="&vinculado)
end if
if isnull(tp_resp_ped) or tp_resp_ped="" then
response.redirect("altera.asp?opt=err3&cod="&cod&"&v="&vinculado)
end if
end if
...
'if do opt
END IF
response.redirect("altera.asp?opt=ok&cod="&cod)
%>