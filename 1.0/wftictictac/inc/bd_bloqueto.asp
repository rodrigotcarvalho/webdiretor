...
function GeraBloqueto(P_DADOS, P_MATRICULA, P_MES, P_SALVAR)
 GeraBloqueto = GeraBloquetoNN(P_DADOS, P_MATRICULA, P_MES, P_SALVAR, "T", "NULO")
end function

function GeraBloquetoNN(P_DADOS, P_MATRICULA, P_MES, P_SALVAR, P_TIPO, P_NOSSO_NUMERO)
'P_TIPO pode ser
'M - Mensalidade
'S - Servico
'T - Todos

'Se P_NOSSO_NUMERO <> NULO, P_TIPO deve ser igual a T
...
min = DatePart("n", now) 
    ...
    codigo_banco = "033-7"	
	nome_banco_msg = "Banco Santander"
	nome_banco_tit = "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"
	...
	Set Itau = Doc.OpenImage( Server.MapPath( addPastas&"../img/logo_santander.png") )	
...
'================================================================================================================		
			...
			if P_TIPO = "M" then
				sql_cond_pos = "NO_Lancamento='Mensalidade' AND" 
			...
			if P_NOSSO_NUMERO<>"NULO" then
				sql_cond_pos = sql_cond&" SQ_Bloqueto = "&P_NOSSO_NUMERO&" AND "
				sql_cond_bloq = " SQ_Bloqueto = "&P_NOSSO_NUMERO&" AND "				
			else
				sql_cond_pos = sql_cond&" VA_Realizado=0 AND "	
				sql_cond_bloq = ""					
			end if
			
			
			SQL4= "SELECT * FROM TB_Posicao WHERE "&sql_cond_pos&" CO_Matricula_Escola ="& cod_cons &" AND Mes = "&wrk_mes
			RS4.Open SQL4, CON4	
			...
			Set RS1 = Server.CreateObject("ADODB.Recordset")
			SQL1 = "SELECT * FROM TB_Bloqueto WHERE DA_Vencimento>=#"& vencimento_inicial &"# and DA_Vencimento<#"& vencimento_fim &"# AND "&sql_cond_bloq&" CO_Matricula_Escola ="& cod_cons
			'response.Write(	SQL1)	
			RS1.Open SQL1, CONBL
...
'			Table(2, 8).AddText ano_letivo, "size=9;alignment=right", Font 
			...
			Set param_table1 = Pdf.CreateParam("width=533; height=120; rows=7; cols=1; border=0; cellborder=0; cellspacing=0;")
			Set Table = Doc.CreateTable(param_table1)
			Table.Font = Font
			y_table=y_segundo_separador-10
			Table.Rows(2).Cells(1).Height = 10
			Table.Rows(4).Cells(1).Height = 10
			Table.Rows(6).Cells(1).Height = 10
			Table(1, 1).AddText msg01&"<BR>"&msg02&"<BR>"&msg03&"<BR>"&msg04, "size=9; expand=true; html=true", Font 
			'Table(2, 1).AddText "", "size=5; expand=true", Font 					
			'Table(3, 1).AddText msg02, "size=9; expand=true", Font 
			'Table(4, 1).AddText "", "size=5; expand=true", Font 						
			'Table(5, 1).AddText msg03, "size=9; expand=true", Font 
			'Table(6, 1).AddText "", "size=5; expand=true", Font 						
			'Table(7, 1).AddText msg04, "size=9; expand=true", Font 		
			Page.Canvas.DrawTable Table, "x="&margem_x&", y="&y_table&"" 				
			...
			if va_inicial ="" or isnull(va_inicial) then
				va_inicial=0
			end if						
...
'Cria tabelas com informações de cobrança---------------------------------------------------------------------	
			...
			y_tabela_2=y_table-80
			for i = 1 to 2 
			
			
			var_instrucoes="<font style=""font-size:6pt;"">INSTRU&Ccedil;&Otilde;ES DE RESPONSABILIDADE DO BENEFICI&Aacute;RIO. QUALQUER D&Uacute;VIDA SOBRE ESTE BOLETO, CONTATE O BENEFICI&Aacute;RIO</font><font style=""font-size:3pt;""><BR>&nbsp;<br></FONT><font style=""font-size:7pt;"">"&msg01&"<BR>"&msg02
			
			var_instrucoes=	var_instrucoes&"</FONT><font style=""font-size:6pt;""><BR>"				

				var_instrucoes=	var_instrucoes&msg03&"<BR>&nbsp;<BR>Ap&oacute;s o vencimento pagar com acr&eacute;scimo de MULTA de R$"&multa&" mais mora di&aacute;ria de R$"&juros&"<BR>"
	...
	'			var_instrucoes=	var_instrucoes&"Servi&ccedil;os Adicionais poder&atilde;o ser cobrados a parte conforme prevista cláusula V par&aacute;grafo primeiro do contrato.<BR>"&TEXTO_UCASE&"</font>" 
				var_instrucoes=	var_instrucoes&"Ap&oacute;s o vencimento pagar somente nas ag&ecirc;ncias do Banco Santander;"
				...
				End With
				'width_stb_1 = width_tabela_2+75'formatnumber(width_tabela_2/7,0) -5 + ((formatnumber(width_tabela_2/7,0)-30)*2)+formatnumber(width_tabela_2/7,0)-5 +formatnumber(width_tabela_2/7,0)-20+formatnumber(width_tabela_2/7,0)+20 
				width_stb_1 = width_tabela_2+95
				...
				SmallTable15.At(2, 1).AddText "<CENTER>101</CENTER>", " html=true; size=7;", Font			
				...
				SmallTable17.At(2, 1).AddText "<CENTER>&nbsp;</CENTER>", " html=true; size=7;", Font		
				...
				SmallTable22.At(2, 3).Canvas.DrawText "Curso: "&no_curso&"/Turno:"&no_turno, "x=1; y=11, size=7; html=true", Font						
...
'Fim da tabela de informações de cobrança--------------------------------------------------------------------		
			...
			x_texto_3 = 350
			y_texto_3 = formatnumber(y_table - 285,0)
			for j = 1 to 2 
			...
			y_primeira_linha = formatnumber(y_table - 310 - wrk_y_osmar -margem_y,0)			
				With Page.Canvas
			...
			Param_Itau("y") = y_texto_4-altura_Itau-3
