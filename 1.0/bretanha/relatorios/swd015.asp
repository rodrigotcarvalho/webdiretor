...
if curso=0 then
	...
	SQLtabela = "SELECT * FROM TB_Da_Aula where NU_Unidade ="& unidade &" AND CO_Curso ='"& curso &"' AND CO_Etapa ='"& co_etapa &"' AND CO_Turma ='"& turma &"' AND CO_Materia_Principal = '"& co_materia &"'" 
	RStabela.Open SQLtabela, CON2

		
	if RStabela.EOF then
		Set RStabela = Server.CreateObject("ADODB.Recordset")
		SQLtabela = "Select * from TB_Da_Aula_Subs WHERE CO_Professor= "& co_prof &"AND NU_Unidade = "& unidade &" AND CO_Curso = '"& curso &"' AND CO_Etapa = '"& co_etapa &"' AND CO_Turma = '"& turma &"' AND CO_Materia = '"& co_materia &"'"
		Set RStabela = CON2.Execute(SQLtabela)	
		...
		mat_princ=RStabela("CO_Materia_Principal")	
		co_materia= RStabela("CO_Materia")	
			Set RS5a = Server.CreateObject("ADODB.Recordset")
			SQL5a = "SELECT * FROM TB_Programa_Subs where CO_Etapa ='"& co_etapa &"' AND CO_Curso ='"& curso &"' AND CO_Materia_Principal ='"& mat_princ &"' AND CO_Materia_Filha = '"&co_materia&"'"
			'response.Write(SQL5a)
