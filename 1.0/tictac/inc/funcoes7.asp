					...
					'response.Write(colunas_notas_periodo(cln_notas)&"<BR>")
						...
						if colunas_notas_calc(cln_notas)= "BDM" or colunas_notas_calc(cln_notas)= "BDR"or colunas_notas_calc(cln_notas)= "RF" or colunas_notas_calc(cln_notas)= "BDF" then
							if tp_materia="T_F_F_N" or tp_materia="F_T_F_N"	 or tp_materia="F_F_T_N" then				
								...
								media=Calcula_Media_T_F_F_N(unidade, curso, co_etapa, turma, cod_cons, codigo_materia_pr, co_materia_consulta, CONn, tb_nota, colunas_notas_periodo(cln_notas), var_bd, outro)
					'response.Write(">>>"&colunas_notas_periodo(cln_notas)&"<BR>")
							elseif tp_materia="T_T_F_N" then
							
								vetor_materia=busca_materias_filhas(co_materia_consulta)
								media=Calcula_Media_T_T_F_N(unidade, curso, co_etapa, turma, cod_cons,co_materia_consulta, vetor_materia, CONn, tb_nota, colunas_notas_periodo(cln_notas), var_bd, outro)		

							elseif tp_materia="T_F_T_N" then
								vetor_materia=busca_materias_filhas(co_materia_consulta)						
								media=Calcula_Media_T_F_T_N(unidade, curso, co_etapa, turma, cod_cons, co_materia_consulta, vetor_materia, CONn, tb_nota, colunas_notas_periodo(cln_notas), var_bd, outro)	
...
Server.ScriptTimeout = 900
	...
	Set RS = Server.CreateObject("ADODB.Recordset")
	SQL_A = "Select * from TB_Matriculas WHERE NU_Ano="&ano_letivo&" AND NU_Unidade = "& unidade &sql_curso&sql_etapa&sql_turma&" order by NU_Chamada"
...
Server.ScriptTimeout = 900
	...
	elseif tipo="opt" then	
		tabela_nota=opcao
	elseif tipo = "coo" then
		tabela_nota=coordenador	
