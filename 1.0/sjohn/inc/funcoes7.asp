...
Server.ScriptTimeout = 900 'valor em segundos
	...
	IF RS5.EOF Then
		...
		if session("ano_letivo") < ano_letivo_prog_aula then
			...
			vetor_materia_exibe=programa_aula_boletim_ficha(vetor_materia, unidade, curso, co_etapa, "nulo")			
		end if
...
'	nome_periodo=split(vetor_nom_periodos,"#!#")
	...
	'			medias="#$#"
			...
			'response.Write(medias&"<BR>")
				...
				response.Write(qtd_medias_materia&"<BR>")						
				ordem_exibe=1
...
Server.ScriptTimeout = 900
	...
	if outro="nome" or outro="nome_ativo" then
...
'	END IF	
'	
		if unidade="999990" or unidade="" or unidade="nulo" or isnull(unidade) then
			SQL_BUSCA_ALUNOS= "Select TB_Matriculas.CO_Matricula, TB_Matriculas.NU_Chamada, TB_Matriculas.CO_Situacao, TB_Alunos.NO_Aluno  from TB_Matriculas, TB_Alunos WHERE TB_Matriculas.NU_Ano="& ano_letivo &" AND TB_Matriculas.CO_Matricula=TB_Alunos.CO_Matricula  order by TB_Matriculas.NU_Unidade,TB_Matriculas.CO_Curso, TB_Matriculas.CO_Etapa, TB_Matriculas.CO_Turma, TB_Alunos.NO_Aluno"	
		else	
			SQL_ALUNOS= "Select TB_Matriculas.CO_Matricula, TB_Matriculas.NU_Chamada, TB_Matriculas.CO_Situacao, TB_Alunos.NO_Aluno  from TB_Matriculas, TB_Alunos WHERE TB_Matriculas.NU_Ano="& ano_letivo &" AND TB_Matriculas.CO_Matricula=TB_Alunos.CO_Matricula AND TB_Matriculas.NU_Unidade = "& unidade		
			if curso="999990" or curso="" or curso="nulo" or isnull(curso) then
				SQL_CURSO=""
			else
				SQL_CURSO=" AND TB_Matriculas.CO_Curso = '"& curso &"'"			
			end if
		
			if co_etapa="999990" or co_etapa="" or co_etapa="nulo" or isnull(co_etapa) then
				SQL_ETAPA=""		
			else
				SQL_ETAPA=" AND TB_Matriculas.CO_Etapa = '"& co_etapa &"'"				
			end if
		
			if turma="999990" or turma="" or turma="nulo" or isnull(turma) then
				SQL_TURMA=""		
			else
				SQL_TURMA=" AND TB_Matriculas.CO_Turma = '"& turma &"' "			
			end if
			
			if outro="nome_ativo" then
				SQL_ATIVO=" AND TB_Matriculas.CO_Situacao = 'C' "	
			else
				SQL_ATIVO=""				
			end if
		
		SQL_BUSCA_ALUNOS= SQL_ALUNOS&SQL_CURSO&SQL_ETAPA&SQL_TURMA&SQL_ATIVO&" order by TB_Matriculas.NU_Unidade,TB_Matriculas.CO_Curso, TB_Matriculas.CO_Etapa, TB_Matriculas.CO_Turma, TB_Alunos.NO_Aluno"
		end if


	Set RSA = Server.CreateObject("ADODB.Recordset")
	CONEXAOA = SQL_BUSCA_ALUNOS
	'CONEXAOA = "Select TB_Matriculas.CO_Matricula, TB_Matriculas.NU_Chamada, TB_Matriculas.CO_Situacao, TB_Alunos.NO_Aluno from TB_Matriculas, TB_Alunos WHERE TB_Matriculas.NU_Ano="& ano_letivo &" AND TB_Matriculas.CO_Matricula=TB_Alunos.CO_Matricula AND TB_Matriculas.NU_Unidade = "& unidade&" AND TB_Matriculas.CO_Curso = '"& curso &"' AND TB_Matriculas.CO_Etapa = '"& co_etapa &"' AND TB_Matriculas.CO_Turma = '"& turma &"' order by "&order	
	...
	While Not RSA.EOF
		...
		if co_aluno_check=0 then
			alunos_vetor=nu_matricula&"#!#"&nu_chamada&"#!#"&nome_aluno&"#!#"&situacao
		else
			alunos_vetor=alunos_vetor&"#$#"&nu_matricula&"#!#"&nu_chamada&"#!#"&nome_aluno&"#!#"&situacao
		end if
		co_aluno_check=co_aluno_check+1	
		...
		if qto_falta="s" then
			...
			for v=0 to ubound(vetor_periodo)
				if session("ano_letivo") >=2017 then
					...
					elseif vetor_periodo(v)="TRI3" then	
						...
						periodo_exibe=periodo_exibe&"#!#"&vetor_periodo(v)&"#!#MT#!#QF2"					
			...
			for v=0 to ubound(vetor_periodo)
				if session("ano_letivo") >=2017 then
					...
					elseif vetor_periodo(v)="TRI3" then	
						temp_num_periodo=temp_num_periodo&"#!#3#!#0"
						periodo_exibe=periodo_exibe&"#!#"&vetor_periodo(v)&"#!#MT"					
					elseif vetor_periodo(v)="FINAL" then	
						temp_num_periodo=temp_num_periodo&"#!#4#!#0"
						periodo_exibe=periodo_exibe&"#!#"&vetor_periodo(v)&"#!#MF"	
					elseif vetor_periodo(v)="REC" then	
			...
			for per=0 to ubound(nom_periodo)
				...
				For mat=0 to ubound(ajusta_materias)		
					if ajusta_materias(mat)="" or isnull(ajusta_materias(mat)) then
						...
						'cod_cons=cod_cons*1
						'if cod_cons = 100741 then
						'			response.write(nom_periodo(per)&" "&periodo_m1&" "&periodo_m2&" "&periodo_m3&"<BR>")	
						'end if
...
if codigo_materia="MED" then
	if no_periodo<>"QF1" and no_periodo<>"QF2" and no_periodo<>"MB" and no_periodo<>"MT" and no_periodo<>"MF" then		
		periodo=periodo*1
		if periodo=1 then
			media=Session("md_p1")
			Session("md_p1")=""
		...
		elseif no_periodo="QF2" then	
			...
			Session("md_qf2")=""
		elseif no_periodo="MB" or no_periodo="MT" then		
			media=Session("md_mb")
			Session("md_mb")=""
...
'response.Write(SQL)
	...
	'response.end()
	'end if
				'response.write(no_periodo&" "&codigo_materia&" "&wrk_qtd_periodos&" "&wrk_periodo_m2&" "&wrk_periodo_m3)
			'response.End()	
	if (mae=TRUE and fil=FALSE and in_co=FALSE and isnull(peso)) then
		if no_periodo<>"QF1" and no_periodo<>"QF2" and no_periodo<>"MB" and no_periodo<>"MT"  and no_periodo<>"MF" then	
			media=Calcular_Media_T_F_F_N(unidade, curso, co_etapa, turma, cod_aluno, codigo_materia, caminho_nota, tb_nota, periodo)	
		else
			acumula_media=0
			if no_periodo="QF1" then
			periodo_qf=periodo_m1-1
				for periodo=1 to periodo_qf
					qf=Calcular_Media_T_F_F_N(unidade, curso, co_etapa, turma, cod_aluno, codigo_materia, caminho_nota, tb_nota, periodo)	
			...
			elseif no_periodo="QF2" then	
				resultado=Calc_Med_An_Fin(unidade, curso, co_etapa, turma, cod_aluno, codigo_materia, caminho_nota, tb_nota, wrk_qtd_periodos, wrk_periodo_m2, wrk_periodo_m3,"anual", 0)
				if resultado="&nbsp;#!#&nbsp;" then
					media=""	
				else				
					...
					if media_qf(0)>=nota_m1 then
						...
						'response.Write(resultado&" + "&cod_aluno&" "&codigo_materia)
						'response.end					
			...
			elseif no_periodo="MB" or no_periodo="MT" then		
				...
				resultado=Calc_Med_An_Fin(unidade, curso, co_etapa, turma, cod_aluno, codigo_materia, caminho_nota, tb_nota, wrk_qtd_periodos, wrk_periodo_m2, wrk_periodo_m3,"anual", outro)						
				media_qf=split(resultado,"#!#")
				media=media_qf(0)
			elseif no_periodo="MF" then
				m5=Calcular_Media_T_F_F_N(unidade, curso, co_etapa, turma, cod_aluno, codigo_materia, caminho_nota, tb_nota, wrk_periodo_m2)	
				if m5="&nbsp;" or m5="" or isnull(m5) then
					acumula_media=acumula_media
					media="&nbsp;"
				else
					resultado=Calc_Med_An_Fin(unidade, curso, co_etapa, turma, cod_aluno, codigo_materia, caminho_nota, tb_nota, wrk_qtd_periodos, wrk_periodo_m2, wrk_periodo_m3,"recuperacao", outro)						
					media_qf=split(resultado,"#!#")
					media=media_qf(0)			
				end if				
	...
	elseif mae=TRUE and fil=TRUE and in_co=FALSE and isnull(peso) then
		...
		if no_periodo<>"QF1" and no_periodo<>"QF2" and no_periodo<>"MB" and no_periodo<>"MT" and no_periodo<>"MF" then	
			media=Calcular_Media_T_T_F_N(unidade, curso, co_etapa, turma, cod_aluno, codigo_materia, vetor_materia, caminho_nota, tb_nota, periodo)	
			periodo=periodo*1
			...
			periodo_qf=periodo_m1-1
				for periodo=1 to periodo_qf
					qf=Calcular_Media_T_T_F_N(unidade, curso, co_etapa, turma, cod_aluno, codigo_materia, vetor_materia, caminho_nota, tb_nota, periodo)	
			...
			elseif no_periodo="MB" or no_periodo="MT" then		
				...
				media=""
			elseif no_periodo="MF" then
				m5=Calcular_Media_T_T_F_N(unidade, curso, co_etapa, turma, cod_aluno, codigo_materia, vetor_materia, caminho_nota, tb_nota, wrk_periodo_m2)	
				if m5="&nbsp;" or m5="" or isnull(m5) then
					acumula_media=acumula_media
					media="&nbsp;"
				else
	...
	elseif(mae=TRUE and fil=FALSE and in_co=TRUE and isnull(peso)) then		
		...
		if no_periodo<>"QF1" and no_periodo<>"QF2" and no_periodo<>"MB" and no_periodo<>"MT" and no_periodo<>"MF" then	
			...
			media=Calcular_Media_T_F_T_N(unidade, curso, co_etapa, turma, tp_modelo, tp_freq, cod_aluno, codigo_materia, vetor_materia, caminho_nota, tb_nota, periodo, nome_nota, outro)
		else
			acumula_media=0
			if no_periodo="QF1" then
			periodo_qf=periodo_m1-1
				for periodo=1 to periodo_qf
					qf=Calcular_Media_T_F_T_N(unidade, curso, co_etapa, turma, tp_modelo, tp_freq, cod_aluno, codigo_materia, vetor_materia, caminho_nota, tb_nota, periodo, nome_nota, outro)	
					'response.Write(acumula_media&"p"&periodo&"<br>")
					if qf="&nbsp;" or qf="" or isnull(qf) then
						acumula_media=acumula_media
					else
			...
			elseif no_periodo="QF2" then	
				...
				'response.Write(wrk_qtd_periodos&" / "&wrk_periodo_m2&" "&wrk_periodo_m3)					
				'response.Write(resultado&" / "&cod_aluno&" "&codigo_materia)
				'response.end					
						peso_m2_m1=peso_m2_m1*1
						peso_m2_m2=peso_m2_m2*1		
			...
			elseif no_periodo="MB" or no_periodo="MT" then		
				...
				media=media_qf(0)
			elseif no_periodo="MF" then
				m5=Calcular_Media_T_F_T_N(unidade, curso, co_etapa, turma, tp_modelo, tp_freq, cod_aluno, codigo_materia, vetor_materia, caminho_nota, tb_nota, wrk_periodo_m2, nome_nota, outro)
				if m5="&nbsp;" or m5="" or isnull(m5) then
					acumula_media=acumula_media
					media="&nbsp;"
				else
	...
	elseif (mae=FALSE and fil=TRUE and in_co=FALSE) then
		...
		if no_periodo<>"QF1" and no_periodo<>"QF2" and no_periodo<>"MB" and no_periodo<>"MT" and no_periodo<>"MF" then	
			media=Calcular_Media_F_T_F_N(unidade, curso, co_etapa, turma, cod_aluno, codigo_materia_pr, codigo_materia, caminho_nota, tb_nota, periodo)	
			
		else
			acumula_media=0
			'QF1, QF2, QF3, MT e MF passam a ser  calculados a partir do ano letivo de 2017			
			if no_periodo="QF1" then
				if session("ano_letivo") >= 2017 then
					periodo_qf=periodo_m1-1
					for periodo=1 to periodo_qf
						qf=Calcular_Media_F_T_F_N(unidade, curso, co_etapa, turma, cod_aluno, codigo_materia_pr, codigo_materia, caminho_nota, tb_nota, periodo)	
						if qf="&nbsp;" or qf="" or isnull(qf) then
							acumula_media=acumula_media
			...
			elseif no_periodo="QF2" then	
				if session("ano_letivo") >= 2017 then			
					resultado=Calc_Med_An_Fin(unidade, curso, co_etapa, turma, cod_aluno, codigo_materia, caminho_nota, tb_nota, wrk_qtd_periodos, wrk_periodo_m2, wrk_periodo_m3,"anual_ftf", 0)
					if resultado="&nbsp;#!#&nbsp;" then
						media=""	
					else
						media_qf=split(resultado,"#!#")
			...
			elseif no_periodo="MB" or no_periodo="MT" then	
				if session("ano_letivo") >= 2017 then					
					resultado= Calc_Med_An_Fin(unidade, curso, co_etapa, turma, cod_aluno, codigo_materia, caminho_nota, tb_nota, wrk_qtd_periodos, wrk_periodo_m2, wrk_periodo_m3,"anual_ftf", outro)						
					media_qf=split(resultado,"#!#")
					media=media_qf(0)
				else
					media=""
			...
			elseif no_periodo="MF" then
				if session("ano_letivo") >= 2017 then					
					m5=Calcular_Media_F_T_F_N(unidade, curso, co_etapa, turma, cod_aluno, codigo_materia_pr, codigo_materia, caminho_nota, tb_nota, wrk_periodo_m2)	
					
					if m5="&nbsp;" or m5="" or isnull(m5) then
						acumula_media=acumula_media
						media="&nbsp;"
					else
						resultado=Calc_Med_An_Fin(unidade, curso, co_etapa, turma, cod_aluno, codigo_materia, caminho_nota, tb_nota, wrk_qtd_periodos, wrk_periodo_m2, wrk_periodo_m3,"recuperacao_ftf", outro)						
						media_qf=split(resultado,"#!#")
						media=media_qf(0)			
					end if				
				else
...
Function Calcular_Media_T_F_F_N(unidade, curso, co_etapa, turma, cod_aluno, codigo_materia, caminho_nota, tb_nota, periodo)
	Set CON0 = Server.CreateObject("ADODB.Connection") 
	ABRIR0 = "DBQ="& CAMINHO_pr & ";Driver={Microsoft Access Driver (*.mdb)}"
	CON0.Open ABRIR0
	...
	Calcular_Media_T_F_F_N=va_m3

end function

Function Calcular_Media_F_T_F_N(unidade, curso, co_etapa, turma, cod_aluno, codigo_materia_pr, codigo_materia, caminho_nota, tb_nota, periodo)
	Set CON0 = Server.CreateObject("ADODB.Connection") 
	ABRIR0 = "DBQ="& CAMINHO_pr & ";Driver={Microsoft Access Driver (*.mdb)}"
	CON0.Open ABRIR0
	...
	Calcular_Media_F_T_F_N=va_m3

end function


'===========================================================================================================================================
Function Calcular_Media_T_T_F_N(unidade, curso, co_etapa, turma, cod_aluno, codigo_materia_pr, vetor_materia, caminho_nota, tb_nota, periodo)	
anulou="n"
acumula=0
divisor=0
	Set CON0 = Server.CreateObject("ADODB.Connection") 
	...
	for j=0 to ubound(co_materia_mae_fil)			
		...
		media_aluno=Calcular_Media_F_T_F_N(unidade, curso, co_etapa, turma, cod_aluno, codigo_materia_pr, disciplina_filha, caminho_nota, tb_nota, periodo)	
			if media_aluno="" or isnull(media_aluno) or media_aluno="&nbsp;" then
				anulou="s"
			else
				acumula=acumula*1	
				media_aluno=media_aluno*1
				acumula=acumula+(media_aluno*peso)
				'response.Write(media_aluno&"-"&acumula&"-"&divisor&"<BR>")
			end if					
	next
	...
	if anulou="s" then
		...
		va_m3=arredonda(va_m3,"mat",0,0)
				...
				'response.Write(va_m3&"<BR>")
Calcular_Media_T_T_F_N=va_m3	
'response.End()	
end function

Function Calcular_Media_T_F_T_N(unidade, curso, co_etapa, turma, tp_modelo, tp_freq, cod_aluno, codigo_materia_pr, vetor_materia, caminho_nota, tb_nota, periodo, nome_nota, outro)	
