			...
			'response.Write(co_materia_consulta&"   "&medias_materia(k)&" - "&k&"<BR>")			
				...
				RS4.addnew	
					...
					for tn=0 to ubound(grava_notas)			
						...
						if n_campo<12 and ((curso=1 and co_etapa<6 and (co_materia_consulta="ART" or co_materia_consulta="ESP")) or (co_materia_consulta="EFI" or co_materia_consulta="ARTE")) then									
			...
			for per=0 to ubound(nom_periodo)
				...
				For mat=0 to ubound(ajusta_materias)		
					if ajusta_materias(mat)="" or isnull(ajusta_materias(mat)) then
						...
						if nom_periodo(per)<>"QF1" and nom_periodo(per)<>"QF2" and ((curso=1 and co_etapa<6 and (ajusta_materias(mat)="ART" or ajusta_materias(mat)="ESP" or ajusta_materias(mat)="ING")) or (ajusta_materias(mat)="EFI" or ajusta_materias(mat)="ARTE")) then																	
