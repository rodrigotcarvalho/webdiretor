...
if opt="i" or opt="a" then
	...
	unidade_grava5 = request.form("unidade5")
	curso_grava5 = request.form("curso5")
	etapa_grava5 = request.form("etapa5")
	turma_grava5 = request.form("turma5")	
...
if unidade_grava5="999990" and curso_grava5="999990" and etapa_grava5="999990" and turma_grava5="999990" then
	assoc5="n"
	else
	if unidade_grava5="999990" or unidade_grava5="" or isnull(unidade_grava5) then
	'sql_un="Unidade = NULL, "
	unidade_grava5= NULL
	else
	'sql_un="Unidade= '"&unidade_grava5&"', "
	end if
	
	if curso_grava5="999990" or curso_grava5="" or isnull(curso_grava5) then
	'sql_cu="Curso = NULL,"
	curso_grava5=NULL
	else
	'sql_cu="Curso='"&curso_grava5&"', "
	end if
	
	if etapa_grava5="999990" or etapa_grava5="" or isnull(etapa_grava5) then
	'sql_et="Etapa = NULL, "
	etapa_grava5=NULL
	else
	'sql_et="Etapa='"&etapa_grava5&"', "
	
	end if
	
	if turma_grava5="999990" or turma_grava5="" or isnull(turma_grava5) then
	'sql_tu="Turma = NULL , "
	turma_grava5=NULL
	else
	'sql_tu="(Turma='"&turma_grava5&"', "
	end if
...
if opt="i" then
	...
	CONEXAOA = "Select CO_Matricula from TB_Matriculas WHERE CO_Situacao = 'C' And NU_Ano="& ano_letivo &SQL_UNIDADE&SQL_CURSO&SQL_ETAPA&SQL_TURMA
	...
	if assoc5="s" then
		...
		RS_updt.addnew
			...
			RS_updt("NO_Doc")=arquivo
			RS_updt("Unidade") = unidade_grava5
			RS_updt("Curso") = curso_grava5
			RS_updt("Etapa") = etapa_grava5
			RS_updt("Turma")=turma_grava5
			RS_updt("TI1_Doc")=tit
			RS_updt("TP_Doc")=tipo_arquivo
			RS_updt("DA_Doc")=data_inclui	
		...
		if isnull(unidade_grava5) then
			SQL_UNIDADE=""
		else
			SQL_UNIDADE="AND TB_Matriculas.NU_Unidade = "& unidade_grava5
		end if
	
		if isnull(curso_grava5) then
			SQL_CURSO=""
		else
			SQL_CURSO=" AND TB_Matriculas.CO_Curso = '"& curso_grava5 &"'"
		end if
		
		if isnull(etapa_grava5) then
			SQL_ETAPA=""
		else
			SQL_ETAPA=" AND TB_Matriculas.CO_Etapa = '"& etapa_grava5 &"'"
		end if	
		
		if isnull(turma_grava5) then
			SQL_TURMA=""
		else
			SQL_TURMA=" AND TB_Matriculas.CO_Turma = '"& turma_grava5 &"'"
		end if	
