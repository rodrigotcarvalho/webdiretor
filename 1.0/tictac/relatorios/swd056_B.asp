...
'response.end()
			...
			linha=1
			'resultados=Calc_Med_An_Fin(unidade, curso, co_etapa, turma, vetor_matriculas, vetor_materia, caminho_nota, tb_nota, 5, 4, 5, "recup", 0)		
'response.Write(resultados_apurados)
...
'				resultados_alunos = split(resultados_apurados(a), "#$#" )	
				...
				for n=0 to ubound(co_materia_exibe)	
					...
					SQLm = "SELECT * FROM TB_Materia where CO_Materia ='"& co_materia_exibe(n) &"'"
					RSm.Open SQLm, CON0				
					...
					coluna=coluna+1					
					resultados = Calc_Ter_Media (unidade, curso, co_etapa, turma, dados_alunos(0), co_materia_exibe(n), caminho_nota, tb_nota, 5, tipo_calculo, outro)
					preserva_results=resultados
					resultados_materia = SPLIT(resultados,"#!#")
...
'response.Write(calcula_frequencia&"-"&disc_obrigatoria&"<<BR>")					
					media=resultados_materia(0)
					res_aluno=resultados_materia(1)
...
'					end if						
					...
					resultado_final_aluno=result_temp
					if res_aluno = "DEP" then
						if qtd_dep>qtd_max_dep then
							resultado_final_aluno=res_rep	
						else	
							resultado_final_aluno=res_dep	
						end if
					end if	
				next		
'			response.Write(		calcula_frequencia)
'			response.End()
...
'					response.Write(horas_aula&"-"&total_faltas)	
'response.End()
