...
'	vetor_materia_exibe=vetor_disciplinas(ano_letivo,unidade,curso,co_etapa,turma,"s",0)
	...
	prd_seg_media=Periodo_Media(tp_modelo,"MF",outro)
...
'	nome_periodo=split(vetor_nom_periodos,"#!#")
	...
	if alunos_vetor="nulo" then
		...
		for al=0 to ubound(n_alunos)
			...
			for k=0 to ubound(co_materia_exibe)		
...
'					response.Write(medias_materia(k)&"<BR>")	
					for notas=0 to ubound(colunas_notas_periodo)
					'response.Write(notas&"<BR>")
						var_bd=var_bd_periodo(tp_modelo,tp_freq,tb_nota,colunas_notas_periodo(notas),colunas_notas_calc(notas))
						...
						if colunas_notas_calc(notas)= "BDM" or colunas_notas_calc(notas)= "BDR"or colunas_notas_calc(notas)= "RF" or colunas_notas_calc(notas)= "BDF" then
							if tp_materia="T_F_F_N" or tp_materia="F_T_F_N"	 or tp_materia="F_F_T_N" then				
								...
								media=Calcula_Media_T_F_F_N(unidade, curso, co_etapa, turma, cod_cons, codigo_materia_pr, co_materia_consulta, CONn, tb_nota, colunas_notas_periodo(notas), var_bd, outro)

							elseif tp_materia="T_T_F_N" then
							
								vetor_materia=busca_materias_filhas(co_materia_consulta)
								media=Calcula_Media_T_T_F_N(unidade, curso, co_etapa, turma, cod_cons,co_materia_consulta, vetor_materia, CONn, tb_nota, colunas_notas_periodo(notas), var_bd, outro)		

							elseif tp_materia="T_F_T_N" then
								vetor_materia=busca_materias_filhas(co_materia_consulta)						
								media=Calcula_Media_T_F_T_N(unidade, curso, co_etapa, turma, cod_cons, co_materia_consulta, vetor_materia, CONn, tb_nota, colunas_notas_periodo(notas), var_bd, outro)	
							...
							if colunas_notas_calc(notas)= "ASTER" then
								media=Calcula_Asterisco(tp_modelo, tp_freq, unidade, curso, co_etapa, turma, cod_cons, co_materia_consulta, CONn, tp_materia, tb_nota, colunas_notas_periodo(notas))		
							...
							elseif colunas_notas_calc(notas)= "PF"	then
								prd_ter_media=Periodo_Media(tp_modelo,"PF",outro)
							...
							elseif colunas_notas_calc(notas)= "CMT" then
								...
								media=calcula_medias(unidade, curso, co_etapa, turma, colunas_notas_periodo(notas), co_matric_alunos_turma, co_materia_consulta, caminho_nota, tb_nota, var_bd, "media_turma")	
			...
			if qto_falta="s" then
				...
				for v=0 to ubound(vetor_periodo)
					...
					elseif vetor_periodo(v)="BIM2" then	
						temp_num_periodo=temp_num_periodo&"#!#2#!#2#!#1#!#2"
						periodo_exibe=periodo_exibe&"#!#"&vetor_periodo(v)&"#!#Rec.P#!#BIM1*#!#BIM2*"
					...
					elseif vetor_periodo(v)="REC" then	
						temp_num_periodo=temp_num_periodo&"#!#5#!#0#!#0"
...
'	nome_periodo=split(vetor_nom_periodos,"#!#")
	...
	periodo_m2=Periodo_Media(tp_modelo,"MF",outro)
	periodo_m3=Periodo_Media(tp_modelo,"PF",outro)
...
'
	...
	if no_periodo="BIM1" or no_periodo="BIM2" or no_periodo="BIM3" or no_periodo="BIM4" or no_periodo="TRI1" or no_periodo="TRI2" or no_periodo="TRI3" then
		...
		if tp_materia="T_F_F_N" or tp_materia="F_T_F_N"	 or tp_materia="F_F_T_N" then	
			...
			media=Calcula_Media_T_F_F_N(unidade, curso, co_etapa, turma, cod_aluno, codigo_materia_pr, codigo_materia, CONn, tb_nota, periodo, var_bd, outro)	 
