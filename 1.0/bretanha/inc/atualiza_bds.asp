...
<%
Function Grava_BDS(p_nu_matricula, p_co_materia_pr, p_co_materia, p_periodo, p_in_faltas, p_in_bonus, p_rec_semestral, p_va_faltas, p_va_pt, p_va_pp, p_va_t1, p_va_t2, p_va_t3, p_va_p1, p_va_p2, p_va_p3, p_va_bon, p_va_rec, p_data, p_horario, p_co_usr,p_todas_mt_subs_lancadas, p_todas_mp_subs_lancadas)
...
if not p_in_faltas then
	va_faltas=NULL
	
end if

if not p_in_bonus then
	va_bon = NULL
end if	

periodo = periodo*1
if (not p_rec_semestral) then
	va_rec = NULL	
end if	
...
if grava_rec = "ok" then
	CON.BeginTrans 
		...
		CON.CommitTrans 
		'CON.RollbackTrans para rollback
	end if		
end if
'response.Write("AQUI")
'response.End()	
	if fail = 1 then
		Grava_BDS = co_materia_pr&"$!$"&url 
	else
		Grava_BDS = "S"		
	end if	
