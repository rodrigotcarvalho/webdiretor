...
'		SQL_A = "Select * from TB_Matricula WHERE NU_Unidade = "& unidade &" AND CO_Curso = '"& curso &"' AND CO_Etapa = '"& co_etapa &"' AND CO_Turma = '"& turma &"' order by NU_Chamada"
...
response.Write(no_curso)%>
	...
	<%
		...
		CONEXAO = "Select * from TB_Da_Aula WHERE NU_Unidade = "& unidade &" AND CO_Curso = '"& curso &"' AND CO_Etapa = '"& co_etapa &"' AND CO_Turma = '"& turma &"' AND CO_Materia_Principal = '"& co_materia &"'"
		Set RSTB = CON2.Execute(CONEXAO)
		
		if RSTB.EOF then
			Set RS2 = Server.CreateObject("ADODB.Recordset")
			CONEXAO2 = "Select * from TB_Da_Aula_Subs WHERE CO_Professor= "& co_prof &" AND NU_Unidade = "& unidade &" AND CO_Curso = '"& curso &"' AND CO_Etapa = '"& co_etapa &"' AND CO_Turma = '"& turma &"' AND CO_Materia = '"& co_materia &"'"
...
elseif nota = "TB_NOTA_S" Then
	CAMINHOn = CAMINHO_ns
	Call notas(CAMINHO_al,CAMINHOn,unidade,curso,co_etapa,turma,co_materia,periodo,ano_letivo,co_usr,"S","imp",0)
	'response.Redirect("imprime_c.asp?p="&p&"&obr="&obr)	
else
