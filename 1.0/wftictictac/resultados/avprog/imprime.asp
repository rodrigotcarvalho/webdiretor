...
session("chave") = "WFU-U-U-CBA"	
	...
	CON1.Open ABRIR1
		...
		ABRIR3 = "DBQ="& CAMINHO_USR & ";Driver={Microsoft Access Driver (*.mdb)}"
		CON3.Open ABRIR3
...
if isnull(un_uf) or isnull(un_cidade) or un_uf=""  or un_cidade=""  then

else
	cidade= GeraNomes("Mun",un_uf,un_cidade,variavel3,variavel4,variavel5,CON0,outro)	
	
	if isnull(un_bairro) or un_bairro="" then	
	else	
...
if periodo_check = 1 then
	if tp_modelo="B" then
		no_periodo = "Bimestre "&periodo_check
	else
		no_periodo = "Trimestre "&periodo_check		
	end if			
...
>
  <tr> 
    ...
    <td width="824" bgcolor="#FFFFFF"><table width="100%" border="0" align="right" cellspacing="0">
        <tr> 
          <td width="54%" rowspan="2"><font size="3" face="Verdana, Arial, Helvetica, sans-serif"><strong>ESCOLA 
                <%
			no_unidade= ucase(no_unidade)
			response.Write("&nbsp;"&no_unidade)
			%>
            </strong></font></td>
          <td width="46%" height="8" class="linhaBaixo">&nbsp; </td>
        </tr>
        <tr> 
          <td height="5" ></td>
        </tr>
  ...
  <tr> 
    <td height="12" colspan="2" bgcolor="#EEEEEE" class="linhaTopoL"><table width="912" border="0" cellspacing="0">
        ...
        <tr class="tabela"> 
          <td width="78" height="12" bgcolor="#EEEEEE"> <div align="right"><strong>Grupo:</strong></div></td>
          <td width="168" height="12" bgcolor="#EEEEEE"> 
            <%
response.Write(no_curso)%>
  ...
  <tr> 
	...
	CAMINHOn = CAMINHO_na
	Call boletim_avaliacao(CAMINHO_al,caminho_nota,unidade,curso,etapa,turma,co_materia,periodo_check,ano_letivo,cod,co_usr,"A","WFI","n","s","s",0)
	else
		if tb_nota="TB_NOTA_B" then
		CAMINHOn = CAMINHO_nb
		Call boletim_avaliacao(CAMINHO_al,caminho_nota,unidade,curso,etapa,turma,co_materia,periodo_check,ano_letivo,cod,co_usr,"B","WFI","n","s","s",0)
		else
			if tb_nota ="TB_NOTA_C" then
			CAMINHOn = CAMINHO_nc
			Call boletim_avaliacao(CAMINHO_al,caminho_nota,unidade,curso,etapa,turma,co_materia,periodo_check,ano_letivo,cod,co_usr,"C","WFI","n","s","s",0)
			else
				if tb_nota ="TB_NOTA_D" then
				CAMINHOn = CAMINHO_nd
				Call boletim_avaliacao(CAMINHO_al,caminho_nota,unidade,curso,etapa,turma,co_materia,periodo_check,ano_letivo,cod,co_usr,"D","WFI","n","s","s",0)
				else
					if tb_nota ="TB_NOTA_E" then
					CAMINHOn = CAMINHO_ne
					Call boletim_avaliacao(CAMINHO_al,caminho_nota,unidade,curso,etapa,turma,co_materia,periodo_check,ano_letivo,cod,co_usr,"E","WFI","n","s","s",0)
					else
						if tb_nota ="TB_NOTA_F" then
						CAMINHOn = CAMINHO_nf
						Call boletim_avaliacao(CAMINHO_al,caminho_nota,unidade,curso,etapa,turma,co_materia,periodo_check,ano_letivo,cod,co_usr,"F","WFI","n","s","s",0)
						else
						response.Write("ERRO")
						End if			
					End if			
