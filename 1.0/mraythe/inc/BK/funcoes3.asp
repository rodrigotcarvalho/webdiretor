...
Function pauta (CAMINHO_al,CAMINHOn,unidade,curso,etapa,turma,co_materia,periodo,ano_letivo,co_prof,opcao,subopcao,outro)
...
datas_formatado = buscaDataPauta(CAMINHOn, co_prof, unidade,curso,etapa,turma, mat_princ, co_materia, periodo, p_Vetor_Datas_Consulta, outro)
...
<table id="cabecalhoHorizontal">
 ...
 %>
        <td style="min-width: <%response.Write(width)%>px;width: <%response.Write(width)%>px;max-width: <%response.Write(width)%>px;" class="<%response.Write(classe_subtit)%>" align="center">
          <%response.Write(valor)%>
        </td>
        <%	next%>
        <td style="min-width: 200px;" class="<%response.Write(classe_subtit)%>" align="center">
          <%response.Write("Total de Faltas")%>
       </td>          
      </tr>    
...
%>        
<td  class="<%response.Write(classe_subtit)%>" align="center">
...
<%
	  ...
	  %>
        <td style="min-width: 40px;width: <%response.Write(width)%>px;max-width: <%response.Write(width)%>px;" class="<%response.Write(classe_subtit)%>" align="center">
          <%response.Write(nome_tempo)%>
        </td>  
      <%next
%>
</tr>
...
<%for c= 0 to ubound(linha_nome_colunas)
	  ...
	  for col=0 to ubound(seq_aula) 	
			 ...
			 %>
        <td style="min-width: <%response.Write(widthMin)%>px;width: <%response.Write(widthMin)%>px;max-width: <%response.Write(width)%>px;" align="center">&nbsp;
...
nu_matricula=nu_matricula*1	
	...
	'Verificando se algum aluno mudou de turma e inserindo uma linha cinza para o lugar do aluno
      ...
      <tr>
        <%for m= 0 to ubound(nome_variaveis)
							...
							conteudo="&nbsp;"
					 %>
        <td style="max-width: 55px;" class="<%response.Write(classe)%>" align="center">
          <%response.Write(conteudo)%>
        </td>
        <%next%>
        <td style="min-width: 198px;" class="<%response.Write(classe)%>" align="center">
          <%response.Write(wrkTotalFaltas)%>
        </td>        
      </tr>
      <%				
					next
	...
	for c= 0 to ubound(linha_nome_colunas)
	  ...
	  for col=0 to ubound(seq_aula) 	
			 ...
			 %>
        <td style="min-width: <%response.Write(widthMin)%>px;width: <%response.Write(widthMin)%>px;max-width: <%response.Write(width)%>px;" class="<%response.Write(classe)%>" align="center">
          <%response.Write(conteudo)%>
        </td>
        <%	next  
		next
...
<%Function pauta_bk (CAMINHO_al,CAMINHOn,unidade,curso,etapa,turma,co_materia,periodo,ano_letivo,co_prof,opcao,subopcao,outro)
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" bgcolor="#FFFFFF">
  <tr>
    <td><table width="<%response.Write(tableWidth)%>" border="0" cellspacing="0" cellpadding="0">
      <tr>
        <% for j= 0 to ubound(linha_nome_colunas)
		valor = linha_nome_colunas(j)
 		if j=0 then
		...
		elseif j=1 then	
			...
			seq_pauta = buscaSeqDataPauta(CAMINHOn, datas_pauta(j-2), co_prof, unidade,curso,etapa,turma, mat_princ, co_materia, periodo, outro)		
			vetorSeqAula=buscaSeqAula(CAMINHOn,seq_pauta, datas_pauta(j-2), outro)
			seq_aula=split(vetorSeqAula,"#!#")
			colspan=ubound(seq_aula)+1
			span = "colspan="&colspan&""
			width=width_else*colspan	
			data_form=replace(datas_pauta(j-2),"/",".")
			valor = "<input name=""data_form"" type=""checkbox"" value="""&data_form&""" />"&linha_nome_colunas(j)				
		end if		

					
 %>
        <td <%response.Write(span)%> width="<%response.Write(width)%>" class="<%response.Write(classe_subtit)%>"><div align="<%response.Write(align)%>">
          <%response.Write(valor)%>
        </div></td>
        <%	next%>
        <td rowspan="2" width="<%response.Write(width)%>" class="<%response.Write(classe_subtit)%>"><div align="<%response.Write(align)%>">
          <%response.Write("Total de<Br>Faltas")%>
        </div></td>          
      </tr>
      <tr>
      <% 
for j= 0 to ubound(linha_nome_colunas)
	if j>1 then	  
	  ...
	  %>
        <td width="<%response.Write(width_else)%>" class="<%response.Write(classe_subtit)%>"><div align="<%response.Write(align)%>">
          <%response.Write(nome_tempo)%>
        </div></td>      
      <%next
	  end if
...
nu_matricula=nu_matricula*1	
	...
	if c>1 then	  
		align="center"
		seq_pauta = buscaSeqDataPauta(CAMINHOn, datas_pauta(c-2), co_prof, unidade,curso,etapa,turma, mat_princ, co_materia, periodo, outro)		
		vetorSeqAula=buscaSeqAula(CAMINHOn,seq_pauta, datas_pauta(c-2), outro)
		seq_aula=split(vetorSeqAula,"#!#")	
	  for col=0 to ubound(seq_aula) 	
				Set RS3 = Server.CreateObject("ADODB.Recordset")
				SQL_N = "Select * from TB_Pauta_Faltas WHERE CO_Matricula = "& nu_matricula & " AND NU_Pauta = "& seq_pauta &" AND NU_Seq = "& seq_aula(col) 
...
%>
  ...
  <tr>
    <td><table width="1000" border="0" align="center" cellspacing="0">
            <tr>
              <td colspan="4"><hr /></td>
            </tr>
            <tr>
              ...
              <td width="25%" align="center"><input name="submit" type="submit" class="botao_cancelar" id="alterar"  value="Alterar" /></td>
              <td width="25%"><div align="center">
