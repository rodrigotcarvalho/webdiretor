...
conta_filhas = 0								
				while not RSFIL.EOF 	
					...
					'response.write(p_matric&" curso "&curso&" vetor_materia_simulado "&vetor_materia_simulado&"-"&conta_filhas&"<BR>")
				RSFIL.Movenext
				wend
			end if				
		end if
		p_matric=p_matric*1
'if p_matric = 100803 then 
'	response.end()	   
'end if		
...
'if p_matric = 100803 then 
'response.Write("Atualizando planilha de notas "&tb_nota&" CO_Materia "& co_materia_fil &" TP_Disciplina "&RSR("TP_Disciplina")&" valor_gravacao "&valor_gravacao&"<BR>")
'end if					
			if tb_nota="TB_NOTA_L" then
				va_t1 = RS3("VA_Teste1") 
				va_t2 = RS3("VA_Teste2")
