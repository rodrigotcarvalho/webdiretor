...
horario = hora & ":"& minwrt	
	...
	Set AssSecretaria = Doc.OpenImage( Server.MapPath( "../img/assinaturas/secretaria.jpeg") )
	Set AssDiretor = Doc.OpenImage( Server.MapPath( "../img/assinaturas/diretor.jpeg") )		
	...
	if curso=0 then
		...
		if gera_pdf="sim" then	
			...
			if bairro_unidade="" or isnull(bairro_unidade)then
				...
				SQLb = "SELECT * FROM TB_Bairros WHERE SG_UF='"& uf_unidade&"' AND CO_Municipio ="&municipio_unidade&" AND CO_Bairro="&bairro_unidade
			...
			SQL4 = "SELECT * FROM TB_Etapa WHERE CO_Etapa ='"& co_etapa &"' AND CO_Curso='"& curso &"'"
			RS4.Open SQL4, CON0
				...
				while not RS5.EOF
					...
					if co_materia_check=1 then
						...
						if in_mae=TRUE then
							...
							vetor_carga_materia=carga_materia	
...
'response.end()
			...
			vetor_carga_materia_exibe=Split(vetor_carga_materia,"#!#")										
			colunas_de_notas=18
			total_de_colunas=20					
			altura_medias=25
			...
			linha=1
			resultados=Calc_Med_An_Fin(unidade, curso, co_etapa, turma, vetor_matriculas, vetor_materia, caminho_nota, tb_nota, 4, 4, 0, "final", 0)		
...
'response.Write(dados_alunos(0)&"-"&resultado_final_aluno&"=<BR><BR>")
				...
				for n=0 to ubound(co_materia_exibe)	
					...
					'if (curso=1 and co_etapa<6 and (co_materia_verifica(n)="ART" or co_materia_verifica(n)="ESP" or co_materia_verifica(n)="ING")) or co_materia_verifica(n)="EFI"  then
					if (curso=1 and co_etapa<6 and (co_materia_verifica(n)="ART" or co_materia_verifica(n)="ESP")) or co_materia_verifica(n)="EFI"  then									
				...
				if calcula_frequencia="s" then
					...
					SQL_N = "Select * from "&tb_nota&" WHERE CO_Matricula = "& dados_alunos(0)
					Set RSF = CON_N.Execute(SQL_N)
					soma_faltas=0			
					
					if RSF.eof or dados_alunos(3)="E" THEN
						f1="&nbsp;"
						f2="&nbsp;"
						f3="&nbsp;"
						f4="&nbsp;"	
...
while not RSF.eof 
	...
	'						end if		
					...
					soma_faltas=soma_faltas*1
					carga_total=carga_total*1
					if carga_total=0 then
						carga_total=1					
					end if
					
					frequencia=(carga_total-soma_faltas)/carga_total
'					response.Write(frequencia&"="&carga_total&"-"&soma_faltas&"/"&carga_total&"<BR>")
'					response.End()					
...
'				Loop 
			...
			Set Param_AssSecretaria = Pdf.CreateParam	
		   Param_AssSecretaria("x") = margem + 80
		   Param_AssSecretaria("y") = y_declaracao-45
		   Param_AssSecretaria("ScaleX") = 0.25
		   Param_AssSecretaria("ScaleY") = 0.25
		   Page.Canvas.DrawImage AssSecretaria, Param_AssSecretaria

			y_declaracao=margem*4
				SET Param = Pdf.CreateParam("x="&margem&";y="&y_declaracao&"; height=10; width="&area_utilizavel&"; alignment=center; size=8; color=#000000; html=true")
				Text = "E, para constar, eu &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; Secret&aacute;rio(a), lavrei a presente ata que vai assinada pelo(a) Diretor(a) do Estabelecimento" 			
...
'				End With 
		   ...
		   Set Param_AssDiretor = Pdf.CreateParam	
		   Param_AssDiretor("x") = 350
		   Param_AssDiretor("y") = margem
		   Param_AssDiretor("ScaleX") = 0.25
		   Param_AssDiretor("ScaleY") = 0.25
		   Page.Canvas.DrawImage AssDiretor, Param_AssDiretor				
