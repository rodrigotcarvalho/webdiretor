...
Function Calcula_Media_T_F_F_N(unidade, curso, co_etapa, turma, tp_modelo, tp_freq, cod_aluno, codigo_materia_pr, codigo_materia, conexao , tb_nota, periodo, nome_nota, outro)
		...
		SQL1 = "SELECT "&nome_nota&" FROM "&tb_nota&" where CO_Matricula ="& cod_aluno &" AND CO_Materia_Principal ='"& codigo_materia_pr &"' AND CO_Materia ='"& codigo_materia &"'"
...
'=================================================
FUNCTION CALCULA_LINHA_BOLETIM(unidade, curso, co_etapa, turma, tp_modelo, tp_freq, cod_aluno, co_materia_mae, disciplina, CONn , tb_nota, p_vetor_periodo, nome_nota, PERIODO_ANUAL, PERIODO_RECUPERACAO, PERIODO_FINAL, P_TIPO_MATERIA, outro)

	vetor_linha_boletim = -1
	
	vetor_periodo= split(p_vetor_periodo,"#!#")	
	'response.write(P_TIPO_MATERIA&"("&unidade&","& curso&","&co_etapa&","&turma&","&tp_modelo&","&tp_freq&","&cod_aluno&","&co_materia_mae&","&disciplina&","&CONn&","&tb_nota&","&p_vetor_periodo&","&nome_nota&","&outro&")<br>")
	'response.end()
	for f2a=0 to ubound(vetor_periodo)
		periodo_cons=vetor_periodo(f2a)
		nome_nota=var_bd_periodo(tp_modelo,tp_freq,tb_nota,periodo_cons,"BDM")
		'response.write(co_materia(f2))		
		IF P_TIPO_MATERIA="T_F_F_N" THEN	
		...
		ELSEIF P_TIPO_MATERIA="T_T_F_N" THEN		
			...
			'response.write(P_TIPO_MATERIA&" "&media)
			'response.end()
		ELSE 
		...
		if not isnumeric(media) then
			...
			conceito_media=converte_conceito(unidade, curso, co_etapa, turma, periodo, co_materia_mae, media, outro)			
		...
		elseif periodo_cons<=PERIODO_ANUAL then
			vetor_linha_boletim = vetor_linha_boletim&"#!#"&conceito_media
		end if		
'	response.write(periodo_cons&" = "&PERIODO_ANUAL&" "&vetor_linha_boletim&"ok<br>")
		...
		if periodo_cons=PERIODO_ANUAL then
			conceito=converte_conceito(unidade, curso, co_etapa, turma, periodo, co_materia_mae, media, outro)							

			if calcula_media_anual="sim" and periodo_cons = PERIODO_ANUAL then
		
				media_calc1=Calc_Prim_Media (unidade, curso, co_etapa, turma, tp_modelo, tp_freq, cod_aluno, co_materia_mae, CONn, tb_nota, PERIODO_ANUAL, tipo_calculo, aproxima_m1, outro)	
				...
				if resultado_anual<>"&nbsp;" then
					tipo_media = "MA"
					modifica_result = Verifica_Conselho_Classe(cod_aluno, co_materia_mae, tipo_media, outro)
					if modifica_result <> "N" then
						resultado_anual = modifica_result
					end if		
				end if							
				media_anual = arredonda(media_anual,"mat",1,outro)	
				conceito_anual=converte_conceito(unidade, curso, co_etapa, turma, periodo, co_materia_mae, media_anual, outro)													
		...
		elseif periodo_cons=PERIODO_RECUPERACAO then									
			if media="" or isnull(media) or media_anual="" or isnull(media_anual) or resultado_anual = "APR" then
				conceito_recup=""
				resultado_recup=""
			else
				media_calc2=Calc_Seg_Media (unidade, curso, co_etapa, turma, tp_modelo, tp_freq, cod_aluno, co_materia_mae, CONn, tb_nota, PERIODO_RECUPERACAO, tipo_calculo, compara_m2, aproxima_m2, outro)
				...
				conceito_recup = converte_conceito(unidade, curso, co_etapa, turma, periodo, co_materia_mae, media_recup, outro)			
		...
		elseif periodo_cons=PERIODO_FINAL then									
			if media="" or isnull(media) or media_anual="" or isnull(media_anual)  or media_recup="" or isnull(media_recup) or resultado_anual = "APR" or resultado_recup="APR" then
				conceito_recup=""
				resultado_final=""							
			else
				media_calc3=Calc_Ter_Media (unidade, curso, co_etapa, turma, tp_modelo, tp_freq, cod_aluno, co_materia_mae, CONn, tb_nota, PERIODO_FINAL, tipo_calculo, compara_m3, aproxima_m3, outro)
				...
				conceito_final = converte_conceito(unidade, curso, co_etapa, turma, periodo, co_materia_mae, media_final, outro)			
