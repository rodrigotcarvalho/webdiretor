...
tb_nota_anterior = tb_nota
		
if co_materia = "RED2" AND (tb_nota = "TB_NOTA_F" or tb_nota = "TB_NOTA_K" or tb_nota = "TB_NOTA_M") THEN
	tb_nota = "TB_NOTA_V"
END IF		
	...
	elseif tb_nota="TB_NOTA_F" then	
			...
			ln_pesos_cols="&nbsp;#!#Pesos#!#&nbsp;#!#&nbsp;#!#PESO1#!#PESO3#!#PESO4#!#&nbsp;#!#&nbsp#!#&nbsp;#!#&nbsp;#!#&nbsp;"
	...
	elseif tb_nota="TB_NOTA_M" then					
			...
			ln_pesos_vars="&nbsp;#!#&nbsp;#!#&nbsp;#!#&nbsp;#!#&nbsp;#!#&nbsp;#!#&nbsp;#!#&nbsp;#!#&nbsp;#!#&nbsp;#!#&nbsp;#!#&nbsp;#!#&nbsp;#!#&nbsp;#!#&nbsp;#!#&nbsp;"			
			nm_pesos_vars="&nbsp;#!#&nbsp;#!#&nbsp;#!#&nbsp;#!#&nbsp;#!#&nbsp;#!#&nbsp;#!#&nbsp;#!#&nbsp;#!#&nbsp;#!#&nbsp;#!#&nbsp;#!#&nbsp;#!#&nbsp;#!#&nbsp;#!#&nbsp;"			
			ln_nom_cols="N&ordm;#!#Nome#!#AV1#!#AV2#!#AV3#!#AV4#!#AV5#!#MAV#!#SIM#!#BAT#!#BSI#!#M1#!#Bon#!#M2#!#REC#!#M3"
			nm_vars="av1#!#av2#!#av3#!#av4#!#av5#!#media_teste#!#simul_coord#!#bat_coord#!#bsi_coord#!#media1#!#bon#!#media2#!#rec#!#media3"
			nm_bd="VA_Av1#!#VA_Av2#!#VA_Av3#!#VA_Av4#!#VA_Av5#!#VA_Mav#!#VA_Sim#!#VA_Bat#!#VA_Bsi#!#VA_Media1#!#VA_Bonus#!#VA_Media2#!#VA_Rec#!#VA_Media3"
	...
	if gera_pdf="sim" then	
		...
		for a=0 to ubound(alunos_encontrados)	
			...
			if RSpeso.EOF then
				...
				if tb_nota<> "TB_NOTA_V" and tb_nota<> "TB_NOTA_K" and tb_nota<> "TB_NOTA_L" and tb_nota<> "TB_NOTA_M" then
					peso_teste=RSpeso("PE_Teste")
					if peso_teste="" or isnull(peso_teste) then
						va_pt=va_pt			
					else	
						va_pt=peso_teste						
					end if	
					...
					if tb_nota="TB_NOTA_F" then
						peso_prova1=RSpeso("PE_Prova1")		
						peso_prova2=RSpeso("PE_Prova2")		
						if peso_prova1="" or isnull(peso_prova1) then
							va_pp1=va_pp1				
						else	
							va_pp1=peso_prova1				
						end if	
						if peso_prova2="" or isnull(peso_prova2) then
							va_pp2=va_pp2				
						else	
							va_pp2=peso_prova2				
						end if																		
					else
						peso_prova=RSpeso("PE_Prova")	
						if peso_prova="" or isnull(peso_prova) then
							va_pp=va_pp				
						else	
							va_pp=peso_prova				
						end if					
...
'================================================================================================================			
		...
		tabela_col=1
			for d=0 to ubound(linha_pesos)
				if linha_pesos(d)="PESO1" then
					linha_pesos(d)=va_pt
				elseif linha_pesos(d)="PESO2" then
					linha_pesos(d)=va_pp
				elseif linha_pesos(d)="PESO3" then
					linha_pesos(d)=va_pp1
				elseif linha_pesos(d)="PESO4" then
					linha_pesos(d)=va_pp2										
				end if	
		...
		for b=0 to ubound(alunos_encontrados)	
			...
			for c=0 to ubound(nome_variaveis)	
				if RS3.EOF then 
					...
					if tb_nota = "TB_NOTA_V" and (variaveis_bd(c) = "VA_Media1" or variaveis_bd(c) = "VA_Bonus" or variaveis_bd(c) = "VA_Media2" or variaveis_bd(c) = "VA_Rec" or variaveis_bd(c) = "VA_Media3") then
						Set RS3F = Server.CreateObject("ADODB.Recordset")
						SQL_NF = "Select * from "&tb_nota_anterior&" WHERE CO_Matricula = "& dados_alunos(0) & " AND CO_Materia_Principal = '"& mat_princ &"' AND CO_Materia = '"& co_materia &"' AND NU_Periodo="&periodo
						Set RS3F = CON_NF.Execute(SQL_NF)	
						
						valor=RS3F(""&variaveis_bd(c)&"")															
					elseif variaveis_bd(c) <> "rs" and variaveis_bd(c)<> "rb" then
