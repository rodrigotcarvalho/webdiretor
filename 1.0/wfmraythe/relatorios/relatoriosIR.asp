...
if instr(1,request.ServerVariables("URL"),"/wf")>0 or instr(1,request.ServerVariables("URL"),"/testewf") then
	ano_IR=ano_IR-1
	...
	For i=0 to ubound(alunos_encontrados)	
		...
		if NOT RS1.EOF then
			...
			if gera_relatorio_aluno="s" then
				...
				length_co_user = Len(session("co_user"))
				fim_substring=length_co_user-2
				inicio_substring=length_co_user-1				
				cpf_formatado = Mid(session("co_user"),1,fim_substring)&"-"&Mid(session("co_user"),inicio_substring,length_co_user)
				
				cpf_formatado = string(12 - Len(cpf_formatado),"0") & cpf_formatado
				
				Set RSdn = Server.CreateObject("ADODB.Recordset")
				'SQLdn = "SELECT * FROM TB_Contatos WHERE CO_Matricula ="& cod_cons&" AND TP_Contato='"&responsavel_financ&"'"
				SQLdn = "SELECT * FROM TB_Contatos WHERE CO_Matricula ="& cod_cons&" AND CO_CPF_PFisica='"&cpf_formatado&"'"
				RSdn.Open SQLdn, CONCONT	
				nome_resp_financ=RSdn("NO_Contato")	
				cpf_resp_financ=RSdn("CO_CPF_PFisica")					
...
'		End With 						
				...
				size_assinatura = 15
				assinatura = "<center>Col&eacute;gio Maria Raythe</center>"
				if co_declaracao="EIR"  then
					...
					SQLQ = "SELECT * FROM TB_NF_Ano_Anterior WHERE NU_Ano = "&ano_IR&" AND CO_CPF = '"&cpf_formatado&"'  AND CO_Matricula  ="& cod_cons
					'response.write(SQLQ)
					'response.end()					
					RSQ.Open SQLQ, CONBL					
