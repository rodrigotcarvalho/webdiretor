...
Function GeraBoletos(nivel, dados, cod_matric, mes_solici, ucet, de, ate, tipo, restricao)
	GeraBoletos = GeraBoletosNN(nivel, dados, cod_matric, mes_solici, ucet, de, ate, tipo, restricao, "")
End function

Function GeraBoletosNN(nivel, dados, cod_matric, mes_solici, ucet, de, ate, tipo, restricao, nosso_numero)
...
p_nosso_numero = ""
ind_sempre_mensalidade="S"

if tipo="CEX" then
	tipo=""
end if	

if not isnull(nosso_numero) then
    p_nosso_numero = nosso_numero
    ind_sempre_mensalidade="N"
...
if nivel= 4 then
	...
	Set Itau = Doc.OpenImage( Server.MapPath( pastas&"img/logo_santander.png") )	
...
cod_cons = vetorMatriculas(l)
	...
	'		RS3.Open SQL3, CON0
		...
		if tipo="rematricula" then
			...
			Set RS4 = Server.CreateObject("ADODB.Recordset")
			SQL4= "SELECT * FROM TB_Posicao WHERE CO_Matricula_Escola ="& cod_cons &" AND Mes = "&wrk_mes						
			if tipo="EBP" or ind_sempre_mensalidade = "N" then
				if restricao = "S" then
					if isnumeric(p_nosso_numero) then
						sql_cond ="NO_Lancamento<>'Mensalidade' AND SQ_Bloqueto = "&p_nosso_numero
					else				
						sql_cond ="NO_Lancamento<>'Mensalidade'"
					end if	
				else
					sql_cond ="VA_Realizado=0 AND NO_Lancamento='Mensalidade'"			
				end if	
				SQL4= SQL4&" AND "&sql_cond 				
				if opcao="mat" then
        ...
        'response.Write(wrk_gera_boleto&"<BR>")		
			if wrk_gera_boleto = "S" then
				SQL1 = "SELECT * FROM TB_Bloqueto WHERE DA_Vencimento>=#"& vencimento_inicial &"# and DA_Vencimento<=#"& vencimento_final &"# AND CO_Matricula_Escola ="& cod_cons
				sql_cond =""
				IF ind_sempre_mensalidade = "N" then
					if isnumeric(p_nosso_numero) then
						sql_cond =" and SQ_Bloqueto = "&p_nosso_numero
					end if								
				END IF
	...
	'			var_instrucoes=	var_instrucoes&"Servi&ccedil;os Adicionais poder&atilde;o ser cobrados a parte conforme prevista cláusula V par&aacute;grafo primeiro do contrato.<BR>"&TEXTO_UCASE&"</font>" 
				var_instrucoes=	var_instrucoes&"Ap&oacute;s o vencimento pagar somente nas ag&ecirc;ncias do Banco Santander;"
					...
					Set param_table2 = Pdf.CreateParam("width="&width_tabela_2&"; height=200; rows="&rows_tabela_2&"; cols=7; border=1; cellborder=0.5; cellspacing=0;")																		
						...
						SmallTable3.At(2, 1).AddText co_agencia&" / "&nu_cedente&" ", " alignment=right; size=7;", Font	
						...
						SmallTable11.Rows(1).Cells(2).Width = 220								
						SmallTable11.Rows(1).Cells(3).Width = 20												
