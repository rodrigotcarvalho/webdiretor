...
Function Calcula_Media_T_F_T_N(unidade, curso, co_etapa, turma, tp_modelo, tp_freq, cod_aluno, codigo_materia_pr, vetor_materia, conexao, tb_nota, periodo, nome_nota, outro)	
	...
	SQL1a = "SELECT * FROM TB_Materia where CO_Materia_Principal ='"& codigo_materia_pr &"' order by NU_Ordem_Boletim"
	...
	if RS1a.EOF then
		...
		while not RS1a.EOF
			...
			SQL_N = "Select VA_Media3 from "& tb_nota &" WHERE CO_Matricula = "& cod_aluno &" AND CO_Materia = '"& co_mat_fil &"' AND CO_Materia_Principal = '"& codigo_materia_pr &"' AND NU_Periodo="&periodo
