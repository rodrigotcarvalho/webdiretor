	...
	if RS1.EOF then
		...
		if curso=0 then
			...
			if RS2.EOF then
				...
				dia_nascimento = vetor_nascimento(0)
				mes_nascimento = vetor_nascimento(1)	
				if dia_nascimento<10 then
				dia_nascimento="0"&dia_nascimento
				end if							
				if mes_nascimento<10 then
				mes_nascimento="0"&mes_nascimento
				end if
				
				nascimento = dia_nascimento &"/"& mes_nascimento &"/"& vetor_nascimento(2)				
			end if	
...
'================================================================================================================			
			...
			Set param_table = Pdf.CreateParam("width=50; height=72; rows=6; cols=8; border=0.5; cellborder=0; cellspacing=0;")
			Set Table = Doc.CreateTable(param_table)
			Page.Canvas.DrawTable Table, "x=500, y="&y_primeira_tabela&"" 		
...
'response.End()					
				...
				elseif (resultado_final_aluno="App" or resultado_final_aluno="APP") then
					'	legenda_resultado="<b>Resultado:</b>"
						resultado_exibe="Aprovado por Progress&atilde;o Parcial"
...
'				Legenda(1, 10).AddText "<b><Div align=""right"">Documento impresso em: "&data_exibe&"</div></b>", "size=8; html=true", Font 
			...
			y_obs_linha3=y_obs_linha2-16
				...
				End With 				
		'Data===========================================================================
			SET Param_data = Pdf.CreateParam("x=463;y=80; height=40; width=100; alignment=Left; size=8; color=#000000;html=true")
			data_preenche = "Niter&oacute;i, _____/_____/_____"
