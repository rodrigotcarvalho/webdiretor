...
response.Redirect("default.asp?opt=03")
	...
	grupo_usuario= tabela2("CO_Grupo")
		...
		SQLano = "SELECT MAX(NU_Ano_Letivo) AS max_ano_letivo FROM TB_Ano_Letivo where ST_Ano_Letivo='L'"
		RSano.Open SQLano, conexao

if RSano.EOF then
		Set RSano = Server.CreateObject("ADODB.Recordset")
		SQLano = "SELECT MAX(NU_Ano_Letivo) AS max_ano_letivo FROM TB_Ano_Letivo"
		RSano.Open SQLano, conexao
		
	ano_letivo=RSano("max_ano_letivo")
	...
	con_wf.Open ABRIR_wf
		...
		SQLanowf = "SELECT MAX(NU_Ano_Letivo) AS max_ano_letivo FROM TB_Ano_Letivo where ST_Ano_Letivo='L'"
		RSanowf.Open SQLanowf, con_wf

if RSano.EOF then
		Set RSanowf = Server.CreateObject("ADODB.Recordset")
		SQLanowf = "SELECT MAX(NU_Ano_Letivo) AS max_ano_letivo FROM TB_Ano_Letivo"
		RSanowf.Open SQLano, SQLanowf
		
	ano_letivo=RSanowf("max_ano_letivo")
	session("ano_letivo_wf") = ano_letivo
else
	ano_letivo=RSanowf("max_ano_letivo")
	session("ano_letivo_wf") = ano_letivo
end if
