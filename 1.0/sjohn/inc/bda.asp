...
'response.Write("if "&teste_1_lancado&"="no" and "&teste_2_lancado&"="no" and "&teste_3_lancado&"="no" and "&teste_4_lancado&"="no" then<BR>")
	...
	s_va_p=s_va_p1+s_va_p2+s_va_p3
	
	if prova_1_lancado="no" AND prova_2_lancado="no" AND prova_3_lancado="no" THEN
	media_p="no"
	mp=NULL		
	else
	media_p="ok"
	...
	if (co_materia_pr="EF" and co_materia="EF") or periodo>4 then
...
'		sql_atualiza= "UPDATE TB_Nota_A SET VA_Teste1 ="&sql_va_t1&", VA_Teste2 ="&sql_va_t2&", VA_Teste3 ="&sql_va_t3&", VA_Teste4 ="&sql_va_t4&", MD_Teste =FORMAT("&sql_mt&",2), "
