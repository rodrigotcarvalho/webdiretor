...
Function Calc_Prim_Media (unidade, curso, co_etapa, turma, tp_modelo, tp_freq, cod_aluno, codigo_materia, conexao, tb_nota, prd_prim_media, tipo_calculo, aproxima, outro)	
	...
	nome_nota=var_bd_periodo(tp_modelo,tp_freq,tb_nota,prd_prim_media,tp_dados)	
...
'response.Write(prd_prim_media&" "&soma_media&"<BR>")
	if soma_media = "&nbsp;" then
		calcula_media="n"
	else
		calcula_media="s"		
...
'response.Write(primeira_media&"#!#"&resultado&"<BR>")
...
Function Calc_Seg_Media (unidade, curso, co_etapa, turma, tp_modelo, tp_freq, cod_aluno, codigo_materia, conexao, tb_nota, prd_seg_media, tipo_calculo, compara, aproxima, outro)
	...
	nome_nota=var_bd_periodo(tp_modelo,tp_freq,tb_nota,prd_seg_media,tp_dados)	
	tp_materia=tipo_materia(codigo_materia, curso, co_etapa)	
	vetor_materia_filhas=busca_materias_filhas(codigo_materia)	
	arred_md = parametros_gerais(unidade, curso, co_etapa, turma, codigo_materia_pr,"arred_md",0)
	decimais_md = parametros_gerais(unidade, curso, co_etapa, turma, codigo_materia_pr,"decimais_md",0)		
	...
	prim_resultado=inf_primeira_media(1)
		response.write("prd_seg_media "&prd_seg_media&"<BR>")
	if prim_resultado = "APR" or prim_resultado="&nbsp;" then
		...
		var_bd=var_bd_periodo(tp_modelo,tp_freq,tb_nota,prd_seg_media,"BDM")		
		vetor_materia=busca_materias_filhas(codigo_materia)		
		response.write("var_bd "&var_bd&"<BR>")
...
''end if			
'			var_bd=var_bd_periodo(tp_modelo,tp_freq,tb_nota,prd_ter_media,"BDM")		
'			vetor_materia=busca_materias_filhas(codigo_materia)		
''response.Write(tb_nota&periodo&" 0<br />")			
'			if tp_materia="T_F_F_N" or tp_materia="F_T_F_N"	 or tp_materia="F_F_T_N" then
...
'Calc_Ter_Media=terceira_media&"#!#"&resultado
	...
	tb_nota=tabela_nota(session("ano_letivo"),unidade,curso,co_etapa,turma,"tb",0)
	caminho_nota=tabela_nota(session("ano_letivo"),unidade,curso,co_etapa,turma,"cam",0)

'response.write(cod_aluno&" "&tb_nota&" "&caminho_nota&"<BR>")


terceira_media=Calc_Med_An_Fin(unidade, curso, co_etapa, turma, cod_aluno, codigo_materia, caminho_nota, tb_nota, 4, 4, 0, "boletim", 0)

'response.write(cod_aluno&" "&codigo_materia&" "&terceira_media&"<BR>")		
'response.write(cod_aluno&" "&terceira_media&"#!#"&resultado&"<BR>")
'cod_aluno=cod_aluno*1
'if cod_aluno = 6834 then
'response.end()
'end if
'response.write(terceira_media&"<BR>")	
vetor_med_res = split(terceira_media,"#!#")

'Calc_Ter_Media=vetor_med_res(3)&"#!#"&vetor_med_res(4)
Calc_Ter_Media=vetor_med_res(0)&"#!#"&vetor_med_res(1)
end Function
...
'mostra_res_prim_media = parametro que indica que deve-se mostrar ou n�o disciplinas com resultado diferente de APR ou REP na primeira m�dia
'mostra_res_seg_media = parametro que indica que deve-se mostrar ou n�o disciplinas com resultado diferente de APR ou REP na segunda m�dia

'Valores de tipo_apuracao
'ata - Serve para a ata de resultados e para as declara��es
