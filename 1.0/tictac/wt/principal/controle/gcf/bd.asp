...
'============						
	...
	if b1_bolsa<>"nulo" or b2_bolsa<>"nulo" or b3_bolsa<>"nulo" then	
		...
		RS.Open sql_atualiza_bolsa, CON3
		...
		if bolsista="Sim" then				
			...
			Set RS = server.createobject("adodb.recordset")
			RS.open "TB_Contrato_Bolsas", CON3, 2, 2 'which table do you want open
			RS.addnew	
...
elseif opt="e" then
	...
	Set RS = Server.CreateObject("ADODB.Recordset")
	sql_atualiza= "DELETE * from TB_Compromissos where CO_Matricula = "&nu_matric&" AND NU_Ano_Letivo = "&ano_contrato&" AND NU_Contrato = "&contratos&" AND TP_Compromisso = '"&co_compromissos&"' AND NU_Parcela = "&nu_parcela&" AND NU_Cota = "&nu_cota&" AND NU_Sequencial = "&nu_sequencial
	RS.Open sql_atualiza, CON3
	
	call GravaLog (nvg,"Excluiu: "&concatena_contrato)
		
	NEXT		
response.Redirect("compromissos.asp?opt=ok")
