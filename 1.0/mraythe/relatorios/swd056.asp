...
'response.End()		
				...
				for m = 0 to ubound(resultado_aluno_por_materia)
					...
					if media_res_aluno_na_materia(1) = "REC" AND st_ano_letivo="L" then
				
						tipo_resultado="A"
						
					end if	
				next
					'if dados_alunos(0) = 20180062 then
				...
				resultado_final_aluno=apura_resultado_geral_aluno(curso, co_etapa, resultado_aluno,tipo_resultado)										
					'if dados_alunos(0) = 20180062 then
					'	response.Write(curso&", "&co_etapa&", "&resultado_aluno&", "&tipo_resultado&" -r")
