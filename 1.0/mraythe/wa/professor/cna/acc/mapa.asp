...
//Essa Fun��o funciona apenas dentro de DETALHE.ASP
...
resultado_c = resultado_c.replace(/\+/g," ")
resultado_c = unescape(resultado_c)
//Essa DIV est� dentro de DETALHE.ASP
...
//o 120 e o 16 se referem ao tamanho de cabe�alho do navegador e a barra de rolagem respectivamente
...
<%	next%>  
	...
	<%
		for na=0 to ubound(n_alunos)
			...
			if right(nome_aluno,10)=" - Inativa" then
...
<td width="<%response.Write(width_nu_chamada)%>" <%response.Write(rowspan)%>  onclick="mudar_cor_focus(<%response.Write(parametros_chamada_jscript)%>);chamajavascript(<%response.Write(num_cham)%>,<%response.Write(cod_cons)%>,'<%response.Write(no_exibe_per)%>','<%response.Write(vet_co_materia(0))%>',<% response.Write(periodo_real)%>)" onMouseOut="<%response.Write(onblur)%>(<%response.Write(parametros_chamada_jscript)%>)"><div align="center"><%response.Write(num_cham)%></div></td>
				...
				<%else%>   
                      <tr class="<%response.Write(cor)%>" id="<%response.Write("celula"&num_cham&"p"&seq_per)%>">       
                        <td width="<%response.Write(width_periodo)%>" onClick="mudar_cor_focus(<%response.Write(parametros_chamada_jscript)%>);recuperarImgTbAluno(<%response.Write(num_cham)%>,<%response.Write(cod_cons)%>,'<%response.Write(no_exibe_per)%>')" onMouseOut="<%response.Write(onblur)%>(<%response.Write(parametros_chamada_jscript)%>)"><div align="center"><%response.Write(no_exibe_per)%></div></td>
                        <%For dsc=0 to ubound(vet_co_materia)	%>	
                         <td width="<%response.Write(width_else)%>" onClick="mudar_cor_focus(<%response.Write(parametros_chamada_jscript)%>);chamajavascript(<%response.Write(num_cham)%>,<%response.Write(cod_cons)%>,'<%response.Write(no_exibe_per)%>','<%response.Write(vet_co_materia(dsc))%>',<% response.Write(periodo_real)%>)" onMouseOut="<%response.Write(onblur)%>(<%response.Write(parametros_chamada_jscript)%>)">
                         <div align="center">
                        <%
...
if transicao = "S" then
	url = "http://simplynet2.tempsite.ws/"
else
	url = "http://mr.simplynet.com.br/"
end if
if left(ambiente_escola,5)="teste" then
	pasta_ambiente="ateste/"
else
	'pasta_ambiente="wd"
end if
	hora = DatePart("h", now) 
  ...
  xmlhttp.open "GET",url&pasta_ambiente&ambiente_escola&"/wa/professor/cna/acc/alunos.xml?t="&seg&min&hora,false
