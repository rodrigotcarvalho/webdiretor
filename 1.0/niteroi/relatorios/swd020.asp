...
CONEXAOA = "Select TB_Matriculas.CO_Matricula, TB_Matriculas.NU_Chamada, TB_Alunos.NO_Aluno, TB_Matriculas.CO_Situacao, TB_Matriculas.NU_Unidade, TB_Matriculas.CO_Curso, TB_Matriculas.CO_Etapa, TB_Matriculas.CO_Turma from TB_Matriculas, TB_Alunos WHERE TB_Matriculas.NU_Ano="& ano_letivo &" AND TB_Matriculas.CO_Matricula=TB_Alunos.CO_Matricula AND TB_Matriculas.CO_Matricula = "&cod_cons&" order by TB_Matriculas.NU_Chamada"
...
'else
	...
	CAMINHOn=tabela_nota(ano_letivo,unidade,curso,co_etapa,turma,"cam",0)	
	opcao=tabela_nota(ano_letivo,unidade,curso,co_etapa,turma,"opt",0)	
		...
		ln_nom_cols=verifica_dados_tabela(opcao,"bol_av_col",outro)
		nm_vars=verifica_dados_tabela(opcao,"bol_av_wrk_var",outro)
		nm_bd=verifica_dados_tabela(opcao,"bol_av_bd_var",outro)
		vars_calc=verifica_dados_tabela(opcao,"bol_av_calc",outro)
...
'		end if	
		...
		texto_aluno = "&nbsp;&nbsp;&nbsp;&nbsp;-&nbsp;&nbsp;&nbsp;&nbsp;<b>Matr&iacute;cula:</b> "&nu_matricula&"&nbsp;&nbsp;&nbsp;&nbsp;-&nbsp;&nbsp;&nbsp;&nbsp;<b>Aluno:</b> "&nome_aluno&"&nbsp;&nbsp;&nbsp;&nbsp;-&nbsp;&nbsp;&nbsp;&nbsp;<b>Chamada:</b> "&nu_chamada
		texto_periodo = "&nbsp;&nbsp;&nbsp;&nbsp;-&nbsp;&nbsp;&nbsp;&nbsp;<b>Per&iacute;odo:</b> "&no_periodo
		mensagem_cabecalho=ano_letivo&"&nbsp;&nbsp;&nbsp;&nbsp;-&nbsp;&nbsp;&nbsp;&nbsp;"&no_curso&texto_turma&texto_aluno&texto_periodo
...
'CABEÇALHO==========================================================================================		
		...
		Text = "<center><i><b><font style=""font-size:18pt;"">RELAT&Oacute;RIO DE AVALIA&Ccedil;&Atilde;O</font></b></i></center>"
...
'================================================================================================================			
		colunas_de_notas=ubound(nome_variaveis)+3
		...
		Notas_Tit.Font = Font				
		largura_colunas=(area_utilizavel-200)/colunas_de_notas		
...
'	response.Write(e&":"&tabela_col&":"&linha_nome_colunas(e)&"<BR>")						
			tabela_col=tabela_col+1
		next			
...
''Se os números de chamada estiverem completos. Se não faltar aluno na turma.
		...
		SQL_A = "SELECT * FROM TB_Programa_Aula where CO_Etapa ='"& co_etapa &"' AND CO_Curso ='"& curso &"' order by NU_Ordem_Boletim "
		RS.Open SQL_A, CON0
		...
		While Not RS.EOF
			...
			no_materia = replace_latin_char(no_materia,"html")	
			...
			if mat_princ="" or isnull(mat_princ) then
				...
				linha=linha+1
				param_materias.Add "indentx=5; html=true"				
				Set Row = Notas_Tit.Rows.Add(15) ' row height						
				...
				if RS3.EOF then 
					...
					co_usuario = RS3("CO_Usuario")
					
					if co_usuario="" or isnull(co_usuario) then
						no_usuario="&nbsp;"
					else
							Set RS_pro = Server.CreateObject("ADODB.Recordset")
							SQL_pro = "SELECT * FROM TB_Usuario WHERE CO_Usuario="& co_usuario
					...
					no_usuario = replace_latin_char(no_usuario,"html")	
				...
				for c=0 to ubound(nome_variaveis)	
					if RS3.EOF then 
						if variaveis_bd(c) = "TX_Avalia" then
							valor="Sem Lan&ccedil;amento"						
						else
							valor="&nbsp;"
						end if	
					else
				...
				'response.Write(c&":"&coluna&"<BR>")				
					
	
				next	
				coluna=coluna+1								
				Notas_Tit(linha, coluna).AddText "<div align=""center"">"&no_usuario&"</DIV>", param_materias	
				
				coluna=coluna+1						
				Notas_Tit(linha, coluna).AddText "<div align=""center"">"&data_exibe&"</DIV>", param_materias					
			RS.MOVENEXT
			WEND
...
'================================================================================================================				 
		...
		Relatorio = "SWD020 - Sistema Web Diretor"
		Do While Len(Relatorio) > 0
			CharsPrinted = Page.Canvas.DrawText(Relatorio, Param_Relatorio, Font )			
			If CharsPrinted = Len(Relatorio) Then Exit Do
			SET Page = Page.NextPage
