		...
		if (mae=TRUE and fil=FALSE and in_co=FALSE and isnull(peso)) or (mae=FALSE and fil=FALSE and in_co=TRUE)  then
			...
			if RS1.EOF then
				...
				if UBOUND(vetor_periodo)>0 then
					...
					va_ma2=RS1("VA_Mc2")
					f2=RS1("NU_Faltas_P2")
					if UBOUND(vetor_periodo)>1 then									
						va_m3=RS1("VA_Me3")				
						va_ma3=RS1("VA_Mc3")	
						f3=RS1("NU_Faltas_P3")
					else
...
for r=0 to ubound(resultados_materia)	
	...
	if tipo="F" then
		...
		if isnull(md_aluno) or md_aluno="" or md_aluno="&nbsp;" or md_aluno=" " then
			qtd_nula=qtd_nula+1
		else
			md_aluno=md_aluno*1	
		end if
		...
		if result_aluno_temp="rep" or result_aluno_temp="dep" then
			
		else
			result_aluno_temp=result_temp
		end if		
	elseif tipo="A" then	
		res_aluno=nota_materia(1)
		if isnull(res_aluno) or res_aluno="" or res_aluno="&nbsp;" or res_aluno=" " then
			result_temp="ECE"	
			qtd_nula=qtd_nula+1			
		else
		...
		if result_aluno_temp="REP" or result_aluno_temp="ECE" then
...
if (qtd_nula>0 and tipo="F") or (qtd_nula=(ubound(resultados_materia)+1) and tipo="A") then
	libera_resultado="n"
end if

if 	libera_resultado="s" then
	if result_aluno_temp="apr" or result_aluno_temp="APR" then
		apura_resultado_geral_aluno=res_apr
	elseif result_aluno_temp="rep"or result_aluno_temp="REP"  then
		apura_resultado_geral_aluno=res_rep
	elseif result_aluno_temp="dep" then	
		qtd_dep=qtd_dep*1
	...
	elseif result_aluno_temp="ECE" then	
			apura_resultado_geral_aluno=result_aluno_temp		
	end if	
else
