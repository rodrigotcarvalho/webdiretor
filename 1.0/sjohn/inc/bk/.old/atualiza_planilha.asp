...
response.write(p_cod_materia&" "&co_materia&" "&zera_media&" "&valor_gravacao&"<BR>")
if co_materia="BIO2" then
...
'Zerar as médias de simulado previamente lançadas para todas as disciplinas	

	Set RSR = Server.CreateObject("ADODB.Recordset")
	SQLR = "SELECT * FROM TB_Programa_Aula WHERE CO_Curso = '"&curso&"' AND CO_Etapa = '"&etapa&"' order by NU_Ordem_Boletim"
	RSR.Open SQLR, CON0
	...
	while not RSR.EOF
		...
		while not RS3.EOF											
			...
			if tb_nota="TB_NOTA_L" then
				...
				fail = calcula_medias_L(CONOTA, curso, etapa, co_materia, mat_princ, periodo, p_matric, va_t1, va_t2, va_t3, va_t4, va_p1, va_p2, "", va_bat, va_bon, va_rec)
			else
				va_av1 = RS3("VA_Av1") 
				va_av2 = RS3("VA_Av2")
				va_av3 = RS3("VA_Av3") 
				...
				fail = calcula_medias_M(CONOTA, curso, etapa, co_materia, mat_princ, periodo, p_matric, va_av1, va_av2, va_av3, va_av4, va_av5, "", va_bat, va_bsi, va_bon, va_rec)
			end if				
