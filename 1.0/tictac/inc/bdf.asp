...
obr=co_materia&"$!$"&nu_matricula&"$!$"&unidade&"$!$"&curso&"$!$"&etapa&"$!$"&turma&"$!$"&periodo&"$!$"&ano_letivo&"$!$"&co_prof&"$!$"&todos
...
if opt="cln" then
	...
	if todos = "S" then
		...
		SQLM = "Select * from TB_Matriculas WHERE NU_Ano="&ano_letivo&" AND NU_Unidade = "& unidade &" AND CO_Curso = '"& curso &"' AND CO_Etapa = '"& etapa &"' AND CO_Turma = '"& turma &"' AND CO_Situacao='C' order by NU_Chamada"
		Set RSM = CON_AL.Execute(SQLM)	
		
		while not RSM.EOF 
		
		wrk_matricula = RSM("CO_Matricula")
		
			Set RS0 = Server.CreateObject("ADODB.Recordset")
			CONEXAO0 = "Select * from "&tb&" WHERE CO_Matricula = "& wrk_matricula &" AND CO_Materia_Principal = '"& co_materia_pr &"' AND CO_Materia = '"& co_materia &"' AND NU_Periodo="&periodo
			...
			'response.Write("TESTE")
				...
				Set RS0 = Server.CreateObject("ADODB.Recordset")
				CONEXAO0 = "DELETE * from "&tb&" WHERE CO_Matricula = "& wrk_matricula &" AND CO_Materia_Principal = '"& co_materia_pr &"' AND CO_Materia = '"& co_materia &"' AND NU_Periodo="&periodo
				Set RS0 = CON.Execute(CONEXAO0)
