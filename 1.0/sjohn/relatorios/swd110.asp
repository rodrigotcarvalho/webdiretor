...
''response.Flush()										
	...
	IF RSDISC.EOF Then
			...
			while not RSDISC.EOF
				...
				if ind_tipo_materia <> "T_T_F_N" and ind_tipo_materia <> "T_F_T_N" then
					...
					'CABEÇALHO==========================================================================================		
						...
						Text = "<center><i><b><font style=""font-size:18pt;"">Rela&ccedil;&atilde;o de Alunos em Recupera&ccedil;&atilde;o  Trimestral</font></b></i></center>"
				...
				'			
						...
						for a=0 to ubound(alunos_encontrados)
							...
							SQLNN = "SELECT * FROM "&tabela&" where CO_Matricula ="& dados_alunos(1) &" AND CO_Materia ='"&co_mat_fil &"' And NU_Periodo="&periodo_form
							RSNN.Open SQLNN, CONn		
						
							if NOT RSNN.EOF Then
								if RSNN("VA_Media3")>=70 then
									show_aluno = "N"
								end if
							end if                                 
		...
		'Ensino Médio===================================================================================================================================================================
			while not RSDISC.EOF
				linha=2
				co_materia_filha= RSDISC("CO_Materia")	
				'response.write("__"&co_materia_filha&"___")
				ind_tipo_materia = tipo_materia(co_materia_filha, curso_form, etapa_form)
				'response.write(ind_tipo_materia&"<BR>")
				if ind_tipo_materia <> "F_F_T_N" and tp_materia<>"F_T_F_N" then
					SET Page = Doc.Pages.Add(595,842)
					mudar_pagina="S"            
					'CABEÇALHO==========================================================================================		
						Set Param_Logo_Gde = Pdf.CreateParam
					...
					'	
						...
						tabela = tabela_notas(CON2, unidade_form, curso_form, etapa_form, turma_form, periodo_form, co_materia_filha, outro)
						caminho_tabela_nota = caminho_notas(CON2, tabela, outro)

						SET Param_Relatorio = Pdf.CreateParam("x="&margem&";y="&altura_materia&"; height=50; width="&area_utilizavel&"; alignment=left; size=9; color=#000000")
						Materia = GeraNomesNovaVersao("D",co_materia_filha,variavel2,variavel3,variavel4,variavel5,CON0,outro)
						Do While Len(Materia) > 0
							CharsPrinted = Page.Canvas.DrawText(Materia, Param_Relatorio, Font )
							If CharsPrinted = Len(Materia) Then Exit Do
							SET Page = Page.NextPage
...
'===============================
							...
							var_bd=var_bd_periodo(tp_modelo,tp_freq,tabela,periodo_form,"BDM2")
							vetor_materia=busca_materias_filhas(co_materia_filha)
							'response.write("++++"&co_materia_filha&","&ind_tipo_materia&"<BR>" )
							if ind_tipo_materia="T_F_F_N" then
								media_recup=Calcula_Media_T_F_F_N(unidade_form, curso_form, etapa_form, turma_form, tp_modelo, tp_freq, dados_alunos(1), co_materia_filha, vetor_materia, CONn, tabela, periodo_form, var_bd, outro)
								
							elseif ind_tipo_materia="T_T_F_N" then
								media_recup=Calcula_Media_T_T_F_N(unidade_form, curso_form, etapa_form, turma_form, tp_modelo, tp_freq, dados_alunos(1), co_materia_filha, vetor_materia, CONn, tabela, periodo_form, var_bd, outro)		
									
							elseif ind_tipo_materia="T_F_T_N" then	
								media_recup=Calcula_Media_T_F_T_N(unidade_form, curso_form, etapa_form, turma_form, tp_modelo, tp_freq, dados_alunos(1), co_materia_filha, vetor_materia, CONn, tabela, periodo_form, var_bd, outro)					
								'response.write(media_recup&"<BR>")
							end if											
				
							if isnumeric(media_recup) then
								media_recup = media_recup*1
								if media_recup>=70 then
									show_aluno = "N"
								end if
							end if
'====================================		                                 
