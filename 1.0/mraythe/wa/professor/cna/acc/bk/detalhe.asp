...
idade=calcula_idade(ano_a,mes_a,dia_a) 
	...
	if RS.EOF then
		nome_situac="detalhe.asp#42 - Erro na busca por TX_Descricao_Situacao."	
	else
...
vetor_arquivos=Session("vetor_fotos")
...
Anos</td>
      ...
      <tr>
        <td height="110" valign="bottom"><div align="center"><input name="button" type="button" class="botao_cancelar" id="button" value="Retornar" onClick="MM_showHideLayers('fundo','','hide','lupa','','hide');focar('<%response.Write(num_cham&"c2")%>');mudar_cor_focus(<%response.Write(parametros_chamada_jscript)%>);limpalupa();"/></div>
...
cor = "tb_fundo_linha_par" 
		...
		for n=0 to ubound(nom_periodo)
%> 
      <tr class="<%response.Write(cor)%>">         
        
        <td width="<%response.Write(width_periodo)%>" class="zoom_texto"><div align="center"><%response.Write(nom_periodo(n))%></div></td>            
        
        <%For dsc=0 to ubound(vet_co_materia)	%>	
        <td width="<%response.Write(width_else)%>" class="zoom_texto" onFocus="recuperarNotaZoom(<% response.Write(width_tabela)%>,<% response.Write(cod_cons)%>,<% response.Write(ano_letivo)%>,<% response.Write(curso)%>,<% response.Write(co_etapa)%>,'<%response.Write(vet_co_materia(dsc))%>','<% response.Write(caminho_ajax)%>','<% response.Write(opcao)%>',<% response.Write(num_periodo(n))%>)"><div align="center">
          <%
