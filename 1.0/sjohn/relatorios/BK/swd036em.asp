...
Set RS3 = Server.CreateObject("ADODB.Recordset")
SQL3 = "SELECT * FROM TB_Cabecalhos WHERE CO_Documento=3" 
RS3.Open SQL3, CON7		
...
IF NOT RS3.EOF then
	...
	tit = "<font style=""font-size:12pt;""><b>"&cabec_1&"</b></font><br><font style=""font-size:8pt;"">	"&cabec_2&"<br>"&cabec_3&"<br>"&cabec_4&"<br>"&cabec_5&"<br>"&cabec_6&"<br>"&cabec_7&"<br>"&cabec_8&"<br><font style=""font-size:18pt;"">Hist&oacute;rico Escolar - Ensino M&eacute;dio</font>"																				
end if
...
Text = "<p><center><i>"&tit&"</i></center></p>"
...
altura_primeiro_separador= y_texto -altura
With Page.Canvas
...
altura_segundo_separador= altura_primeiro_separador -50
With Page.Canvas
...
Table(2, 2).AddText formata(nascimento,"DD/MM/YYYY")&" &nbsp;&nbsp;&nbsp;&nbsp; Cidade: "&cid_nat&" &nbsp;&nbsp;&nbsp;&nbsp; Estado: "&uf_nat, "size=8;html=true", Font 							
...
Set param_table2 = Pdf.CreateParam("width=533; height="&altura_medias&"; rows="&wrk_row&"; cols=7; border=1; cellborder=0.1; cellspacing=0;")
...
wrkEtapaAnoHistorico = etapaAnoHistorico ("EM","S")
...
nomDisciplinas = HistoricoDisciplinas (cod_cons, "EM")
...
for anos = 0 to ubound(vetorAnos)
	...
	if not isnull(seqAno) and seqAno<>"" then					
		Set RSAN = Server.CreateObject("ADODB.Recordset")
		SQLAN = "SELECT DA_Ano, NU_Carga_Horaria, TX_Frequencia FROM TB_Historico_Nota WHERE CO_Matricula="&cod_cons&" AND DA_Ano="&seqAno
		RSAN.Open SQLAN, CON7	
	end if					
...
'	response.Write(vetorDisciplinas(d)&"-"&seqAno&"-"&coluna_disc&"-"&wrkCargaHoraria&"<BR>")														
...
cargaAno = totaisHistorico (cod_cons, "EM", "C")			
vetorWrkcargaAno = split(wrkcargaAno,"#!#")
vetorCargaAno = split(cargaAno,"#!#")	
coluna=2

for anos = 0 to ubound(vetorCargaAno)
	cargaExibe = 0

	if vetorCargaAno(anos)="" or isnull(vetorCargaAno(anos)) then
			if vetorWrkcargaAno(anos)=0 then
				cargaExibe = ""
			else
				cargaExibe = vetorWrkcargaAno(anos)
			end if						

	else
		if vetorCargaAno(anos)=0 then	
			if vetorWrkcargaAno(anos)=0 then
				cargaExibe = ""
			else
				cargaExibe = vetorWrkcargaAno(anos)
...
freqAno = totaisHistorico (cod_cons, "EM", "F")
...
AnoEscola = tbHistoricoAnoEscola (cod_cons, "EM", NULL)
...
for a = 0 to anos
	...
	serie = etapaHistorico ("EM",vetorAno(1),"A")
...
next		
wrkObsHistorico = observacaoHistorico (cod_cons, "EM")			
...
y_data = y_linha+20
'Data===========================================================================
 SET Param_data = Pdf.CreateParam("x="&margem&";y="&y_data&"; height=40; width=300; alignment=Left; size=8; color=#000000")
