...
'response.end()	
	...
	'	response.Write(periodo_max(Ubound(periodo_max ))&"<BR>")		
		...
		p=p*1
			...
			sigla_periodo=RS0("SG_Periodo")
			if p=3 then
				per_medias_turma=temp_nomes_periodos&"#!#"&sigla_periodo
				temp_nomes_periodos=per_medias_turma&"#!#Total"
			elseif p=4 then
				temp_nomes_periodos=temp_nomes_periodos&"#!#"&sigla_periodo&"#!#Total#!#M&eacute;dia<br>Final#!#PA1#!#PA2#!#PA3#!#PA1#!#PA2#!#PA3#!#Total"
'			elseif p=5 then
'				temp_nomes_periodos=temp_nomes_periodos&"#!#"&sigla_periodo&"#!#M&eacute;dia<br>Final#!#"&per_medias_turma&"#!#"&per_medias_turma				
			else
...
'response.End()
			...
			qtd_medias_materia = ubound(medias_materia)-1						
...
'	END IF	
	...
	CONEXAOA = "Select TB_Matriculas.CO_Matricula, TB_Matriculas.NU_Chamada, TB_Matriculas.CO_Situacao, TB_Alunos.NO_Aluno from TB_Matriculas, TB_Alunos WHERE TB_Matriculas.NU_Ano="& ano_letivo &" AND TB_Matriculas.CO_Matricula=TB_Alunos.CO_Matricula "&SQL_UNIDADE&SQL_CURSO&SQL_ETAPA&SQL_TURMA&SQL_ATIVO&" order by "&order	
	...
	While Not RSA.EOF
		...
		if situacao<>"C" then
			...
			CONEXAOST = "Select * from TB_Situacao_Aluno WHERE CO_Situacao='"& situacao &"'"	
			Set RSST = CON_0.Execute(CONEXAOST)		
			
			IF not RSST.eof THEN
				wrk_nome_situacao = " - "&RSST("TX_Descricao_Situacao")
			else
				wrk_nome_situacao = ""	
			END IF
			
			nome_aluno=nome_aluno&wrk_nome_situacao
		end if
...
'				'elseif vetor_periodo(v)="FINAL" then	
			...
			for v=0 to ubound(vetor_periodo)
				if vetor_periodo(v)="TRI1" then	
					temp_num_periodo=1
					periodo_exibe=vetor_periodo(v)
				elseif vetor_periodo(v)="TI2" then	
					temp_num_periodo=temp_num_periodo&"#!#2#!#0"
					periodo_exibe=periodo_exibe&"#!#"&vetor_periodo(v)&"#!#QF1"
				elseif vetor_periodo(v)="TRI3" then	
					temp_num_periodo=temp_num_periodo&"#!#3#!#0#!#0"
...
Function grava_ACC(unidade, curso, co_etapa, turma, periodo, acumulado, qto_falta, nota_m1, nota_m2, nota_m3, peso_m2_m1, peso_m2_m2, peso_m3_m1, peso_m3_m2, peso_m3_m3, ult_matric_gerada)
Server.ScriptTimeout = 1800 'valor em segundos
Session.Timeout=30
	ult_matric_gerada = ult_matric_gerada*1
	ano = DatePart("yyyy", now)
	mes = DatePart("m", now) 
	dia = DatePart("d", now) 
	hora = DatePart("h", now) 
...
'	nome_periodo=split(vetor_nom_periodos,"#!#")
	...
	n_alunos= split(alunos_vetor,"#$#")			
		...
		for al=0 to ubound(n_alunos)
			...
			cod_cons=aluno(0)
			cod_cons = cod_cons*1
			if cod_cons=ult_matric_gerada then
				grava_dados = "S"
			end if
			if grava_dados = "S" then						
...
Function ACC(unidade, curso, co_etapa, turma, cod_aluno, codigo_materia, caminho_nota, tb_nota, no_periodo, periodo, periodo_m1, periodo_m2, periodo_m3, nota_m1, nota_m2, nota_m3, peso_m2_m1, peso_m2_m2, peso_m3_m1, peso_m3_m2, peso_m3_m3)
	...
	if mae=TRUE and fil=FALSE and in_co=FALSE and isnull(peso) then
		if no_periodo<>"QF1" and no_periodo<>"QF2" and no_periodo<>"MA" and no_periodo<>"MF" then	
			...
			periodo_qf=periodo_m1-1
				...
				if media_qf<0 or media_qf="" or isnull(media_qf)then
					...
					media=arredonda(media_qf,"quarto_dez",1,0)				
				end if
				...
				if resultado="&nbsp;#!#&nbsp;" then
					...
					if media_qf(0)>=nota_m1 then
						...
						if media<0 then
							...
							media=arredonda(media,"quarto_dez",1,0)
						end if
					end if	
				end if
			'media=""
	...
	elseif mae=TRUE and fil=TRUE and in_co=FALSE and isnull(peso) then
		...
		if no_periodo<>"QF1" and no_periodo<>"QF2" and no_periodo<>"MA" and no_periodo<>"MF" then		
			media=Calcula_Media_T_T_F_N(unidade, curso, co_etapa, turma, cod_aluno, vetor_materia, caminho_nota, tb_nota, periodo)	
		else
			acumula_media=0
			if no_periodo="QF1" then
			periodo_qf=periodo_m1-1
				for periodo=1 to periodo_qf
					qf=Calcula_Media_T_T_F_N(unidade, curso, co_etapa, turma, cod_aluno, codigo_materia, caminho_nota, tb_nota, periodo)	
	...
	elseif mae=TRUE and fil=TRUE and in_co=FALSE then
		...
		if no_periodo<>"QF1" and no_periodo<>"QF2" and no_periodo<>"MA" and no_periodo<>"MF" then		
			media=Calcula_Media_T_T_F(unidade, curso, co_etapa, turma, cod_aluno, vetor_materia, caminho_nota, tb_nota, periodo)		
		else
			acumula_media=0
			if no_periodo="QF1" then
			periodo_qf=periodo_m1-1
				for periodo=1 to periodo_qf
					qf=Calcula_Media_T_T_F(unidade, curso, co_etapa, turma, cod_aluno, codigo_materia, caminho_nota, tb_nota, periodo)	
					'response.Write(acumula_media&"p"&periodo&"<br>")
					if qf="&nbsp;" or qf="" or isnull(qf) then
						acumula_media=acumula_media
					else
	...
	elseif(mae=TRUE and fil=FALSE and in_co=TRUE and isnull(peso)) then		
		...
		if no_periodo<>"QF1" and no_periodo<>"QF2" and no_periodo<>"MA" and no_periodo<>"MF" then		
			media=Calcula_Media_T_F_T_N(unidade, curso, co_etapa, turma, cod_aluno, codigo_materia, caminho_nota, tb_nota, periodo)				
		else
			acumula_media=0
			if no_periodo="QF1" then
			periodo_qf=periodo_m1-1
			...
			'media=""
			elseif no_periodo="MA" then	
'			response.Write(	unidade&"-"&curso&"-"&co_etapa&"-"&turma&"-"&cod_aluno&"-"&codigo_materia&"-"&caminho_nota&"-"&tb_nota)
				resultado=Calc_Med_An_Fin(unidade, curso, co_etapa, turma, cod_aluno, codigo_materia, caminho_nota, tb_nota, 4, 4, 0,"anual", outro)						
				media_qf=split(resultado,"#!#")
				media=media_qf(0)
			elseif no_periodo="MF" then
