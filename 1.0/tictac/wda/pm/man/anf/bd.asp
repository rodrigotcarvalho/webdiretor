		...
		if isnull(item_fornecedor_n) or item_fornecedor_n="" then
		    ...
		    if item_fornecedor_n <> "nulo" then
				...
				if RSC.EOF then
				   ...
				   qtd_atual = RSC("QT_Atual")	
				   valor_atual = RSC("VA_Medio_Refer")  				      
				   if isnull(qtd_atual) or qtd_atual="" then
					  qtd_atual = 0
				   end if
				   
				   if isnull(valor_atual) or valor_atual="" then
					  valor_atual = 0
				   end if				   
				   ...
				   sql_atualiza="UPDATE TB_Item SET "
				   sql_atualiza=sql_atualiza & "QT_Atual=" & qtd_atual & ", "
		   		   sql_atualiza=sql_atualiza & "VA_Medio_Refer=" & valor_n & ", "	
				   sql_atualiza=sql_atualiza & "VA_Ult_Medio_Refer=" & valor_atual & ", "					   			   
				   sql_atualiza=sql_atualiza & "DA_Ult_Atua=#" & data_atualiza & "#"
				...
				response.Write("<BR>"&sql_atualiza)
					...
					if Err.number<>0 then
					 response.write("<BR>UPDATE: "&Err.Description)
					end if
				end if			
			end if
...
elseif opt="alt" then
   ...
   if conferido_por<>"nulo" then    
    sql_atualiza=sql_atualiza & ", CO_Usuario_Conf=" & conferido_por   
   end if      
...
'	response.Write(SQLC&"<BR>")		
		...
		if RSC.EOF then
		   ...
		   qtd_atual = RSC("QT_Atual")	
		   valor_anterior= RSC("VA_Ult_Medio_Refer")   		      
		   if isnull(qtd_atual) or qtd_atual="" then
			  qtd_atual = 0
		   end if
		   
		   if isnull(valor_anterior) or valor_anterior="" then
			  valor_anterior = 0
		   end if		   
		   ...
		   sql_atualiza="UPDATE TB_Item SET "
		   sql_atualiza=sql_atualiza & "QT_Atual=" & qtd_atual & ","
		   sql_atualiza=sql_atualiza & "VA_Medio_Refer=" & valor_anterior & ","		   
		   sql_atualiza=sql_atualiza & "DA_Ult_Atua=#" & data_atualiza & "#"
	...
	'response.Write(sql_atualiza&"<BR>")
				...
				if Err.number<>0 then
				 response.write("Update 1: "&Err.Description)
				end if
		end if					
		...
		if isnull(item_fornecedor_n) or item_fornecedor_n="" then
		    ...
		    if item_fornecedor_n <> "nulo" then
				...
				'response.Write("<BR>"&sql_atualiza)
					...
					if Err.number<>0 then
					 response.write("Update 2: "&Err.Description)
					end if
				end if						
			end if
		end if
