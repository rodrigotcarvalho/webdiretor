...
session("t_pub")=session("t_pub")
		...
		Set RSG0 = Server.CreateObject("ADODB.Recordset")
		SQLG0 = "SELECT DISTINCT CO_Materia FROM TB_Da_Aula where CO_Professor ="& prof_altera&" and CO_Etapa = '"&session("e_pub") &"' AND NU_Unidade = "&session("u_pub")&" and CO_Curso = '"&session("c_pub") &"' AND CO_Materia<> CO_Materia_Principal order by CO_Materia"
		RSG0.Open SQLG0, CONg
		exibe_select="N"
		conta_disciplinas=0
		while not RSG0.EOF
			co_mat_prin= RSG0("CO_Materia")
			if conta_disciplinas=0 then
				exibe_select="S"
				vetor_materias_mae = co_mat_prin 
			else
				vetor_materias_mae = vetor_materias_mae&"','"&co_mat_prin 			
			end if
		
			conta_disciplinas=conta_disciplinas+1
		RSG0.MOVENEXT
		WEND

		if exibe_select="S" then
			
%>
                    <select name="mat_prin" class="select_style" onChange="MM_callJS('recuperarPeriodo()')">
                        <option value="999999" selected></option> 
                        <%
			vtr = split(vetor_materias_mae,"','")
			For dsc=0 to ubound(vtr) 
				co_mat_prin=vtr(dsc)
...
%>
	...
	<%
    	...
    	response.Write("Sem Subdisciplina")
    end if
