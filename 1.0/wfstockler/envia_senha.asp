...
if opt="mail" then
	...
	if isnumeric(lg) then
		...
		response.Redirect("envia_senha.asp?opt=11")	
	end if	
...
'		
	mail_suporte=email_suporte_escola
'	mail_CC=RS1("Mail_Simplynet")		
'	nome_escola=RS2("NO_Escola")
	...
	objCDOSYSMail.HtmlBody = "<font size=2 face=Arial, Verdana, Courier New, Courier, mono>Prezado(a) "&nome&"<BR><BR><BR> Conforme sua solicitação lembramos que o usuário "&lg&" possui a senha "&senha&".<BR><BR><BR> Atenciosamente,<BR><BR><BR> Administração "&nome_escola&"</font>"
	objCDOSYSMail.Send
...
case 10
	...
	ti = session("ti")	
case 11
	msg="O campo Usuário deve ser numérico!"
	tipo="e"
	lg = ""
	mail= session("mail")	
	ti = session("ti")	
...
form_login.texto_imagem.focus(); 
<%elseif opt=09 or opt=10 or opt=12 then%>
form_login.mail.focus(); 
<%elseif opt=11 then%>
form_login.pas2.focus(); 
<%else%>
...
{
 if (isNaN(document.form_login.login.value))
 {
	 	alert("O Campo usuário deve ser numérico.")
	 return false
 }
    ...
    if(((document.form_login.mail.value.search(exclude) != -1)||(document.form_login.mail.value.search(check)) == -1)||(document.form_login.mail.value.search(checkend) == -1))
 {
	 	alert("E-mail é inválido. Favor digitar corretamente o endereço de e-mail.")
	 return false
 }
...
<form action="envia_senha.asp?opt=mail" method="post" name="form_login" id="form_login" onSubmit="return checksubmit()">
