...
'================================================================================================================			
	...
	tabela = tabela_notas(CON2, ucet(0), ucet(1), ucet(2), ucet(3), nulo, nulo, nulo)	
	
	caminho_n= caminho_notas(CON2, tabela, outro)
	...
	while not RSNN.EOF
		...
		if (co_materia="ARTC" and curso=1 and co_etapa<6) then
			...
			elseif in_mae=TRUE AND in_fil=FALSE AND in_co=FALSE then	
				wrk_tipo_materia="msf"	
			elseif in_mae=TRUE AND in_fil=FALSE AND in_co=TRUE then	
				wrk_tipo_materia="mcd"					
			elseif (in_mae=FALSE AND in_fil=TRUE) or (in_mae=FALSE AND in_co=TRUE) then
				wrk_tipo_materia="f"
			else				
	...
	RSNN.MOVENEXT
	wend
	'response.Write(vetor_materias&"<BR>")
...
response.Write(nu_matricula&"<BR>")					
		for q=0 to ubound(co_mat_cons)
			...
			if tipo_calculo = "MA" then		
				resultado=Calc_Med_An_Fin(ucet(0), ucet(1), ucet(2), ucet(3), nu_matricula, disciplina, caminho_n, tabela, 4, 4, 0,"anual", 0)	
				...
				if tp_disc="msf" then
					media_aluno=Calcula_Media_T_F_F_N(ucet(0), ucet(1), ucet(2), ucet(3), nu_matricula, disciplina, caminho_n, tabela, periodo)	
					'media_aluno=Calcula_Media_T_F_F_N(ucet(0), ucet(1), ucet(2), ucet(3), tp_modelo, tp_freq, nu_matricula, disciplina, disciplina, CON_N , tabela, periodo, "VA_Media2", outro)	
					...
					'response.Write("msf "&nu_matricula&"|"&disciplina&"|"&media_aluno&"|"&pula_aluno&"<BR>")			
				elseif	tp_disc="mcf" then
					...
					media_aluno=Calcula_Media_T_T_F_N(unidade, curso, co_etapa, turma, nu_matricula, disciplina, vetor_filhas, caminho_n, tabela, periodo)	
					'media_aluno=Calcula_Media_T_T_F_N(ucet(0), ucet(1), ucet(2), ucet(3), tp_modelo, tp_freq, nu_matricula, disciplina, vetor_filhas, CON_N, tabela, periodo, "VA_Media2", outro)	
					...
					'response.Write("mcf "&nu_matricula&"|"&disciplina&"|"&media_aluno&"|"&pula_aluno&"<BR>")
				elseif tp_disc="mcd" then
							...
							media_aluno=Calcula_Media_T_F_T_N(ucet(0), ucet(1), ucet(2), ucet(3), nu_matricula, disciplina, caminho_n, tabela, periodo)
							'Calcula_Media_T_F_T_N(ucet(0), ucet(1), ucet(2), ucet(3), tp_modelo, tp_freq, nu_matricula, disciplina, vetor_filhas, CON_N, tabela, periodo, "VA_Media2", outro)
						...
						response.Write("mcd "&nu_matricula&"|"&disciplina&"|"&media_aluno&"|"&pula_aluno&"<BR>")					
	...
	next
'response.Write(vetor_aluno_media)
'response.End()	
'faixa1=0
'faixa2=0
'faixa3=0
