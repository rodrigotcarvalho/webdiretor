...
if opt="wsa" or opt="wf"  then
	...
	SQL ="SELECT * FROM TB_Alunos INNER JOIN TB_Matriculas ON TB_Alunos.CO_Matricula=TB_Matriculas.CO_Matricula where TB_Matriculas.CO_Matricula ="& co_aluno &" and TB_Matriculas.NU_Ano="&ano_letivo
	RS.Open SQL, CON1	
	
	if RS.EOF Then
		Response.Write("Matrícula "&co_aluno&" não existe em TB_Matriculas em "&ano_letivo)
		response.End()
	else	
...
'response.Write(CONEXAOA)
		...
		While Not RSA.EOF
			...
			elseif nu_chamada_check = 1 then
				...
				vetor_matrics=vetor_matrics&"#!#"&nu_matricula
			end if
		nu_chamada_check=nu_chamada_check+1			
		RSA.MoveNext
		Wend 
...
'CABEÇALHO==========================================================================================		
		...
		if opt<>"wsa" and opt<>"wst" then 
			...
			y_texto=y_texto-altura_logo_gde+50					
		end if
		...
		altura_primeiro_separador= Page.Height - altura_logo_gde-45		
		...
		nu_chamada_ckq = 0
			...
			no_materia = Server.HTMLencode(no_materia)					 
				...
				for c=0 to notas_a_lancar	
					...
					if (c=0 and show_p1 = "N") or (c=1 and show_p2 = "N") or (c=2 and show_p3 = "N") or (c=3 and show_p4 = "N") or (c=4 and show_p5 = "N") or (c=5 and show_p6 = "N")  then
						valor="&nbsp;"
					end if															
