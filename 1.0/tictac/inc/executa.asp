...
<%
elseif opt="proj" then
etapa_altera=Request.Form("e_pub")

%>
<select name="projeto" class="select_style" id="projeto">
                        <option value="nulo" selected></option>
                        <%
		Set RSprjt = Server.CreateObject("ADODB.Recordset")
		sqprjt = "Select * From TB_Projeto where CO_Etapa = '"&etapa_altera&"' order by NO_Projeto"
		RSprjt.Open sqprjt, CON9  
		
		while not RSprjt.EOF 
		cod_pro=RSprjt("CO_Projeto")		
		nome_pro=RSprjt("NO_Projeto")		  
		
		%>
                        <option value="<%response.Write(cod_pro)%>">
                          <%response.Write(Server.URLEncode(nome_pro))%>
                        </option>
                        <%
		RSprjt.MOVENEXT
		WEND 		
    ...
    <td><table width="100%" border="0" cellspacing="0" cellpadding="0">
		...
		'response.Write(SQLP&"<BR>")						
                    ...
                    <% for cp=1 to conta_periodo %>
                      <td width="71" class="tb_subtit"><div align="center"> Avalia&ccedil;&otilde;es </div></td>
...
%>
                      <select name="curso" id="curso" class="select_style" id="curso" <%response.Write(onchange)%>>
...
%>
                      <select name="etapa" id="etapa" class="select_style" <%response.Write(onchange)%>>
                        <option value="999990" selected></option>
                        <%		
		Set RS0b = Server.CreateObject("ADODB.Recordset")
		SQL0b = "SELECT DISTINCT CO_Etapa FROM TB_Unidade_Possui_Etapas where NU_Unidade="&session("u_pub")&"AND CO_Curso='"&session("c_pub")&"'"
...
<%elseif Left(opt, 1)="t" then
	...
	if opt<>"t6" then
%>
    <select name="turma" id="turma" class="select_style" <%response.Write(onchange)%>>
                            <option value="999990" selected></option>
        <%
            Set RS3 = Server.CreateObject("ADODB.Recordset")
