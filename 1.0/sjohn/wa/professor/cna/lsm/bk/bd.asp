...
if grava="no" then
		...
		'response.Write(i&"-grava-"&nu_matricula&" va_sim "&va_sim&"<br>")	
		Set RS0 = Server.CreateObject("ADODB.Recordset")
		CONEXAO0 = "Select * from "&tb&" WHERE CO_Matricula = "& nu_matricula &" AND TP_Area = "&areadoconhecimento&" AND NU_Periodo="&periodo		
		Set RS0 = CON.Execute(CONEXAO0)
...
if submit<>"Salvar" then
	...
	While Not RSA.EOF
		nu_matricula = RSA("CO_Matricula")
			...
			While Not RSF.EOF
				areadoconhecimentoaatualizar =	RSF("TP_Area")
								
				atualizou = atualiza_planilha_simulado(opt,unidade, curso, etapa, turma, periodo, areadoconhecimentoaatualizar, nu_matricula, va_sim, outro)
