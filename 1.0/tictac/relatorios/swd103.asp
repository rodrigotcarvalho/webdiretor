...
'		nome_aluno =strReplacement
	...
	'CABEÇALHO==========================================================================================		
		   ...
		   Page.Canvas.DrawImage Logo, Param_Logo_Gde
			...
			Text = "<center><i><b><font style=""font-size:18pt;"">MAP&Atilde;O DE M&Eacute;DIAS ANUAIS</font></b></i></center>"
...
'response.end()
			...
			linha=2
			'resultados=Calc_Med_An_Fin(unidade, curso, co_etapa, turma, vetor_matriculas, vetor_materia, caminho_nota, tb_nota, 4, 4, 0, "anual", 0)	
...
'response.Write(dados_alunos(0)&"<BR>")
				...
				for n=0 to ubound(co_materia_exibe)	
					coluna=coluna+1	
'					response.Write(	dados_alunos(0)&"-"&co_materia_exibe(n)&"<BR>")
'					
'					response.Write(unidade&"-"&curso&"-"&co_etapa&"-"&turma&"-"&dados_alunos(0)&"-"&co_materia_exibe(n)&"-"&caminho_nota&"-"&tb_nota&"-"&prd_prim_media&"-"&tipo_calculo&"-"&outro&"<BR>")
...
'					response.Write("R:"&resultados&"<BR>")
			...
			limite=limite+1
			   LastRow = Page.Canvas.DrawTable( Notas_Tit, param_table2 )
				...
				if LastRow >= Notas_Tit.Rows.Count Then 
					...
					Relatorio = "SWD103 - Sistema Web Diretor"
