                ...
                if (exibe_notas(n)="0" or mostra_nota="s") then
					...
					if n=0 then	
						if conteudo="&nbsp;" or conteudo = "" or isnull(conteudo) then	
                    		...
                    		response.Write("<a class='alerta_normal' href='"&subpasta&"relatorios/swd016.asp?obr="&dados_relatorio&"'>Lançado</a>")
						end if	
					elseif variaveis_bd(n) = "DA_Ult_Acesso" then	
						if isnull(conteudo) or conteudo="" or conteudo="&nbsp;" then
						
						else
					
							dados_dtd= split(conteudo, "/" )
							dia_de= dados_dtd(0)
							mes_de= dados_dtd(1)
							ano_de= dados_dtd(2)
							dia_de=dia_de*1
