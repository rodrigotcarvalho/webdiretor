...
Set CON = Server.CreateObject("ADODB.Connection") 
		ABRIR = "DBQ="& CAMINHO_ne & ";Driver={Microsoft Access Driver (*.mdb)}"
		CON.Open ABRIR
...
'			Alterado por conta de e-mail de 11/05/2012
	...
	'response.Write(nu_matricula&" - "&va_t1&" - "&va_pt&" - "&va_pp&"<BR>")
		...
		CONEXAO0 = "Select * from TB_Nota_E WHERE CO_Matricula = "& nu_matricula &" AND CO_Materia_Principal = '"& co_materia_pr &"' AND CO_Materia = '"& co_materia &"' AND NU_Periodo="&periodo
	...
	If RS0.EOF THEN	
		...
		RS.open "TB_Nota_E", CON, 2, 2 'which table do you want open
		RS.addnew
		...
		Set RS0 = Server.CreateObject("ADODB.Recordset")
		CONEXAO0 = "DELETE * from TB_Nota_E WHERE CO_Matricula = "& nu_matricula &" AND CO_Materia_Principal = '"& co_materia_pr &"' AND CO_Materia = '"& co_materia &"' AND NU_Periodo="&periodo
		Set RS0 = CON.Execute(CONEXAO0)
				'response.Write(CONEXAO0&"<BR>")
...
outro="P:"&periodo&",D:"&co_materia&",U:"&unidade&",C:"&curso&",E:"&etapa&",T:"&turma&", Comunicou"
			call GravaLog (chave,outro)
response.Redirect(endereco_origem&"comunicar.asp?or=01&nota=TB_Nota_E&opt=ok&obr="&obr)
else
