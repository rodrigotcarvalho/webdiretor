...
<!--#include file="notas_calculos_diversos.asp"-->
...
dados_tabela=dados_planilha_notas(ano_letivo,unidade,curso,etapa,turma,co_materia,0,periodo,opcao,outro)
 ...
 <% for i= 0 to ubound(linha_pesos)
		...
		if linha_pesos(i)="PESO" then
			...
			Set RStemp = CON_AL.Execute(SQL_temp)
				...
				SQL_peso = "Select "&linha_pesos_variaveis(i)&" from "& tb &" WHERE CO_Matricula = "& nu_matriculatemp & " AND CO_Materia_Principal = '"& mat_princ &"' AND CO_Materia = '"& co_materia &"'"			
...
nu_matricula=nu_matricula*1	
	...
	'Inserindo o aluno seguinte aos que mudaram de turma				
			...
			<td width="<%response.Write(width_nom_aluno)%>" <%response.Write(classe_td_imp)%>><%response.Write(nome_aluno)%></td>               
				...
				SQL_N = "Select * from "& tb &" WHERE CO_Matricula = "& nu_matricula & " AND CO_Materia_Principal = '"& mat_princ &"' AND CO_Materia = '"& co_materia &"'"				
				...
				coluna=0	 
				 for n= 0 to ubound(nome_variaveis)
					...
					if (valor="" or isnull(valor)) and subopcao="imp" then
						...
						if nome_variaveis(n)="VA_Sapr1" or nome_variaveis(n)="VA_Sapr2" or nome_variaveis(n)="VA_Sapr3" or nome_variaveis(n)="VA_Sapr_EC" or nome_variaveis(n)="media_teste" or nome_variaveis(n)="media_prova" or nome_variaveis(n)="media1" or nome_variaveis(n)="media2" or nome_variaveis(n)="media3" then
							coluna=coluna	
							conteudo=valor
						elseif nome_variaveis(n)="VA_Me1" or nome_variaveis(n)="VA_Mc1" or nome_variaveis(n)="VA_Me2" or nome_variaveis(n)="VA_Mc2" or nome_variaveis(n)="VA_Me3" or nome_variaveis(n)="VA_Mc3" or nome_variaveis(n)="VA_Me_EC" or nome_variaveis(n)="VA_Mfinal" then	
							coluna=coluna	
							if (valor="" or isnull(valor)) then
								conteudo=""
							else
								conteudo=valor/10		
								conteudo=arredonda(conteudo,"mat_dez",1,0)													
							end if
						elseif calcula_variavel(n)="CALC1" and situac="C" then
							...
							if nu_matricula = errante then
								...
								conteudo="<input name='"&nome_campo&"' type='"&tipo&"' id='"&linha_tabela&"c"&coluna&"' onFocus=""javascript:this.form."&nome_campo&".select();"" value='"&valor&"' class=""nota"" size=""6"" maxlength=""5"" onkeydown=""keyPressed(this.id,event,"&notas_a_lancar&","&qtd_alunos&")"">"								
							
							elseif comunica="s" or subopcao="blq" or situac<>"C" then
								conteudo=valor&"<input name='"&nome_campo&"' type='"&tipo&"' id='"&linha_tabela&"c"&coluna&"' onFocus=""mudar_cor_focus(celula"&nu_chamada&");javascript:this.form."&nome_campo&".select();"" onBlur="&onblur&"(celula"&nu_chamada&") value='"&valor&"' class=""nota"" size=""6"" maxlength=""5"" onkeydown=""keyPressed(this.id,event,"&notas_a_lancar&","&qtd_alunos&")"">"		

							else
								conteudo="<input name='"&nome_campo&"' type='"&tipo&"' id='"&linha_tabela&"c"&coluna&"' onFocus=""mudar_cor_focus(celula"&nu_chamada&");javascript:this.form."&nome_campo&".select();"" onBlur="&onblur&"(celula"&nu_chamada&") value='"&valor&"' class=""nota"" size=""6"" maxlength=""5"" onkeydown=""keyPressed(this.id,event,"&notas_a_lancar&","&qtd_alunos&")"">"										
							end if
						end if	
					end if	
					'conteudo=n
