...
if RS_nota.EOF then
	...
	width_tb_dados_turma=width_nu_chamada+width_nome+width_periodo-50
	width_else=(width_tabela-width_nome-width_nu_chamada-width_periodo)/qtd_colunas
...
<div id="divImg">
   <div id="alinha" style="position:absolute; width:500px; z-index: 3; height: 536px; visibility: hidden;"> 
    <table border="0" cellspacing="0" bgcolor="#FFFFFF">
        ...
        <tr> 
          <td><div align="center" ><img src="../../../../img/fotos/aluno/<% Response.Write(num_cham_aluno_on_load) %>.jpg" height="500"></div></td>
        </tr>
        <tr>
          <td height="20">
...
<div id="lupa"  style="position:absolute; left:0px; top:0px; width:100%; height:<%response.Write(height_tela)%>px; z-index:2; background-color: #FFFFFF; layer-background-color: #000000; border: 1px none #000000; visibility: hidden;" ><a href="#" onClick="MM_showHideLayers('lupa','','hide')">fechar</a>
</div>  
...
<%	next%>  
 		  <td width="<%response.Write(width_scroll)%>" class="<%response.Write(class_tit)%>"></td>      
      </thead>
	  </tr>
     </table> 
		...
		%>
		  ...
		  <tr class="<%response.Write(cor)%>" id="<%response.Write("celula"&num_cham)%>">
			...
			<td width="<%response.Write(width_nome)%>" <%response.Write(rowspan)%> id="<%response.Write(num_cham)%>c2" onFocus="mudar_cor_focus(<%response.Write(parametros_chamada_jscript)%>);recuperarNomeAluno(<% response.Write(cod_cons)%>);recuperarOcorrencia(<% response.Write(cod_cons)%>);recuperarImgTbAluno(<%response.Write(num_cham)%>,<%response.Write(cod_cons)%>,'<%response.Write(javascript_periodo)%>');recuperarNota(<% response.Write(width_tab_abas)%>,<% response.Write(cod_cons)%>,<% response.Write(ano_letivo)%>,<% response.Write(curso)%>,<% response.Write(co_etapa)%>,'<%response.Write(vet_co_materia(0))%>','<% response.Write(Server.URLEncode(caminho_nota))%>','<% response.Write(opcao)%>',<% response.Write(num_periodo(0))%>)" onBlur="<%response.Write(onblur)%>(<%response.Write(parametros_chamada_jscript)%>)"><%response.Write(nome_aluno)%>
		       <a href="#" title="<% response.Write(nome_aluno_load) %>" onClick="centraliza(500,536);MM_showHideLayers('lupa','','show');mclosetime()"> <img src="../../../../img/lupa.png" alt="Detalhar aluno" border="0" /></a></td>
  ...
  <tr>
    <td width="<%response.Write(width_tb_dados_turma)%>"><table width="<%response.Write(width_tb_dados_turma)%>" border="1" bgcolor="#FFFFFF" cellspacing="0" cellpadding="0">
  <tr>
    <td width="60" height="15" class="form_tit"><div align="left">UNIDADE: </div></td>
    <td width="120" height="15" class="form_dado_texto"><div align="left"><%response.Write(no_unidade)%></div></td>
  </tr> 
  <tr>
    <td width="60" height="15" class="form_tit"><div align="left">CURSO: </div></td>
    <td width="120" height="15" class="form_dado_texto"><div align="left"><%response.Write(no_curso)%></div></td>
  </tr> 
  <tr>
    <td width="60" height="15" class="form_tit"><div align="left">ETAPA: </div></td>
    <td width="120" height="15" class="form_dado_texto"><div align="left"><%response.Write(no_etapa)%></div></td>
  </tr> 
  <tr>
    <td width="60" height="15" class="form_tit"><div align="left">TURMA: </div></td>
    <td width="120" height="15" class="form_dado_texto"><div align="left"><%response.Write(turma)%></div></td>
  </tr>       
</table></td>
...
%>	
    ...
    <a href="#" title="<% response.Write(nome_aluno_load) %>" onClick="centraliza(500,536);MM_showHideLayers('fundo','','show','alinha','','show');mclosetime()"><img src="../../../../img/fotos/aluno/<% response.Write(cod_cons_load) %>.jpg" alt="<% response.Write(nome_aluno_load) %>" width="50" height="60" border="0"></a>
<% end if%>    
    </div>
    </td>
