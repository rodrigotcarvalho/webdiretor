...
tipo_eve = request.form("status")
tit = request.form("tit")
conteudo = request.form("conteudo")
repete = request.form("repete")
...
response.Write(data_inclui)

if opt="i" then

Set RS_updt = server.createobject("adodb.recordset")

RS_updt.open "TB_Agenda", CON_WF, 2, 2 'which table do you want open
RS_updt.addnew

	RS_updt("TP_Evento")=tipo_eve
	RS_updt("NO_Evento")=tit
	RS_updt("TP_Calen")=tipo_arquivo
	RS_updt("EV_DT_IN")=data_inclui
	RS_updt("EV_Repete")=repete
	RS_updt("EV_DT_FI")=data_vig_inclui
...
sql_atualiza= "UPDATE TB_Agenda SET "&sql_un&sql_cu&sql_et&sql_tu&"TP_Calen='"&tipo_arquivo&"', NO_Evento ='"&tit&"', EV_DT_IN ='"&data_inclui&"',EV_DT_FI ='"&data_inclui&"', EV_Repete="&repete&" WHERE NE_Codigo = "& co_not
'sql_atualiza= "UPDATE TB_Agenda SET TP_Noticia='"&tipo_arquivo&"', Unidade ='"&unidade_grava&"', Curso ='"&curso_grava&"', Etapa ='"&etapa_grava&"', Turma ='"&turma_grava&"', NT_Titulo ='"&tit&"', NT_DT_Pb ='"&data_inclui&"', NT_DT_Vg='"&data_vig_inclui&"', NT_Conteudo='"&conteudo&"' WHERE NT_Codigo = "& co_not
...
SQL_doc = "DELETE * FROM TB_Agenda where NE_Codigo="&co_doc
