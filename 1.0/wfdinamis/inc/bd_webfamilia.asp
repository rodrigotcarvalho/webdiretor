...
function gravaRematricula(P_CO_Aluno, P_TP_Contrato)
...
hora = hora & ":"& min
	...
	RSrem.addnew	
		...
		RSrem("TP_Contrato") = P_TP_Contrato
	RSrem.update
	set RSrem=nothing	
...
Function modeloContratoAdendo(P_Unidade,P_Curso,P_Etapa,P_Turma,P_Contrato_Adendo)
		...
		'response.write(SQLRB)
		'response.End()
		RSRB.Open SQLRB, conw
total_contratos_adendos = 0
				'response.write(RSRB.EOF&"<BR>")
if not RSRB.EOF THEN

    if P_Contrato_Adendo = "C" then		
		while not RSRB.EOF
			'	response.write(RSRB("Contrato")&"<BR>")					
			if total_contratos_adendos=0 then 
				contratoAdendo = RSRB("Contrato")
			else
				contratoAdendo = contratoAdendo&"#!#"&RSRB("Contrato")		
			end if
			total_contratos_adendos=total_contratos_adendos+1
		RSRB.Movenext
		Wend
    else
		teste_adendo = ""
		'response.write(RSRB("Adendo")&"<BR>")			
		if isnull(RSRB("Adendo")) or RSRB("Adendo") = "" then
			...
			while not RSRB.EOF
				'response.write(RSRB("Adendo")&"<BR>")			
				if total_contratos_adendos=0 then 
					contratoAdendo = RSRB("Adendo")
				else
					if teste_adendo <> RSRB("Adendo") then	
						contratoAdendo = contratoAdendo&"#!#"&RSRB("Adendo")		
					end if
				end if
				total_contratos_adendos=total_contratos_adendos+1
				teste_adendo = RSRB("Adendo")	
			RSRB.Movenext
			Wend			
		end if	
...
END IF	
'response.End()
modeloContratoAdendo = contratoAdendo
end function
...
Function buscaOpcoesAdendo(P_Modelo_Adendo)

	Set RSRO = Server.CreateObject("ADODB.Recordset")
	SQLRO = "SELECT * FROM TB_AdendosXopcoes where Adendo = '"&P_Modelo_Adendo&"'"
	RSRO.Open SQLRO, conw
	
	total_opcoes = 0

	if not RSRO.EOF THEN
		
			while not RSRO.EOF
				
				if total_opcoes=0 then 
					wrkOpcoesAdendo = RSRO("Seq")&"#-#"&RSRO("Nome_opcao")
				else
					wrkOpcoesAdendo = wrkOpcoesAdendo&"#!#"&RSRO("Seq")&"#-#"&RSRO("Nome_opcao")		
				end if
				total_opcoes=total_opcoes+1
			RSRO.Movenext
			Wend
	ELSE
		wrkOpcoesAdendo = ""
	END IF
	buscaOpcoesAdendo = wrkOpcoesAdendo
end function

Function gravaOpcaoAdendo(p_cod, p_mod_adendo, p_opcao_adendo)

    Set RSrem = Server.CreateObject("ADODB.Recordset")
	SQLrem = "SELECT * FROM TB_Alunos_escolha_Adendos where CO_Matricula_Escola="&p_cod
	RSrem.Open SQLrem, conw
	
	if not RSrem.EOF then
		Set RS0 = Server.CreateObject("ADODB.Recordset")	
		SQL0 = "delete * FROM TB_Alunos_escolha_Adendos where CO_Matricula_Escola="&p_cod
		Set RS0 = conw.Execute(SQL0)		
	end if
	
	Set RSrem = server.createobject("adodb.recordset")
	RSrem.open "TB_Alunos_escolha_Adendos", conw, 2, 2 'which table do you want open
	RSrem.addnew	
		RSrem("CO_Matricula_Escola") = p_cod
		RSrem("Adendo") = p_mod_adendo
		RSrem("Seq") = p_opcao_adendo
	RSrem.update
	set RSrem=nothing	
