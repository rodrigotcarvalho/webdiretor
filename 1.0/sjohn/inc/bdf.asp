...
'pesos
	...
	periodo = periodo*1
if teste_va_pt=true and teste_va_pp1=true and teste_va_pp2=true and periodo < 4 then
	va_pt=va_pt*1
	va_pp1=va_pp1*1
	va_pp2=va_pp2*1
elseif teste_va_pt=true and teste_va_pp1=true and periodo = 4 then
	va_pt=va_pt*1
	va_pp1=va_pp1*1
...
if grava = "ok" then
	...
	if 	media_t="ok" and p1_lancada="ok" and p2_lancada="ok" and periodo<4 then		
'		media_p="ok"		
'			if periodo<5 then
...
'				end if
'			end if
			m1=((mt*va_pt)+(soma_prova1*va_pp1)+(soma_prova2*va_pp2))/(va_pt+va_pp1+va_pp2)
	...
	elseif 	media_t="ok" and p1_lancada="ok" and periodo=4 then
			m1=((mt*va_pt)+(soma_prova1*va_pp1))/(va_pt+va_pp1)
			...
			'm1=m1/10			
			m1 = formatNumber(m1,0)				
	elseif p1_lancada="ok" and periodo>4 then	
		m1=soma_prova1					
	else			
		m1=NULL	
	end if
...
'			Alterado por conta de e-mail de 11/05/2012
	...
	If RS0.EOF THEN	
		...
		RS.open "TB_Nota_F", CON, 2, 2 'which table do you want open
		RS.addnew
