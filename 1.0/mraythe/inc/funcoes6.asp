...
'end if						
			...
			elseif (mae=TRUE and fil=FALSE and in_co=TRUE and isnull(peso)) then	
						conta_filhas = 0
							...
							CONEXAONNa = "Select * from TB_Materia WHERE CO_Materia_Principal = '"& co_materia(c) &"' order by NU_Ordem_Boletim"
							Set RSNNa = CON0.Execute(CONEXAONNa)	
							
							if RSNNa.EOF then
								Response.Write("Erro ao localizar mat&eacute;rias filhas para "&co_materia(c)&" em TB_Materia")
								Response.end()
							else	
								while not RSNNa.EOF
				...
				for periodo=1 to qtd_periodos
					...
					'Calcula_Media_T_F_T_N(unidade, curso, co_etapa, turma, tp_modelo, tp_freq, dados_aluno(0), co_materia(c), vetor_filhas, CONn, tb_nota, periodo, "VA_Media2", outro)	
...
Function apura_resultado_geral_aluno (curso,etapa,vetor_medias, tipo)
	...
	Set RSra = Server.CreateObject("ADODB.Recordset")
	SQLra = "SELECT * FROM TB_Regras_Aprovacao where CO_Curso = '"&curso&"' and CO_Etapa = '"&etapa&"'"
	'response.Write(CAMINHO_pr&"<BR>"&SQLra&"<BR>")	
	RSra.Open SQLra, CON0	
...
'response.Write("NU_Valor_Apr:"&valor_apr&" ; ")	
'response.Write(md_aluno&" ; ")
'response.Write(qtd_dep&" ; ")
'response.Write(result_temp&"<BR>")
Next
'response.Write("<BR>")
'response.Write("NU_Valor_Apr:"&valor_apr&"<BR>")
'response.Write("NU_Valor_Dep:"&valor_dep&"<BR>")
'response.Write(qtd_dep)
'response.End()
if 	libera_resultado="s" then
	qtd_dep=qtd_dep*1
	if 	ficou_reprovado = "N" and ficou_recuperacao="N" and qtd_dep=0 then
		apura_resultado_geral_aluno=res_apr
	elseif ficou_reprovado = "S"  then
		apura_resultado_geral_aluno=res_rep
	elseif ficou_recuperacao="S" then	
		if session("ano_letivo")>=2017 and tipo="A" then
			if curso=2 then
				qtd_max_dep = 6
			else
				qtd_max_dep = 4
		...
		if qtd_dep>qtd_max_dep then
			apura_resultado_geral_aluno=RSra("NO_Expr_Cond_Falso_Abr")
		end if
	end if	
'response.Write(apura_resultado_geral_aluno&"-"&res_rep)
'response.End()		
else
