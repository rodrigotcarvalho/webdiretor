...
%>
		...
		Set RS0 = Server.CreateObject("ADODB.Recordset")
		SQL0 = "SELECT DISTINCT CO_Curso FROM TB_Unidade_Possui_Etapas where NU_Unidade="&session("u_pub")&" and CO_Curso<>'0'"
		RS0.Open SQL0, CON0
...
<%
'Essa combo da etapa chama tamb�m a rotina da disciplina
elseif opt="e2" then
...
<%'Essa combo da etapa chama tamb�m a rotina da avalia��o
elseif opt="e3" then
...
<%'Essa combo da etapa chama tamb�m a rotina do Per�odo
elseif opt="e4" then
...
%>
                      <select name="etapa" class="select_style" onchange="recuperarTurma(this.value);recuperarDisciplina(curso.value,this.value);">
                        <option value="999990" selected></option>
                        <%		
		Set RS0b = Server.CreateObject("ADODB.Recordset")
		SQL0b = "SELECT DISTINCT CO_Etapa FROM TB_Unidade_Possui_Etapas where NU_Unidade="&session("u_pub")&"AND CO_Curso='"&session("c_pub")&"'"
...
RSESQ.MOVENEXT
WEND%> 
</table><input name="qtd_itens" type="hidden" id="qtd_itens" value="<%response.Write(qtd_materias)%>">
...
%>
 ...
 <%
'Essa combo da turma chama tamb�m a submete o form 
 elseif opt="t2" then
...
' S� serve para gravar o valor de turma para as fun��es atualizar ocorr�ncias (AOC), Emitir Ficha e Emitir Boletim do Web Secretaria
elseif opt="t5" then
...
<%
' S� serve para gravar o valor de turma para a fun��o atualizar ocorr�ncias (AOC)
elseif opt="t6" then
...
%>
 <select name="turma" class="select_style" onchange="gravarTurma(this.value);recuperarPeriodo()">
                        <option value="999990" selected></option> 
    <%
		Set RS3 = Server.CreateObject("ADODB.Recordset")
		SQL3 = "SELECT * FROM TB_Turma where NU_Unidade="&session("u_pub")&"AND CO_Curso='"&session("c_pub")&"' AND CO_Etapa='" & session("e_pub") & "' order by CO_Turma" 
...
%>
                       <select name="mat_prin" class="select_style">
                        ...
                        <%
		Set RS5 = Server.CreateObject("ADODB.Recordset")
		SQL5 = "SELECT DISTINCT CO_Materia, NU_Ordem_Boletim FROM TB_Programa_Aula where CO_Etapa ='"& session("e_pub") &"' AND CO_Curso ='"& session("c_pub") &"' order by NU_Ordem_Boletim "
		RS5.Open SQL5, CON0
...
%>
<select name="periodo" class="select_style" id="periodo" onchange="GuardaPeriodo(this.value);recuperarMedia()">
                                      <option value="0" selected></option>
                                      <%
		Set RS4 = Server.CreateObject("ADODB.Recordset")
		SQL4 = "SELECT * FROM TB_Periodo order by NU_Periodo"
...
%>
<select name="mediainformada" class="select_style" id="mediainformada" onChange="GuardaMedia(this.value)">
<option value="0" selected></option>
<%i=0
while i<10.5 %>
                                      <option value="<%=i%>"> 
...
if (mae=TRUE and fil=TRUE and in_co=FALSE and isnull(peso)) or (mae=TRUE and fil=TRUE and in_co=FALSE) or (mae=TRUE and fil=FALSE and in_co=TRUE) then
	...
	Set RSS = Server.CreateObject("ADODB.Recordset")
	SQLS = "SELECT * FROM TB_Materia where CO_Materia_Principal ='"& wrk_materia &"' order by NU_Ordem_Boletim"
	RSS.Open SQLS, CON0
	wrk_co_materia_fil_check=1 
	while not RSS.EOF
		fn_co_mat_fil= RSS("CO_Materia")				
		if wrk_co_materia_fil_check=1 then
			fn_vetor_submateria=fn_co_mat_fil
		else
			fn_vetor_submateria=fn_vetor_submateria&"#!#"&fn_co_mat_fil			
		end if
		wrk_co_materia_fil_check=wrk_co_materia_fil_check+1 									
	RSS.MOVENEXT
	wend

	vetor_submateria_split = split (fn_vetor_submateria, "#!#")

	for sbm = 0 to ubound(vetor_submateria_split)	
		fn_co_mat_fil = vetor_submateria_split(sbm)
	 ...
	 for i= 0 to ubound(linha_pesos)
			...
			if linha_pesos(i)="PESO" then
				...
				nome_pesos_variaveis=split(nm_pesos_vars,"#!#")		
					...
					SQL_peso = "Select "&linha_pesos_variaveis(i)&" from "& tb &" WHERE CO_Matricula = "& co_matric & " AND CO_Materia_Principal = '"& wrk_materia &"' AND CO_Materia = '"& fn_co_mat_fil &"' AND NU_Periodo="&periodo
		...
		<%
			...
			%>
			 ...
			 <%else		
                ...
                <tr>
                    <td width="<%response.Write(width)%>"><%response.Write(sigla_periodo)%></td>
                    <td width="<%response.Write(width)%>" ><%response.Write(fn_co_mat_fil)%></td>               
                     <% 
                    Set RS3 = Server.CreateObject("ADODB.Recordset")
                    SQL_N = "Select * from "& tb &" WHERE CO_Matricula = "& co_matric & " AND CO_Materia_Principal = '"& wrk_materia &"' AND CO_Materia = '"& fn_co_mat_fil &"' AND NU_Periodo="&periodo
                    ...
                    for n= 0 to ubound(nome_variaveis)
                        ...
                        if RS3.EOF then 
                            ...
                            if variaveis_bd(n)="CALCULADO" then
                                valor="&nbsp;"
                                'Nesse caso o valor � calculado pela fun��o calcular_nota chamada mais abaixo
                            else
		...
		<%
			...
			%>
			 ...
			 <%else		
                ...
                <tr>
                    <td width="<%response.Write(width)%>" class="zoom_texto"><%response.Write(sigla_periodo)%></td>
                    <td width="<%response.Write(width)%>" class="zoom_texto"><%response.Write(fn_co_mat_fil)%></td>               
                     <% 
                    Set RS3 = Server.CreateObject("ADODB.Recordset")
                    SQL_N = "Select * from "& tb &" WHERE CO_Matricula = "& co_matric & " AND CO_Materia_Principal = '"& wrk_materia &"' AND CO_Materia = '"& fn_co_mat_fil &"' AND NU_Periodo="&periodo
                    Set RS3 = CON_N.Execute(SQL_N)			 
...
co_matric=Request.Form("matric_pub")
	...
	if RS4.EOF then
	%>
	<div align="left"><%Response.Write(Server.URLEncode("&nbsp;&nbsp;�ltima Ocorrencia Registrada:&nbsp;Sem Ocorr�ncias"))%></div>
	<%else
	...
	%>
	<div align="left"><%Response.Write(Server.URLEncode("&nbsp;&nbsp;�ltima Ocorrencia Registrada:&nbsp;"&data_ocor&", referente a: "&no_ocorrencia))%></div>
	<%end if
