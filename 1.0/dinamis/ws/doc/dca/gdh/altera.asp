...
for i=0 to ubound(conjunto_dados)	
	...
	'response.Write(SQL_N&"<BR>")					
					...
					soma_faltas = soma_faltas+total_faltas
					soma_cargas = soma_cargas+carga_materia
...
'				end if			
	...
	'		response.Write(cod_cons&"-"&soma_faltas&"<BR>")
			soma_faltas=soma_faltas*1
			soma_cargas=soma_cargas*1
			if soma_cargas="" or isnull(soma_cargas) or soma_cargas=0 then
				soma_cargas=1
			end if
			
			frequencia_final=(soma_cargas-soma_faltas)/soma_cargas
			frequencia_final=frequencia_final*100
