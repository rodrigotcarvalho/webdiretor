...
for cnn=0 to ubound(colunas_nome_notas)
	...
	Notas_Tit(2, coluna_afetada).AddText "<div align=""center"">"&colunas_nome_notas(cnn)&"</div>", "size=7;alignment=center;indenty="&indenty&"; html=true", Font 
...
next			
			...
			if tipo_freq="D" then
				...
				for ccc= 0 to UBOUND(colunas_notas_calc)
					IF colunas_notas_calc(ccc) = "RES" then
						campo_resultado=ccc
					end if
			...
			campo_resultado=campo_resultado+2	
...
'					Row.Cells(1).AddText no_materia_exibe, param_materias
					...
					for nn=1 to total_campos		
						...
						if notas_exibe="" or isnull(notas_exibe) then
							...
							if nn=campo_resultado then
								if r=0 then
									resultados_apurados=nota
									r=r+1
								else
									resultados_apurados=resultados_apurados&"#!#"&nota					
								end if								
							end if												
