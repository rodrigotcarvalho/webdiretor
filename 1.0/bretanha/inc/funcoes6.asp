...
Function caminho_notas(CON, tb_nota, outro)
		...
		elseif tb_nota="TB_NOTA_G" then
			CAMINHO_n=CAMINHO_ng
		elseif tb_nota="TB_NOTA_H" then
			CAMINHO_n=CAMINHO_nh
		elseif tb_nota="TB_NOTA_I" then
			CAMINHO_n=CAMINHO_ni
		elseif tb_nota="TB_NOTA_J" then
			CAMINHO_n=CAMINHO_nj
		...
		elseif tb_nota="TB_NOTA_N" then
			CAMINHO_n=CAMINHO_nn	
		elseif tb_nota="TB_NOTA_O" then
			CAMINHO_n=CAMINHO_no	
		elseif tb_nota="TB_NOTA_P" then
			CAMINHO_n=CAMINHO_np	
		elseif tb_nota="TB_NOTA_Q" then
			CAMINHO_n=CAMINHO_nq	
		elseif tb_nota="TB_NOTA_R" then
			CAMINHO_n=CAMINHO_nr	
		elseif tb_nota="TB_NOTA_S" then
			CAMINHO_n=CAMINHO_ns	
		elseif tb_nota="TB_NOTA_T" then
			CAMINHO_n=CAMINHO_nt	
		elseif tb_nota="TB_NOTA_U" then
			CAMINHO_n=CAMINHO_nu	
		elseif tb_nota="TB_NOTA_V" then
			CAMINHO_n=CAMINHO_nv	
		elseif tb_nota="TB_NOTA_W" then
			CAMINHO_n=CAMINHO_nw	
		elseif tb_nota="TB_NOTA_X" then
			CAMINHO_n=CAMINHO_nx	
		elseif tb_nota="TB_NOTA_Y" then
			CAMINHO_n=CAMINHO_ny	
		elseif tb_nota="TB_NOTA_Z" then
			CAMINHO_n=CAMINHO_nz																																																															
		else
			CAMINHO_n = "ERRO"					
		end if
...
'if vetor_aluno = 20190014 then
'response.write(co_materia(f2)&"-"&media_calc1)
'if co_materia(f2) = "Mat2" then
'response.end()
...
' vetor_aluno=vetor_aluno*1				
' if vetor_aluno = 20190014 then
' response.write(co_materia(f2)&"-"&media_calc1&"-"&modifica_result)
' if co_materia(f2) = "Mat2" then
' response.end()
...
'response.write("3<BR>")
	...
	'response.write("4<BR>")
							...
							if modifica_result <> "N" then
								resultado_final = modifica_result
								media_final=formatnumber(media_anual,1)	
							end if															
						else	
...
Function Calc_Med_An_Fin(unidade, curso, co_etapa, turma, vetor_aluno, P_vetor_materia, caminho_nota, tb_nota, qtd_periodos, periodo_m2, periodo_m3,tipo_calculo, outro)
...
'response.Write(vetor_aluno&"<BR>")
		...
		ABRIRn = "DBQ="& caminho_nota & ";Driver={Microsoft Access Driver (*.mdb)}"
		CONn.Open ABRIRn	
'dados_aluno(0)=dados_aluno(0)*1	
'if 	dados_aluno(0) =20190014 then 
'response.Write(	vetor_aluno&">="&medias_necessarias&"<BR>")	
'end if			
	
	'	response.Write("-"&P_vetor_materia&"-<BR>")

	alunos= split(vetor_aluno,"#$#")	
'	vetor_materia = ""
	vetor_materia= split(P_vetor_materia,"#!#")
...
'	response.Write(alunos(a))
		...
		For c=0 to ubound(vetor_materia)
			Set RS = Server.CreateObject("ADODB.Recordset")
			SQL = "SELECT * FROM TB_Programa_Aula where CO_Etapa ='"& co_etapa &"' AND CO_Curso ='"& curso &"' AND CO_Materia ='"& vetor_materia(c) &"'"
	...
	RS.Open SQL, CON0
			...
			if (mae=TRUE and fil=FALSE and in_co=FALSE and isnull(peso)) then
'response.Write("<BR> TFFN -"&dados_aluno(1)&" CO_Matricula ="& dados_aluno(0)&"&"&vetor_materia(c) &"<BR>")	
				for periodo=1 to qtd_periodos
					Set RSn = Server.CreateObject("ADODB.Recordset")
					SQLn = "SELECT * FROM "&tb_nota&" where CO_Matricula ="& dados_aluno(0) &" AND CO_Materia ='"& vetor_materia(c) &"' AND CO_Materia_Principal ='"& vetor_materia(c) &"' AND NU_Periodo="&periodo				
					RSn.Open SQLn, CONn
...
'							response.Write(vetor_materia(c) &"_"&rec_lancado&"-"&	dados_aluno(0) &" mr "&media_rec&"<BR>")										
						else
							if periodo=1 then
...
'response.Write(dividendo_ma&"="&dividendo_anual_ms2&"+"&dividendo_anual_ms3&"<BR>")						
...
'end if	
				if qtd_medias>=medias_necessarias then
					...
					media_anual=formatnumber(media_anual,1)		
'response.Write(media_anual&"="&dividendo_ma&"/"&divisor_ma&"<BR>")			
					media_anual=media_anual*1						
					'if media_anual>67 and media_anual<70 then
					'	media_anual=70
					'end if	
'response.Write("MREC "&media_rec&"-"&tipo_calculo&"-"&dados_aluno(0)&"<BR><BR>")						
					if tipo_calculo="anual" then
'	response.write("5<BR>")
						resultado=regra_aprovacao(curso,co_etapa,media_anual,"&nbsp;","&nbsp;","&nbsp;","&nbsp;","anual")
						media_anual = formatNumber(media_anual,1)
						resultado_materia=resultado		
						vetor_resultado = split(resultado, "#!#")
						tipo_media = "MF"
'	response.write("6<BR>")
						modifica_result = Verifica_Conselho_Classe(vetor_aluno, vetor_materia(c), tipo_media, outro)
						if modifica_result <> "N" then
							resultado = media_anual&"#!#"&modifica_result
						else
							tipo_media = "RF"
'response.write("7<BR>")
							modifica_result = Verifica_Conselho_Classe(vetor_aluno, vetor_materia(c), tipo_media, outro)	
...
'if 	dados_aluno(0) =20110043 then 
...
'end if			
							...
							tipo_media = "RF"				
'	response.write("8<BR>")
							modifica_result = Verifica_Conselho_Classe(dados_aluno(0), vetor_materia(c) , tipo_media, outro)

							if modifica_result <> "N" then
							'	resultado = Calc_Med_An_Fin(unidade, curso, co_etapa, turma, vetor_aluno, vetor_materia(c), caminho_nota, tb_nota, qtd_periodos, periodo_m2, periodo_m3,"anual", outro)
							'	vetor_resultado = split(resultado, "#!#")
							'	media_final = vetor_resultado(0)
							'	media_final=formatnumber(media_final,1)
							'	resultado = media_final&"#!#"&modifica_result
...
'if 	dados_aluno(0) =20040135 and vetor_materia(c) = "MAT1" then 
'response.Write(rec_lancado&"-"&curso&"-"&co_etapa&" ma "&media_anual&" rec "&media_rec&"<BR>")		
'end if						
		...
		if tipo_calculo="recuperacao" or tipo_calculo="final" or tipo_calculo="waboletim" then
			if nota_aux_m2_1="&nbsp;" then
				...
				if tipo_calculo="waboletim" then
					...
					resultado2_waboletim=resultado2	
				end if	
				
				if session("trava")="s" then
					m2_aluno=m1_aluno
					resultado2="rep"
					resultado="REP"					
				end if		
...
Function apura_resultado_aluno2 (cod_cons, curso,etapa,vetor_medias,vetor_materia)
response.Write(vetor_materia)


	co_materia_vetor= split(vetor_materia,"#!#")	
...
'response.Write("---"&res_aluno&"<BR>")				
		if (res_aluno="rep" or res_aluno="dep") and result_temp <> "rep" and result_temp <> "APC"   then
			result_temp=res_aluno		
			if modifica_result = "N" then
			response.Write(cod_cons&" "&co_materia_vetor(r)&"T"&outro&"<BR>")
				modifica_result = Verifica_Conselho_Classe(cod_cons, co_materia_vetor(r), "T", outro)
				if modifica_result <> "N" then				
			...
			if res_aluno="" or isnull(res_aluno) or res_aluno="&nbsp;" or res_aluno=" "then
				...
				if result_temp <> "rep" and result_temp <> "APC" then
					result_temp=res_aluno
					if res_aluno = "dep" then
						qtd_dep=qtd_dep+1		
					end if
...
'response.End()
	...
	if qtd_dep>0 then
		qtd_max_dep=qtd_max_dep*1
		if qtd_dep>qtd_max_dep then
			apura_resultado_aluno_temp=res_rep	
		else	
			apura_resultado_aluno_temp=res_dep	
		end if	
	elseif result_temp="apr" then
		apura_resultado_aluno_temp=res_apr
	elseif result_temp="rep" then
		apura_resultado_aluno_temp=res_rep
	end if	
else
	apura_resultado_aluno_temp="&nbsp;"		
end if	

if modifica_result<>"N" then
	apura_resultado_aluno2 = modifica_result
else
	apura_resultado_aluno2 = apura_resultado_aluno_temp
end if
'response.Write(apura_resultado_aluno2)
'response.End()
end function	
